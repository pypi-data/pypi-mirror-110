...
class Import:
    MIGRATION_DIR = '/efs-data/migration'
    ...
    def run(self):
        ...
        exec_cmd("gbd-download-package %s %s" % (migration_folder,
                                                 self.collection))
        for package in os.listdir(migration_folder):
            print("Importing %s" % package)
            exec_cmd("gbd-import-snapshot %s" % os.path.join(migration_folder, package))
