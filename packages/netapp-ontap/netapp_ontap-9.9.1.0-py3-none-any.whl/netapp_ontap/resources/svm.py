...
Example: 02c9e252-41be-11e9-81d5-00a0986138f7 """
    ...
    """links,aggregates.links,aggregates.name,aggregates.uuid,aggregates_delegated,certificate.links,certificate.name,certificate.uuid,cifs.links,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.links,ipspace.name,ipspace.uuid,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,nfs,nis,nsswitch,nvme,qos_policy.links,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,s3.links,s3.certificate,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.links,snapshot_policy.name,snapshot_policy.uuid,state,subtype,uuid,volume_efficiency_policy.links,volume_efficiency_policy.name,volume_efficiency_policy.uuid,"""
    ...
    """aggregates.name,aggregates.uuid,certificate.name,certificate.uuid,cifs.allowed,comment,fcp,ip_interfaces,iscsi,language,max_volumes,name,nfs,nsswitch,nvme,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,snapmirror,snapshot_policy.name,snapshot_policy.uuid,state,volume_efficiency_policy.name,volume_efficiency_policy.uuid,"""
    ...
    """aggregates.name,aggregates.uuid,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.name,ipspace.uuid,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,nfs,nis,nsswitch,nvme,routes,s3.certificate,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.name,snapshot_policy.uuid,subtype,volume_efficiency_policy.name,volume_efficiency_policy.uuid,"""
...
"""
    ...
    if CLICHE_INSTALLED:
        ...
        def svm_show(
            fields: List[Choices.define(["aggregates_delegated", "comment", "language", "max_volumes", "name", "state", "subtype", "uuid", "*"])]=None,
        ) -> ResourceTable:
