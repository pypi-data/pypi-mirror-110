...
__pdoc__ = {
    ...
    "PerformanceMonitoredFilesMetric.performance_monitored_files_metric_show": False,
    "PerformanceMonitoredFilesMetric.performance_monitored_files_metric_create": False,
    "PerformanceMonitoredFilesMetric.performance_monitored_files_metric_modify": False,
    "PerformanceMonitoredFilesMetric.performance_monitored_files_metric_delete": False,
}


class PerformanceMonitoredFilesMetricSchema(ResourceSchema):
...
def _get_field_list(field: str) -> Callable[[], List]:
    def getter():
        return [getattr(r, field) for r in PerformanceMonitoredFilesMetric.get_collection(fields=field)]
    return getter
...
async def _wait_for_job(response: NetAppResponse) -> None:
    ...
    while True:
        ...
        if job.state == "failure":
            raise NetAppRestError("PerformanceMonitoredFilesMetric modify job failed")
...
class PerformanceMonitoredFilesMetric(Resource):
    ...
    _path = "/api/storage/monitored-files/{monitored_file[uuid]}/metrics"
    _keys = ["monitored_file.uuid"]
    ...
    def get_collection(
        ...
        **kwargs
        ...
        r"""Retrieves historical performance metrics for the monitored file."""
        return super()._get_collection(*args, connection=connection, max_records=max_records, **kwargs)

    get_collection.__func__.__doc__ += "\n\n---\n" + inspect.cleandoc(Resource._get_collection.__doc__)

    if CLICHE_INSTALLED:
        @cliche.command(name="performance monitored files metric show")
        def performance_monitored_files_metric_show(
            monitored_file_uuid,
            ...
            """Fetch a list of PerformanceMonitoredFilesMetric resources
            ...
            return PerformanceMonitoredFilesMetric.get_collection(
                monitored_file_uuid,
                **kwargs
            )
