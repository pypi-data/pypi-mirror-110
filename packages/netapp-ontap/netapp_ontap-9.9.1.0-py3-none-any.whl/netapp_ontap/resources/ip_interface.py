...
Example: 1cd8a442-86d1-11e0-ae1c-123478563412 """
    ...
    """links,ddns_enabled,dns_zone,enabled,ip,ipspace.links,ipspace.name,ipspace.uuid,location,metric,name,scope,service_policy.links,service_policy.name,service_policy.uuid,services,state,statistics,svm.links,svm.name,svm.uuid,uuid,vip,"""
    ...
    """ddns_enabled,dns_zone,enabled,ip,location,name,service_policy.name,service_policy.uuid,"""
    ...
    """ddns_enabled,dns_zone,enabled,ip,ipspace.name,ipspace.uuid,location,name,scope,service_policy.name,service_policy.uuid,svm.name,svm.uuid,vip,"""
...
* [`DOC /network/ip/interfaces`](#docs-networking-network_ip_interfaces)"""
    ...
    if CLICHE_INSTALLED:
        ...
        def ip_interface_show(
            fields: List[Choices.define(["ddns_enabled", "dns_zone", "enabled", "name", "scope", "services", "state", "uuid", "vip", "*"])]=None,
        ) -> ResourceTable:
