...
with HostConnection("<mgmt-ip>", username="admin", password="password", verify=False):
    ...
    resource.prepopulate.exclude_dir_paths = ["/dir1/dir11"]
    resource.post(hydrate=True)
    print(resource)
...
class FlexcacheSchema(ResourceSchema):
    ...
    r""" Specifies whether or not a FlexCache volume has global file locking mode enabled. Global file locking mode is a mode where protocol read locking semantics are enforced across all FlexCaches and origins of a FlexCache volume. """
...
Example: 1cd8a442-86d1-11e0-ae1c-123478563412 """
    ...
    """links,aggregates.links,aggregates.name,aggregates.uuid,constituents_per_aggregate,dr_cache,global_file_locking_enabled,guarantee,name,origins,path,size,svm.links,svm.name,svm.uuid,use_tiered_aggregate,uuid,"""
    ...
    """aggregates.name,aggregates.uuid,constituents_per_aggregate,dr_cache,global_file_locking_enabled,guarantee,name,origins,path,prepopulate,size,svm.name,svm.uuid,use_tiered_aggregate,"""
    ...
    if CLICHE_INSTALLED:
        ...
        def flexcache_show(
            ...
            Args:
                ...
                global_file_locking_enabled: Specifies whether or not a FlexCache volume has global file locking mode enabled. Global file locking mode is a mode where protocol read locking semantics are enforced across all FlexCaches and origins of a FlexCache volume.
                name: FlexCache name
    ...
    if CLICHE_INSTALLED:
        ...
        async def flexcache_modify(
            ...
            Args:
                ...
                global_file_locking_enabled: Specifies whether or not a FlexCache volume has global file locking mode enabled. Global file locking mode is a mode where protocol read locking semantics are enforced across all FlexCaches and origins of a FlexCache volume.
                query_global_file_locking_enabled: Specifies whether or not a FlexCache volume has global file locking mode enabled. Global file locking mode is a mode where protocol read locking semantics are enforced across all FlexCaches and origins of a FlexCache volume.
                name: FlexCache name
