...
#### Test PyPI
    ...
    `python -m twine upload --skip-existing --repository pf-dre-database-test --config-file .pypirc dist/test/*`

#### PyPI
    `python setup.py sdist --dist-dir=dist/prod/ bdist_wheel --dist-dir=dist/prod/`
    `python -m twine upload --skip-existing --repository pf-dre-database --config-file .pypirc dist/prod/*`
