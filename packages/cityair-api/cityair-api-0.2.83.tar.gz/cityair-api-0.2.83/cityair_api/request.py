...
class CityAirRequest:
    ...
    def _make_request(self, method_url: str, *keys: str,
        ...
        **kwargs : dict
        ...
        if "Packets" in keys and not response_data.get("Packets"):
            self.logger.warning("There are no packets available.\n"
                                "url:%s\n"
                                "filter%s", url,
    ...
    def get_device_data(self, serial_number: str, start_date=None,
        ...
        packets, raw_value_types = self._make_request(DEVICES_PACKETS_URL,
                                                      'Packets',
                                                      "MeasureSchemeItems",
                                                      Filter=filter_,
                                                      silent=False)
        if raw_value_types:
            value_types = dict(zip(
                    [info['ValueType'] for info in raw_value_types],
                    [info['TypeName'] for info in raw_value_types]
                    ))
        else:
            value_types = self._device_value_types
        ...
        if format == 'dict':
            ...
            for col in values_cols:
                ...
                value_name = value_types[int(value_id)]
            ...
            for col in list(
                ...
                _, device_id, value_id = col.split(' ')
                serial = self._device_by_id[int(device_id)]
                value_name = value_types.get(int(value_id), "unknown")
    ...
    def get_stations(self, format: str = 'list',
        ...
        df = prep_df(df, index_col='id', dropna=False,
                     cols_to_unpack=['coordinates'])
        if "devices_manual" in df.columns and "devices_auto" in df.columns:
            ...
            df['devices'] += df['devices_manual'].apply(
                    lambda links: [self._device_by_id.get(link.get('DeviceId'))
                                   for link in links] if links else [])
            df['devices'] = df['devices'].apply(
                    lambda x: x if isinstance(x, list) else [])
            df.drop(['devices_auto', 'devices_manual'], axis=1, inplace=True,
                    errors="ignore")
        elif not stations_data:
            df = pd.DataFrame()
        else:
            device_ids = df["device_links"].apply(
                    lambda links: {link.get("DeviceId") for link in links})
            serials = device_ids.apply(lambda ids: list(chain(
                    *[self._device_and_children_by_id.get(id_) for id_ in
                      ids])))
            df["devices"] = serials
        df['location'] = df.get("location", pd.Series()).apply(
            lambda id_: locations.get(id_, None) if id_ else None)
        if not df.empty:
            if not include_3rd_party:
                df = df[df["devices"].apply(lambda serials: not serials or any(
                        filter(lambda serial: serial.startswith("CA"),
                               serials)))]
            if not include_offline:
                df = df[df['is_online']]
        if format == 'dicts':
