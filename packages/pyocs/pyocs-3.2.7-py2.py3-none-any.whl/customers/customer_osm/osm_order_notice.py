...
class OsmOrderNotice:
    ...
    _ocs_id_base_link = 'https://ocs.gz.cvte.cn/tv/Tasks/view/range:all/'
    ...
    def send_mail_for_ocs_with_sw_lock(self, searchid):
        ...
        if 0 == ocs_number:
            return
        for ocs in ocs_list:
            if sw.get_enable_software_list(ocs) is None:
                ocs_request = pyocs_demand.PyocsDemand(ocs)
                sw_engineer = ocs_request.get_ocs_software_engineer()
                sw_lock_receivers = sw.get_email_addr_from_ocs(sw_engineer)
