...
class OsmOrderAssign:
    ...
    def get_distribute_rule_sheet(self):
        ...
        if file.exists() is False:
            ...
            map_list = kb.get_osm_distribute_mapping_info_by_user("OSM订单分配规则表")
            ...
            for id in id_list:
                ...
                for ocs in ocs_list:
                    ...
                    if '' == assigner:  # 缺少分配规则
                        ...
                        continue
                    elif 'ENG' in assigner:
                        user = assigner.strip('ENG')
                        ret = sw.set_engineer(ocs, user)
                        if ret:
                            ocs_dist_success_list.append(ocs)
                            continue
                        else:
                            ocs_dist_fail_list.append(ocs)
                    elif 'AUD' in assigner:
                        user = assigner.strip('AUD')
                        ret = sw.set_sw_audit_user(ocs, user)
                        if ret:
                            ocs_dist_success_list.append(ocs)
                            continue
                        else:
