...
class OpmOrderStatus:
    ...
    def send_mail_for_exception_ocs(self, ocs, content, type):
        ...
        osm = ocs_request.get_software_tracker()
        opm = ocs_request.get_opm()
        if osm is "无" or opm is "无":
            return

        if type == 'type3':
            if opm:
                receivers = sw.get_email_addr_from_ocs(sw_engineer) + ',' + sw.get_email_addr_from_ocs(opm)
            else:
                receivers = sw.get_email_addr_from_ocs(sw_engineer) + ',' + sw.get_email_addr_from_ocs(osm)
            cc = self.cc_regular
        elif type == 'type4':
            receivers = sw.get_email_addr_from_ocs(sw_engineer)
            cc = self.cc_regular
        elif type == 'type8':
            receivers = sw.get_email_addr_from_ocs(osm)
            cc = self.cc_regular
        elif type == 'type9':
            receivers = sw.get_email_addr_from_ocs(sw_engineer)
            cc = self.cc_regular
        ...
        mail.send_email(self.sender, receivers, cc, self.subject, content)
    ...
    def set_enable_software_finish_test_status(self, ocs_list, status):
        ...
        ret_status_dict = {}
        _ocs_link_Prefix = 'https://ocs.gz.cvte.cn/tv/Tasks/view/'
