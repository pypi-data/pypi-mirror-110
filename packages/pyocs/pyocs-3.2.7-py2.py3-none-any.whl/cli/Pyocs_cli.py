...
def mid_ocs_list(search_id, content):
    ...
    if content == 'ocs_id':
        ...
        si = pyocs_searchid.PyocsSearchid(search_id, 'https://ocs.gz.cvte.cn/tv/Reqs/sample_order_index/range:all/SearchId:')
...
def refresh(txt, sw):
    ...
    if txt and ".txt" in txt:
        ...
        for sw_info in sw_list:
            ...
            if not result:
                print(sw_info + "软件未找到，请确认软件版本是否正确!")
            else:
...
@click.command()
@click.option('--searchid', help='通过searchid指定搜索范围')
@click.option('--type', default='mulVer', help='备注类型，mulVer:多版本软件；swLock:被锁定的软件')
...
def ocs_get_link(boardcode):
    ...
    no_Found_Url = "http://ocs.gz.cvte.cn/tv/Tasks/view/range:my/1"
...
@click.command()
@click.option('--tag', type=int, help='GAIA类型[1：GAIA; 3：GAIA AI]')
@click.option('--mac', type=str, help='mac地址，不需要带间隔符，如：b4ada328d99a')
