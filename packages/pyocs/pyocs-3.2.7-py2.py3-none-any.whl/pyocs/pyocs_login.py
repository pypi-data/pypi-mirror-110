...
class PyocsLogin:
    # 全局信息
    __Login_Address = "https://ocs.gz.cvte.cn/users/login"  # OCS登陆站点
    __OCS_Test_URL = "http://ocs.gz.cvte.cn/tv"  # 测试用站点
    Access_URL = ['http://ocs.gz.cvte.cn/tv', 'https://ocs.gz.cvte.cn/tv']  # 任何你想要访问的OCS站点
    _instance = None
    _cookies_file = ""
        ...
        try:
            ...
            with open(_account_json_file, 'r') as load_file:
                ...
                if re.match("^.*=$", account_dict["Username"]):
                    #存储的是密文
                    account_dict["Username"] = k.decrypt(base64.b64decode(account_dict["Username"])).decode()
                    account_dict["Password"] = k.decrypt(base64.b64decode(account_dict["Password"])).decode()
                else:
                    #存储的是明文
                    encrypt = False
                ...
                if ~encrypt:
                    ...
                    account_dict_encrypt["Username"] = str(base64.b64encode(k.encrypt(account_dict["Username"])), encoding="utf-8")
                    account_dict_encrypt["Password"] = str(base64.b64encode(k.encrypt(account_dict["Password"])), encoding="utf-8")
            ...
            while pw1 != pw2:
                ...
                if pw1 == pw2:
                    account_dict_encrypt["Username"] = str(base64.b64encode(k.encrypt(username)), encoding="utf-8")
                    account_dict_encrypt["Password"] = str(base64.b64encode(k.encrypt(pw1)), encoding="utf-8")
                    with open(_account_json_file, 'w') as load_file:
                        load_file.write(str(json.dumps(account_dict_encrypt, indent=4)))
                    encrypt = True
                    break

            return {"Username" : username  , "Password" : pw1}
