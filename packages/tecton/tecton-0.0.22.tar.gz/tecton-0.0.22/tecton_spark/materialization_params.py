...
from tecton_proto.data.feature_package_pb2 import MaterializationParams as MaterializationParamsProto
...
class MaterializationParams(object):
    ...
    def __init__(
        ...
        feature_package_or_view: Optional[FeaturePackageOrView] = None,
        **kwargs,
    ):
        """
        ...
        if feature_package:
            self.feature_package_or_view = FPBackedFeaturePackageOrView(feature_package)
        elif feature_package_or_view:
            self.feature_package_or_view = feature_package_or_view
        else:
            raise AssertionError("One of feature_package or feature_package_or_view must be provided")
    ...
    @staticmethod
    def from_proto(feature_package: FeaturePackage):
        proto = feature_package.materialization_params
        return MaterializationParams(
            feature_package=feature_package,
            min_scheduling_interval=time_utils.proto_to_duration(
                feature_package_utils.min_scheduling_interval(feature_package)
            ),
            batch_schedule=time_utils.proto_to_duration(
                feature_package_utils.batch_materialization_schedule(feature_package)
            ),
            allowed_upstream_lateness=time_utils.proto_to_duration(proto.allowed_upstream_lateness),
            from_timestamp=pendulum.instance(proto.start_timestamp.ToDatetime()),
            data_partitions_coalesce_override=proto.data_partitions_coalesce_override,
        )

    @staticmethod
    def from_package_or_view(feature_package_or_view: FeaturePackageOrView):
        return MaterializationParams(
            feature_package_or_view=feature_package_or_view,
            min_scheduling_interval=feature_package_or_view.min_scheduling_interval,
            batch_schedule=feature_package_or_view.batch_materialization_schedule,
            allowed_upstream_lateness=feature_package_or_view.allowed_upstream_lateness,
            from_timestamp=feature_package_or_view.start_timestamp,
            data_partitions_coalesce_override=feature_package_or_view.data_partitions_coalesce_override,
        )

    def to_proto(self):
        proto = MaterializationParamsProto()
        proto.batch_materialization_schedule.FromTimedelta(self.batch_schedule)
        proto.allowed_upstream_lateness.FromTimedelta(self.allowed_upstream_lateness)
        proto.start_timestamp.FromDatetime(self.from_timestamp)
        proto.data_partitions_coalesce_override = self.data_partitions_coalesce_override
        return proto
    ...
    def get_tile_interval(self):
        return time_utils.assert_is_round_seconds(self.feature_package_or_view.get_tile_interval)
    ...
    def most_recent_anchor(self, timestamp: pendulum.datetime) -> pendulum.datetime:
        ...
        return convert_epoch_to_datetime(anchor_time, self.feature_package_or_view.get_feature_store_format_version)
    ...
    def align(self, timestamp: pendulum.datetime) -> pendulum.datetime:
        ...
        return convert_epoch_to_datetime(aligned_time, self.feature_package_or_view.get_feature_store_format_version)
    ...
    def align_timestamp_left(self, timestamp: datetime.datetime) -> int:
        ...
        is_continuous_aggregate = (
            self.feature_package_or_view.is_temporal_aggregate
            and self.feature_package_or_view.is_continuous_temporal_aggregate
        )
        ...
        version = self.feature_package_or_view.get_feature_store_format_version
        scheduling_interval = convert_pendulum_duration_for_version(self.min_scheduling_interval, version)
