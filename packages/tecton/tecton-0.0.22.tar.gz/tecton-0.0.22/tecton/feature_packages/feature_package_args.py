...
class FeatureAggregation(object):
    """
    This class describes a single aggregation that is applied in :class:`TemporalAggregateFeaturePackage`.

    :param column: Column name of the feature we are aggregating.
    :type column: str
    :param function: One of built-in aggregation functions.
    ...
    In addition to numeric aggregations, :class:`TemporalAggregateFeaturePackage` supports "last-n" aggregation that
    will compute last N distinct values for the column by timestamp. Right now only string column types are supported as inputs
    to this aggregation; that is, the resulting feature value will be a list of strings.

    You can use it with :class:`TemporalAggregateFeaturePackage` via ``last_distinct()`` helper like this:

    .. code-block:: python

        from tecton.aggregation_functions import last_distinct
        my_tfp = TemporalAggregateFeaturePackage(
        ...
...
class EMRClusterConfig:
    ...
    This class describes the attributes of the new clusters which are created in EMR during FeaturePackage
    materialization jobs. You can configure options of these clusters, like cluster size and extra pip depedencies.

    Note on ``extra_pip_dependencies``: This is a list of PyPI package names that will be installed during
    materialization. These libraries will only be available to use inside Spark UDFs. For example, if you set
    ``extra_pip_dependencies=["tensorflow"]``, you can use it in your transformation like this:
    ...
    The import has to be within the UDF body. Putting it at the top of the file or inside transformation
    function won't work.
    """

    instance_type: Optional[str] = None
    """Instance type for the cluster.
    Must be a valid type as listed in https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-supported-instance-types.html.
    If not specified, a value determined by the Tecton backend is used"""
    instance_availability: Optional[str] = None
    """Instance availability for the cluster : spot / on_demand.
    If not specified, default is spot"""
    number_of_workers: Optional[int] = None
    """Number of instances for the materialization job.
    If not specified, a value determined by the Tecton backend is used"""
    root_volume_size_in_gb: Optional[int] = None
    """Size of the root volume in GB per instance for the materialization job.
    If not specified, a value determined by the Tecton backend is used"""
    ...
    """Map of Spark configuration options and their respective values that will be passed to the
    FeaturePackage materialization Spark cluster. Currently, we support only the following options:
        - spark.driver.memory
...
class DatabricksClusterConfig:
    ...
    This class describes the attributes of the new clusters which are created in Databricks during FeaturePackage
    materialization jobs. You can configure options of these clusters, like cluster size and extra pip depedencies.
    ...
    instance_type: Optional[str] = None
    """Instance type for the cluster.
    Must be a valid type as listed in  https://databricks.com/product/aws-pricing/instance-types.
    If not specified, a value determined by the Tecton backend is used"""
    instance_availability: Optional[str] = None
    """Instance availability for the cluster : spot / on_demand.
    If not specified, default is spot"""
...
class MonitoringConfig:
    ...
    This class describes the FeaturePackage materialization freshness and alerting configurations. Requires
    ...
    :param alert_email: Email that alerts for this FeaturePackage will be sent to.
