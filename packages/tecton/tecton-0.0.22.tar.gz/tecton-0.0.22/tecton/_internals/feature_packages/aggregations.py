...
from tecton_spark.time_utils import convert_pendulum_duration_for_version

"""
This file contains internal methods of feature_packages/aggregations.py
...
def _get_feature_data_time_limits(fpov: FeaturePackageOrView, spine_time_limits):
    if spine_time_limits is None:
        ...
        elif fpov.is_temporal or fpov.is_temporal_aggregate:
            materialization_params = MaterializationParams.from_package_or_view(fpov)
...
def _align_feature_data_time_limits(
    ...
    version = materialization_params.feature_package_or_view.get_feature_store_format_version
    time_start = convert_epoch_to_datetime(materialization_params.align_timestamp_left(time_start), version)
...
def _get_raw_data_time_limits(
    fpov: FeaturePackageOrView, time_start: pendulum.DateTime, time_end: pendulum.DateTime
) -> pendulum.Period:
    if fpov.is_temporal_aggregate:
        # Account for final aggregation needing aggregation window prior to earliest timestamp
        max_aggregation_window = fpov.max_aggregation_window
        time_start = time_start - max_aggregation_window.ToTimedelta()
    elif fpov.is_temporal and not fpov.is_feature_view:
        # Need to subtract data_lookback to accommodate enough raw data for features.
        data_lookback = pendulum.Duration(seconds=fpov.feature_type.temporal.data_lookback.ToSeconds())
        time_start = time_start - data_lookback
...
def _get_time_limits(
    ...
    if spine_time_limits is None and spine_df is not None:
        spine_time_limits = data_frame_helper._get_time_limits_of_dataframe(spine_df, fpov.timestamp_key)
    time_start, time_end = _get_feature_data_time_limits(fpov, spine_time_limits)
    ...
    return _get_raw_data_time_limits(fpov, time_start, time_end)
    ...
    if not use_materialized_data:
        if feature_data_time_limits is not None:
            _raw_data_time_limits = _get_raw_data_time_limits(
                fpov, feature_data_time_limits.start, feature_data_time_limits.end
            )
        else:
            _raw_data_time_limits = raw_data_time_limits

        transformation_id2proto = fpov.transformation_id_proto_map if not fpov.is_feature_view else {}
        new_transformations = fpov.new_transformations if fpov.is_feature_view else []
        ...
        plan = materialization_plan.get_batch_materialization_plan(
            ...
            raw_data_time_limits=_raw_data_time_limits,
            coalesce=10,
            vds_proto_map=fpov.vds_proto_map,
            transformation_id2proto=transformation_id2proto,
            new_transformations=new_transformations,
        )
        return plan.offline_store_data_frame

    assert fpov.materialization_enabled and fpov.writes_to_offline_store

    offline_reader_timelimit = raw_data_time_limits
    if feature_data_time_limits is not None:
        offline_reader_timelimit = feature_data_time_limits

    offline_reader = offline_store.get_offline_store_reader(spark, fpov)
    return offline_reader.read(offline_reader_timelimit)


def construct_spine_with_all_join_keys_for_TAFP(
    spine_df,
    ...
    by the feature query for TemporalAggregateFeaturePackage.

    E.g. Let's say FP has fully bound join_key `A` and wildcard join_key `C`.
    ...
    :param partial_aggregations_df: Partially aggregated DataFrame for the FeaturePackage.
    :param feature_package: The feature package
    """
    join_key_conditions = [
        spine_df[join_key] == partial_aggregations_df[join_key] for join_key in fully_bound_join_keys
    ]

    max_aggregation_window = fpov.max_aggregation_window.ToSeconds()
    ...
    return spine_with_all_join_keys.select(
        [functions.col(f"p.{join_key}").alias(join_key) for join_key in fpov.join_keys]
        + [functions.col(f"s.{TEMPORAL_ANCHOR_COLUMN_NAME}").alias(TEMPORAL_ANCHOR_COLUMN_NAME)]
    ).distinct()


def construct_full_tafp_df(
    spark: SparkSession,
    fpov: FeaturePackageOrView,
    spine_df=None,
    ...
    :param fpov: The temporal aggregate feature package or view object
    ...
    if not fpov.is_temporal_aggregate:
        raise ValueError(f"Invalid invocation on non-temporal aggregate package")

    time_aggregation = fpov.trailing_time_window_aggregation
    spine_join_keys = fpov.join_keys
    join_keys = fpov.join_keys
    wildcard_join_keys = fpov.wildcard_join_key
    ...
    if not all_partial_aggregations_df:
        all_partial_aggregations_df = _get_all_partial_aggregation_tiles_df(
            spark, fpov, use_materialized_data, raw_data_time_limits
        )
    ...
    if not spine_df:
        ...
        partial_aggregations_df = all_partial_aggregations_df.withColumn(
            ...
            + convert_pendulum_duration_for_version(fpov.get_tile_interval, fpov.get_feature_store_format_version),
        )
    else:
        ...
        spine_df = spine_df.select(
            materialization_params.most_recent_anchor_time(functions.to_timestamp(fpov.timestamp_key)).alias(
                TEMPORAL_ANCHOR_COLUMN_NAME
            ),
            *spine_join_keys,
        ).distinct()
        # In order to get the training dataframe, we first right-join all_partial_aggregations_df to spine_df on join keys.
        ...
        partial_aggregations_df = all_partial_aggregations_df.join(
            spine_df.select(spine_join_keys).distinct(), spine_join_keys, how="right"
        )
        ...
        if wildcard_key_not_in_spine and wildcard_join_keys:
            ...
            spine_with_all_join_keys = construct_spine_with_all_join_keys_for_TAFP(
                spine_df,
                partial_aggregations_df,
                fpov,
                spine_join_keys,
    ...
    for feature in time_aggregation.features:
        ...
        upper_range = -(convert_pendulum_duration_for_version(window_duration, version)) + 1
        window_spec = (
        ...
        input_columns = [
            functions.col(name) for name in aggregation_plan.materialized_column_names(feature.input_feature_name)
        ]
        aggregations.append(
            aggregation_plan.full_aggregation_transform(input_columns, window_spec).alias(feature.output_feature_name)
        )

    output_df = partial_aggregations_df.select(join_keys + [TEMPORAL_ANCHOR_COLUMN_NAME] + aggregations)
    if spine_df:
        output_df = output_df.join(spine_df, on=spine_join_keys + [TEMPORAL_ANCHOR_COLUMN_NAME], how="inner")
    output_df = output_df.withColumn(
        TEMPORAL_ANCHOR_COLUMN_NAME,
        convert_epoch_to_timestamp_column(functions.col(TEMPORAL_ANCHOR_COLUMN_NAME), version),
    )
    return output_df.withColumnRenamed(TEMPORAL_ANCHOR_COLUMN_NAME, time_aggregation.time_key)


def construct_spine_with_all_join_keys_for_TFP(
    spine_df, tile_df, timestamp_column: str, serving_ttl: int, fully_bound_join_keys: List[str], join_keys: List[str]
):
    """
     Appends a spine with wildcard join_key values to result into set of join_keys that should be returned
     by the feature query for TemporalFeaturePackage.
    ...
    and:
     ...
     :param spine_df: The spine to join against.
     :param tile_df: Partially aggregated DataFrame  for the FeaturePackage.
     :param feature_package: The feature package
    """
...
def construct_full_tfp_or_push_df(
    spark: SparkSession,
    fpov: FeaturePackageOrView,
    spine_df=None,
    all_tiles_df=None,
    ...
    :param feature_package: The temporal feature package
    :param spine_df: (Optional) The spine to join against. If present, the returned data frame
    ...
    :param wildcard_key_not_in_spine: Whether or not the wildcard join_key is present in the spine.
        Defaults to False if spine is not specified or if FeaturePackage has no wildcard join_key.
    """
    if not fpov.is_temporal and not fpov.is_push:
    ...
    if not all_tiles_df:
        all_tiles_df = _get_all_partial_aggregation_tiles_df(spark, fpov, use_materialized_data, raw_data_time_limits)

    all_join_keys = fpov.join_keys + [timestamp_key]
    if not spine_df:
        # For preview we just need to return DataFrame as it is, except we need to drop the unnecessary anchor time
        return all_tiles_df.select(all_join_keys + fpov.features)

    wildcard_join_key = fpov.wildcard_join_key
    spine_join_keys = fpov.join_keys
    if wildcard_key_not_in_spine and wildcard_join_key:
        spine_join_keys.remove(wildcard_join_key)

    # In order to get the training dataframe, we first right-join all_tiles_df to spine_df on join keys.
    # This ensures that the resulting df only contains the join keys that we have in spine.
    tiles_df = all_tiles_df.join(spine_df.select(spine_join_keys).distinct(), spine_join_keys, how="right")
    if wildcard_key_not_in_spine and wildcard_join_key:
        ...
        serving_ttl = int(fpov.serving_ttl.total_seconds())
        timestamp_column = fpov.timestamp_key
        spine_with_all_join_keys = construct_spine_with_all_join_keys_for_TFP(
            spine_df, tiles_df, timestamp_column, serving_ttl, spine_join_keys, fpov.join_keys
        )
    else:
    ...
    # Next, we run the "last" window function with (-serving_ttl, 0) window while partitioning the data on fp.join_keys.
...
def get_all_temporal_push_features(
    spark: SparkSession,
    ...
    assert (
        feature_package_or_view.is_temporal or feature_package_or_view.is_push
    ), f"Invalid invocation on unsupported FeaturePackage type {feature_package_or_view.name}"

    if not use_materialized_data and feature_package_or_view.is_push:
        raise errors.PFP_UNABLE_TO_ACCESS_SOURCE_DATA(feature_package_or_view.name)

    df = _get_all_partial_aggregation_tiles_df(
        spark, feature_package_or_view, use_materialized_data, None, feature_data_time_limits=feature_data_time_limits
    )
    ...
    columns = (
        feature_package_or_view.join_keys + feature_package_or_view.features + [feature_package_or_view.timestamp_key]
    )
