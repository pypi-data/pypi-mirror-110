...
from tecton_proto.args import feature_package_pb2 as args_proto
...
DYNAMODB_LOGSTORE_CLASS = "org.apache.spark.sql.delta.storage.DynamoDBLogStore"
...
def get_offline_store_writer(
    params: OfflineStoreParams, package_config: args_proto.OfflineFeatureStoreConfig, version: int, spark: SparkSession
):
    """Creates a concrete implementation of OfflineStoreWriter based on package_config."""
    case = package_config.WhichOneof("store_type")
    if case == "delta":
        return DeltaWriter(params, package_config.delta, version)
    elif case == "parquet":
...
def get_offline_store_reader(spark: SparkSession, fpov: FeaturePackageOrView) -> "OfflineStoreReader":
    case = fpov.offline_store_config.WhichOneof("store_type")
    if case == "delta":
        return DeltaReader(spark, fpov)
    elif case == "parquet":
        return ParquetReader(spark, fpov)
    # Remove default after database migration is complete.
    # raise KeyError(case)
    return ParquetReader(spark, fpov)
...
class ParquetWriter(OfflineStoreWriter):
    ...
    def append_dataframe(self, data_frame):
        if self._params.is_continuous:
            align_duration = convert_pendulum_duration_for_version(pendulum.duration(seconds=86400), self._version)
...
class ParquetReader(OfflineStoreReader):
    def __init__(self, spark: SparkSession, fpov: FeaturePackageOrView):
        self._spark = spark
        assert fpov.materialization_enabled and fpov.writes_to_offline_store
        self._path = fpov.fp_materialization.materialized_data_location.path
        self._window = fpov.min_scheduling_interval
        self.version = fpov.get_feature_store_format_version
...
class DeltaReader(OfflineStoreReader):
    def __init__(self, spark: SparkSession, fpov: FeaturePackageOrView):
        ...
        assert fpov.materialization_enabled and fpov.writes_to_offline_store
        self._path = fpov.fp_materialization.materialized_data_location.path
        self._window = fpov.offline_store_config.delta.time_partition_size.ToTimedelta()
