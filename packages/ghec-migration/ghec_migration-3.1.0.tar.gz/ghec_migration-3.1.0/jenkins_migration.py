...
def replace_config(job_name, config, server, jenkins_answers):
    ...
    if "BitbucketSCMSource" in config:
        ...
        if "io.jenkins.blueocean.rest.impl.pipeline.credential.BlueOceanCredentialsProvider" in config:
            config = config.replace("<repository>",f"<repository>{team_prefix}.")
        if credentials_id.strip() != "":
            config = re.sub("<credentialsId>.*</credentialsId>", f"<credentialsId>{credentials_id}</credentialsId>",
                            config)
        config = config.replace("<name>master</name>", "<name>main</name>")
