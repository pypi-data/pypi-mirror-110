...
s = requests.Session()
retries = Retry(total=5, backoff_factor=2, status_forcelist=[404, 422, 403])
...
def rename_master_to_main(repo, org, answers, headers):
    try:
        ...
        master_response = s.get(url=f"https://api.github.com/repos/{org}/{repo}/branches/master", headers=headers)
        ...
        if master_response.status_code == 200:
            ...
            logging.info("No Branch named Master found, ignoring")
            return True
