...
def _eps_cmap(eps_r, alpha=1, cmap=None, clim=None):
    ...
    bounds = np.hstack(
        (pmc_eps - 1, pec_eps - 1, pec_eps + 1, np.linspace(cl[0], cl[1], 256))
    )
    ...
    bounds = np.insert(bounds, ind1 + 1, np.array([1 - fp_eps, 1 + fp_eps]))
    eps_cmap = np.insert(eps_cmap, [ind1 + 1], c_insert, 0)

    norm = mpl.colors.BoundaryNorm(boundaries=bounds, ncolors=bounds.shape[0] - 1)
