...
def sql(key):
    ...
    switcher = {
        ...
        'UserID_Notadmin': {'sql': 'select userId from User where loginName != "admin" AND email != "12345678@qq.com";', 'database': 'AnyRobot'},
        'JDBCCollectorId': {
            'sql': "select collectorId from JDBCCollectorConfig where JDBCCollectorConfig.type='jdbc' AND JDBCCollectorConfig.`sql`='select * from CollectorConfig';",
            'database': 'AnyRobot'},
        'vSphereID': {
