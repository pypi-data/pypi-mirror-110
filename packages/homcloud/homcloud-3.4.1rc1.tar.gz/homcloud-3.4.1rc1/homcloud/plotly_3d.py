...
def Simplices(simplices, color=None, width=1, name=""):
    ...
    return go.Scatter3d(
        x=xs, y=ys, z=zs, mode="lines", name=name, showlegend=False, line=dict(color=color, width=width),
    )
...
class VoxelDrawer(object):
    def __init__(self, bitmap, color, name):
        self.bitmap = bitmap
...
def Bitmap3d(bitmap, color=None, name=""):
    """
    ...
    """
    return VoxelDrawer(bitmap, color, name).graph_object()
...
def Cubes1d(cubes, color, width, name):
    ...
    zs = []
    for (cube, d) in cubes:
        xs.extend([cube[0], cube[0] + d[0], None])
        ys.extend([cube[1], cube[1] + d[1], None])
        zs.extend([cube[2], cube[2] + d[2], None])
    return go.Scatter3d(
        x=xs, y=ys, z=zs, mode="lines", name=name, showlegend=False, line=dict(color=color, width=width),
    )
...
def Cubes2d(cubes, color, width, name):
    ...
    return go.Scatter3d(
        x=xyzs[0], y=xyzs[1], z=xyzs[2], mode="lines", name=name, showlegend=False,
...
class CubeMeshDrawer(object):
    ...
    def graph_object(self):
        ...
        return go.Mesh3d(x=coordinates[:, 0], y=coordinates[:, 1], z=coordinates[:, 2],
                         i=triangles[:, 0], j=triangles[:, 1], k=triangles[:, 2],
                         name=self.name, color=self.color, showlegend=False)
