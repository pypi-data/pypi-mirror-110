...
class Volume(object):
    ...
    def to_plotly_graph_objects(self, color="green", **args):
        if self.diagram.filtration_type == "alpha":
            return p3d.Simplices(self.boundary(), color=color, **args)
        else:
            raise(RuntimeError(f"{self.filtration_type} cannot be renderred"))

    to_plotly = to_plotly_graph_objects

    def to_plotly_mesh(self, color="green"):
        if self.diagram.filtration_type == "alpha":
            if self.diagram.degree == 2:
                return p3d.SimplicesMesh(self.boundary(), color=color)
            elif self.diagram.degree == 1:
                return p3d.SimplicesMesh(self.simplices(), color=color)
            else:
