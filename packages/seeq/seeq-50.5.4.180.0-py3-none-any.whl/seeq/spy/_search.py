...
def _search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
    ...
    def _add_to_dict(_dict, _key, _val):
        if _key in ['Archived', 'Cache Enabled'] and isinstance(_val, str):
            _val = _val.lower() is 'true'
    ...
    for status_index in range(len(queries)):
        ...
        if len(current_query) == 0:
            raise ValueError('No recognized properties present in "query" argument. You must provide a dict or a '
