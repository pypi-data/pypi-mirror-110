...
def _create_folder_path_if_necessary(path):
    ...
    for i in range(0, len(workbook_path)):
        ...
        if content_name in [_folder.SHARED, _folder.ALL]:
            raise RuntimeError(f'"path" argument cannot contain {content_name} folder')

        if content_name == _folder.CORPORATE:
            if not _login.corporate_folder:
                raise RuntimeError(f'Attempting to push to Corporate folder but user does not have access')

            parent_id = _login.corporate_folder.id
            folder_id = _login.corporate_folder.id
            continue
