...
class Annotation:
    ...
    def push(self, pushed_workbook_id, pushed_worksheet_id, item_map, datasource_output, push_images, label,
        ...
        self._push_specific(item_map, datasource_output, label, new_annotation, relevant_annotation, item_push_errors)
    ...
    def _push_specific(self, item_map, datasource_output, label, new_annotation, annotation, item_push_errors=None):
    ...
    def _refresh_child_dict(existing_item_dict: dict, new_item_dict: dict, item_map: dict):
        ...
        for existing_item in existing_items:
            if existing_item.id not in item_map:
                # The Content or DateRange failed to get pushed. Keep going.
                del existing_item_dict[existing_item.id]
                continue
            new_item_id = item_map[existing_item.id]
            if new_item_id not in new_item_dict:
                continue
    ...
    def _push_specific(self, item_map, datasource_output, label, new_annotation, existing_annotation,
        ...
        for _, date_range in self.date_ranges.items():  # type: (_, DateRange)
            try:
                ...
                if item_push_errors is not None and e.status == 400:
                    item_push_errors.add(f'Error processing Date Range: {date_range}\n{e.body}')
                continue
        ...
        for _, content in self.content.items():  # type: (_, Content)
            try:
                ...
                if item_push_errors is not None and e.status == 400:
                    item_push_errors.add(f'Error processing Content: {content}\n{e.body}')
                continue
