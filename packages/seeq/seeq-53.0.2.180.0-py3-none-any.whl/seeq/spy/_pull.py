...
def _pull_condition(shape, capsule_properties, calculation_to_use, item_id, item_name, header,
    ...
    while True:
        ...
        if shape == 'capsules' and at_least_one_signal:
            ...
            this_capsule_count, next_start, result_df = _pull_condition_as_a_table(capsule_properties, current_start,
                                                                                   query_df, item_id, item_name,
                                                                                   header, offset, pd_end, tz,
...
def _build_stat_formula_fragment_and_update_parameters(signals_df, header, parameters):
    stat_id_to_stats = defaultdict(list)
    stat_headers = []
    for signal_index, signal_row in signals_df.iterrows():
        ...
        stat_headers.append(f'{signal_item_name} ({statistic})')
        ...
        stat_id_to_stats[signal_item_id].append(stat_formula)

    final_fragment = ''
    for (variable_count, [signal_id, stat_list]) in enumerate(stat_id_to_stats.items()):
        short_id = f's{variable_count}'
        parameters.append(f'{short_id}={signal_id}')
        joined_stats = ', '.join(stat_list)
...
def _pull_condition_as_a_table(capsule_properties, current_start, query_df, item_id, item_name,
                               header, offset, pd_end, tz, result_df, column_names):
    ...
    limit_fragment = _build_limit_fragment(offset, _config.options.pull_page_size)
    ...
    stat_fragment, parameters, stat_headers = _build_stat_formula_fragment_and_update_parameters(signals_df, header,
                                                                                                 parameters)
