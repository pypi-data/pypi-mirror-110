...
def push(jobs_df: pd.DataFrame, spread: Optional[str] = None, datalab_notebook_url: Optional[str] = None,
         ...
         suspend: bool = False, quiet: bool = False, status: Optional[_common.Status] = None):
    """
    ...
    try:
        _schedule.schedule_df(jobs_df=jobs_df, spread=spread, datalab_notebook_url=datalab_notebook_url, label=label,
                              user=user, suspend=suspend, status=status)
    except _schedule.SchedulePostingError:
...
def get_parameters(jobs_df, interactive_index, status):
    if _jobs_common.running_in_executor():
        ...
        if schedule_index is None:
            raise RuntimeError('Job is being executed without a SEEQ_SDL_SCHEDULE_INDEX environment variable')
        if not (schedule_index in jobs_df.index or int_index in jobs_df.index):
            raise IndexError(f'Cannot execute job with index {schedule_index} because it is not in the '
        ...
        if interactive_index is None:
            ...
            raise IndexError(f'Cannot schedule job with interactive_index {interactive_index} because it is not in '
