...
class Tournaments_imagesQuerySet(Model_imagesQuerySet):
    def delete(self):
        return super().delete()
...
class Tournaments_imagesManager(AuditManager):
    ...
    def get_queryset(self):
        return Tournaments_imagesQuerySet(self.model, using=self._db)


class Tournaments_images(Model_images):
    image = ForeignKeyProtect(Images)
    main_model = ForeignKeyProtect(Tournaments)

    objects = Tournaments_imagesManager()
