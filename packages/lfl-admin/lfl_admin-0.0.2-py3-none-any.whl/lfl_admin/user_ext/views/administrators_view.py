...
def Administrators_view_Add(request):
    return JsonResponse(DSResponseAdd(data=Administrators_view.objects.createFromRequest(request=request), status=RPCResponseConstant.statusSuccess).response)


@JsonResponseWithException()
def Administrators_view_Update(request):
    return JsonResponse(DSResponseUpdate(data=Administrators_view.objects.updateFromRequest(request), status=RPCResponseConstant.statusSuccess).response)


@JsonResponseWithException()
def Administrators_view_Remove(request):
    return JsonResponse(DSResponse(request=request, data=Administrators_view.objects.deleteFromRequest(request=request), status=RPCResponseConstant.statusSuccess).response)
