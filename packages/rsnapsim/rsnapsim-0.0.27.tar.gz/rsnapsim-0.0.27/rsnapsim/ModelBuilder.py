    ...
    def run_ssa(self,t, n_traj=10,low_mem = True ):
        ...
        if low_mem == False:
            ...
            for i in range(n_trajectories):
                ...
                ssa_translation_lowmem.run_SSA_generic(result,kelong ,frapresult,t_array, np.array([0,0,0],dtype=np.float64), seeds[i], k_add.flatten()  ,n_enters,n_pauses,n_stops,n_jumps, probe, ncolors ,max_ribs )
                all_results[i,:,:] = result
                #all_frapresults[i,:] = frapresult
