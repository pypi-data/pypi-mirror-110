        ...
        for job in joblist:
            if job.date != None and job.member != None and job.has_parents():
                date_member_cluster[job.name] = str(
                    str(job.long_name[0:13]) + str(job.member))
        ...
        for node in exp.get_nodes():
            ...
            if name in jobs_packages_dict:
                ...
                if package not in packages_subgraphs_dict:
                    packages_subgraphs_dict[package] = pydotplus.Cluster(
                        graph_name=package)
                ...
                nodes_in_wrapper.append(name)
        print(len(date_member_cluster.keys()))
        # for node in exp.get_nodes():
        #     name = node.obj_dict['name']
        #     if name in date_member_cluster.keys():
        #         name_fake_group = date_member_cluster[name]
        #         if name_fake_group not in packages_subgraphs_dict:
        #             packages_subgraphs_dict[name_fake_group] = pydotplus.graphviz.Cluster(graph_name=name_fake_group)
        #             packages_subgraphs_dict[name_fake_group].obj_dict['attributes']['color'] = 'black'
        #             packages_subgraphs_dict[name_fake_group].obj_dict['attributes']['style'] = 'dashed'
        #         packages_subgraphs_dict[name_fake_group].add_node(node)
        #         if name in nodes_in_wrapper:
        #             not_to_fake.append(name_fake_group)
        # print("Done")

        for package, cluster in packages_subgraphs_dict.items():
            if package not in not_to_fake:
                # print(package)
                # print(cluster)
                graph.add_subgraph(cluster)

        # for name in date_member_cluster.keys():
        #     for h in date_member_cluster[name]:
        #         print(str(name) + " : " + str(h))

        # for u in exp.get_node_list():
        #     print(u.obj_dict['name'])

        # for v in exp.get_edge_list():
        #     print(str(v.get_source()) + " to " + str(v.get_destination()))
        #print("Definition finalized")
