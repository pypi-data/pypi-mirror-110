...
class JobList:
    ...
    def get_graph_representation(self, BasicConfig, layout='standard', grouped='none', chunk_unit=None, chunk_size=1):
        ...
        if not graph_drawing_data or len(allJobs) > 2200:
            ...
            print('Traverse Update Ended. Seconds spent: ' +
                  str(time() - start_time))
            # We proceed to start the thread
            # thread_write_graph_drawing = Thread(target=ExperimentGraphDrawing(
            #     self.expid).calculate_drawing, args=(allJobs,))
            # print("Launching Thread.")
            # thread_write_graph_drawing.start()
        else:
        ...
        # Main condition
        if (graph_drawing_data or len(allJobs) <= 2200) and (layout == 'standard'):
            # if (len(allJobs) <= 1000):
            # start_time = time()
            if graph_drawing_data:
                ...
                mainCoordinates = ExperimentGraphDrawing(
                    self.expid).calculate_drawing(allJobs, independent=True)
