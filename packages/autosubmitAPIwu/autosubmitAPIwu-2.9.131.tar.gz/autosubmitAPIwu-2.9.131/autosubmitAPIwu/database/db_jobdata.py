...
JobRow = collections.namedtuple(
    'JobRow', ['name', 'queue_time', 'run_time', 'status', 'energy', 'submit', 'start', 'finish', 'ncpus'])
...
class ExperimentRun():
    ...
    def getSYPD(self, job_list):
        ...
        if job_list:
            ...
            number_SIM = len(job_list)
            # print(len(job_list))
            total_run_time = sum(job.running_time() for job in job_list)
            # print("run {3} yps {0} n {1} run_time {2}".format(
            #     years_per_sim, number_SIM, total_run_time, self.run_id))
            return round((years_per_sim * number_SIM * seconds_per_day) / total_run_time, 2) if total_run_time > 0 else None
        return None
    ...
    def getASYPD(self, job_sim_list, job_post_list, package_jobs):
        ...
        if job_sim_list and len(job_sim_list) > 0 and job_post_list and len(job_post_list) > 0:
            ...
            # print("Sum SIM {0}".format(sum_SIM))
...
class JobData(object):
    ...
    def queuing_time(self, job_data_in_package=None):
        ...
        Returns:
            int: queueing time
        """
        # if self.job_name == 'a3bh_18500101_fc1_138_SIM':
        #     print("Package {} {} {}".format(
        #         self.job_name, self.rowtype, self._start))
        # We consider the highest queue time in the package as the queue time of the whole package.
        max_queue = queue = 0
        job_name_max_queue = None
        # print(job_data_in_package)
        # print("Run {} Job {}".format(self.run_id, self.job_name))
        # print("Raw queue {}".format(self._start - self._submit))
        if job_data_in_package and len(job_data_in_package) > 0:
            ...
            # if self.job_name == 'a3bh_18500101_fc1_138_SIM':
            #     for package in job_data_in_package:
            #         print(job_data_in_package[package])
            # print(jobs_times)
            if jobs_times and len(jobs_times) > 0:
                ...
                # print(jobs_times)
                # if self.job_name == 'a3bh_18500101_fc1_138_SIM':
                #     print("Current {}".format(self.start - self.submit))
                #     print("Max Queue {}".format(max_queue))
                #     print("Job Name Max {}".format(job_name_max_queue))

        if self.status in ["SUBMITTED", "QUEUING", "RUNNING", "COMPLETED", "HELD", "PREPARED", "FAILED"]:
            # parents_sum = 0
            # print("{} {}".format(self.job_name, self.rowtype))
            # if self.rowtype > 2:
            #     # if job_name_max_queue and self.job_name == job_name_max_queue:
            #     #     # path = []
...
class JobDataStructure(MainDataBase):
        ...
        try:
            ...
            if jobdata_historic:
                ...
                for jobdata in jobdata_historic:
                    ...
                    result.append({"counter": jobdata.counter, "created": jobdata.created, "submit": jobdata.submit_datetime_str(), "start": jobdata.start_datetime_str(), "finish": jobdata.finish_datetime_str(
                    ), "queue_time": jobdata.delta_queue_time(job_data_in_package=package_info), "run_time": jobdata.delta_running_time(), "ncpus": jobdata.ncpus, "wallclock": jobdata.wallclock, "qos": jobdata.qos, "platform": jobdata.platform, "job_id": jobdata.job_id, "nodes": jobdata.nnodes, "energy": jobdata.energy_string() if jobdata.run_id is not None and jobdata.energy > 0 else "NA", "status": jobdata.status, "SYPD": jobdata.metric_SYPD, "ASYPD": jobdata.metric_ASYPD, "run_id": jobdata.run_id})
        except Exception as exp:
            print(traceback.format_exc())
            print(str(exp))
            return None
        ...
        try:
            ...
            if os.path.exists(self.folder_path):
                ...
                if current_job_data:
                    ...
                    return {run_id: [x for x in current_collection if x.run_id == run_id and (np.abs((x.running_time() - mean_sd.get(x.run_id, (0, 0))[0]) / mean_sd.get(x.run_id, (1, 1))[1]) if mean_sd.get(x.run_id, (1, 1))[1] > 0 else 0) <= threshold] for run_id in set([job.run_id for job in current_collection])}
    ...
    def _get_current_job_data(self, run_id, all_states=False):
        """Returns detail job_data by run_id
        ...
        try:
            if self.conn:
                ...
                if all_states == False:
                    ...
                    # cur.execute("SELECT id, counter, job_name, created, modified, submit, start, finish, status, rowtype, ncpus, wallclock, qos, energy, date, section, member, chunk, last, platform, job_id, extra_data, nnodes, run_id from job_data WHERE run_id=? and rowtype >= 2 ORDER BY id", (run_id,))
    ...
    def _get_current_job_data_CF_SIM(self):
        ...
        try:
            if self.conn:
                ...
                self.conn.text_factory = str
                cur = self.conn.cursor()
                cur.execute("SELECT id, counter, job_name, created, modified, submit, start, finish, status, rowtype, ncpus, wallclock, qos, energy, date, section, member, chunk, last, platform, job_id, extra_data, nnodes, run_id from job_data WHERE member LIKE '%SIM%' and status in ('COMPLETED') ORDER BY id DESC")  # Only consider COMPLETED
                rows = cur.fetchall()
                # print(rows)
                if len(rows) > 0:
                    return rows
    ...
    def _get_current_job_data_CF_POST(self):
        ...
        try:
            if self.conn:
                self.conn.text_factory = str
                cur = self.conn.cursor()
                cur.execute("SELECT id, counter, job_name, created, modified, submit, start, finish, status, rowtype, ncpus, wallclock, qos, energy, date, section, member, chunk, last, platform, job_id, extra_data, nnodes, run_id from job_data WHERE member = 'POST' and status in ('COMPLETED') ORDER BY id DESC")  # Only consider COMPLETED
                rows = cur.fetchall()
                if len(rows) > 0:
                    return rows
                else:
