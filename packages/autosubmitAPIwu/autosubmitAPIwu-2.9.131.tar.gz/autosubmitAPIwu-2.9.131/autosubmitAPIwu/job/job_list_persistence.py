        ...
        if os.path.exists(path):
            ...
            if file_size <= 6:
                raise AssertionError(
                    "Your pkl file seems to be empty. Make sure that you are running Autosubmit. See the FAQ tab for more information.")
            fd = open(path, 'r')
            return pickle.load(fd)
        else:
            raise AssertionError(
                "The pkl file of your experiment does not exist. Make sure that you are running Autosubmit. See the FAQ tab for more information.")
