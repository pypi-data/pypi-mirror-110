...
class JobList:
    ...
    def get_graph_representation(self, BasicConfig, layout='standard', grouped='none', chunk_unit=None, chunk_size=1):
        ...
        print('Traverse Update Ended. Seconds spent: ' + str(time() - start_time))
        ...
        for job in allJobs:
            ...
            if job._children > 0:
                current_group = job_to_package[job.name] if job.name in job_to_package else None
        ...
        # Main condition
        if ((len(allJobs) <= 1000 or (packages)) and (not (len(self._date_list) == 1 and len(self._chunk_list) > 80)) and (layout == 'standard')):
            # if (len(allJobs) <= 1000):
            print("GraphViz Started")
            start_time = time()
            i_am_true_coordinates = True
            graph = monitor.create_tree_list(
                self.expid, allJobs, packages, dict(), False)
        ...
        if grouped == "date-member" and (len(dates) > 1 or len(self._member_list) > 1):
            ...
            for group in colors:
                ...
                y_coordinate = sum(y_bound)/len(y_bound) if len(y_bound) > 0 else 0
                ...
                for date in dates:
                    for member in self._member_list:
                        for chunk in self._chunk_list:
                            group_name = self._expid + "_" + \
                            str(dates[date]) + "_" + member + "_" + str(chunk) + "_"
                            # print("Group Name: {}".format(group_name))
                            specific_list = filter(
                            lambda x: x.name.startswith(group_name), allJobs)
                            if len(specific_list) > 0:
                                failed_count = sum(1 for x in specific_list if x.status == Status.FAILED)                                                                                    
                                group_color = Monitor.color_status(Status.WAITING)
                                if failed_count > 0:
                                    group_color = Monitor.color_status(Status.FAILED)
                                else:
                                    running_count = sum(1 for x in specific_list if x.status == Status.RUNNING)    
                                    if running_count > 0:
                                        group_color = Monitor.color_status(Status.RUNNING)
                                    else:
                                        queuing_count = sum(1 for x in specific_list if x.status == Status.QUEUING)
                                        if queuing_count > 0:
                                            group_color = Monitor.color_status(Status.QUEUING)
                                        else:
                                            completed_count = sum(1 for x in specific_list if x.status == Status.COMPLETED)
                                            if completed_count > 0:
                                                group_color = Monitor.color_status(Status.COMPLETED)                                        
    ...
    def get_job_times_collection(BasicConfig, allJobs, expid, job_to_package=None, package_to_jobs=None, timeseconds=True):
        ...
        job_data, warning_messages = JobDataStructure(
            expid).process_current_run_collection(allJobs, job_times)
        # Result variables
        job_running_to_min = dict()
        job_running_to_runtext = dict()
        result = dict()
        ...
        for sub in Manager.get_subjoblist():
            ...
            # ) else None
            if current_job_info:
                job_running_to_min[sub.name] = JobRow(sub.name, sub.queue, sub.run, sub.status, current_job_info.energy,
                                                      current_job_info.submit, current_job_info.start, current_job_info.finish, current_job_info.ncpus)
                job_running_to_runtext[sub.name] = job_times_to_text(
                    sub.queue, sub.run, sub.status)

        return (job_running_to_min, job_running_to_runtext, warning_messages)
    ...
    def retrieve_times(status_code, name, tmp_path, make_exception=False, job_times=None, seconds=False, job_data_collection=None):
            ...
            if job_data_collection is not None:
                ...
                if job_data:
                    ...
                    if status == job_data.status:
                        ...
                        if job_times:
                            _, _, c_finish, _, _ = job_times.get(
                                name, (0, 0, t_finish, 0, 0))
                        ...
                        return JobRow(job_data.job_name, int(queue_time), int(running_time), status, energy, JobList.ts_to_datetime(t_submit), JobList.ts_to_datetime(t_start), JobList.ts_to_datetime(t_finish), job_data.ncpus)
        ...
        return JobRow(name, int(queue_time), int(running_time), status, energy, JobList.ts_to_datetime(submit_time), JobList.ts_to_datetime(start_time), JobList.ts_to_datetime(finish_time), 0)
    ...
    def retrieve_packages(BasicConfig, expid, current_jobs=None):
        ...
        :return: job to package, package to jobs, package to package_id
        :rtype: Dictionary(Job Object, Package), Dictionary(Package, List of Job Objects), Dictionary(String, String)
        """
        monitor = Monitor()
        packages = None
        try:
