...
from .custom_widgets import ImageAdjustments, ParametersSetter, SunkenHLine, SmallLabel, XYPlot, XYPlotRange
...
class MainGUI(QMainWindow):
    ...
    def __init__(self, experiment_id, filename, comment, database, data, dx, dt, transposed):
        ...
        # init data
        self.experiment = ExperimentHandler(experiment_id, filename, comment, database, self.image, transposed)

        QCoreApplication.processEvents()
        self.image.normalize(*self.experiment.analysis_range, *self.experiment.analysis_range_space)
        ...
        self.average_intensity_plot = AverageIntensityPlot(self.experiment.xdata,
                                                           self.experiment.tdata,
        ...
        self.background_correction_plot = XYPlotRange(title='Normalized background SD',
                                                      label={'position':'bottom', 'name': 'space', 'units': 'm'},
                                                      analysis_range=self.experiment.analysis_range_space,
                                                      bounds=[0, self.experiment.length])
        ...
        self.overview_plot.update_analysis_region_space(self.experiment.analysis_range_space)
        ...
        self.average_intensity_plot.sigAnalysisRegionChangeFinished.connect(self.update_experiment_histogram)
        self.average_intensity_plot.sigStageRangeChanged.connect(self.experiment.update_stage_range)

        self.background_correction_plot.sigAnalysisRegionChange.connect(self.overview_plot.update_analysis_region_space)
        self.background_correction_plot.sigAnalysisRegionChangeFinished.connect(self.experiment.set_analysis_range_space)
        self.background_correction_plot.sigAnalysisRegionChangeFinished.connect(self.update_experiment_histogram)
        ...
        self.experiment.sigIntesityChanged.connect(self.average_intensity_plot.update_intensity)
        ...
        if database.read_only:
            ...
            self.background_correction_plot(True)
        ...
        self.update_experiment_histogram()
    ...
    def update_parameters(self, mode, spark_params, image_params, auto_detect_params):
        if mode == 'image' or mode == 'auto_detect' or  mode == 'all':
            ...
            self.image.normalize(*self.experiment.analysis_range, *self.experiment.analysis_range_space, **image_params)
    ...
    def update_background_correction_plot(self, b):
        ...
        if corr_mean is not None and corr_meansd is not None:
            x = self.dx*np.arange(corr_mean.size) + self.experiment.analysis_range_space[0]
            d = {
                    'All': {'x': np.array([0, self.experiment.length]), 'y': np.ones((2,))},
    ...
    def update_experiment_histogram(self):
        self.histogram_analysis_range.set_data(self.image.get_slice('Raw', self.experiment.analysis_range_pixels))
