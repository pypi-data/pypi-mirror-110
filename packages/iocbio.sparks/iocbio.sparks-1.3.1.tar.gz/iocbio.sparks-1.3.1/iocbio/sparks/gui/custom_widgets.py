...
class XYPlot(QWidget):
    ...
    def set_data(self, data):
        ...
        if hasattr(self, 'analysis_region'):
            self.pw.addItem(self.analysis_region)
...
class XYPlotRange(XYPlot):
    sigAnalysisRegionChange = pyqtSignal(tuple)
    sigAnalysisRegionChangeFinished = pyqtSignal(tuple)

    def __init__(self, data=None, title=None, label=None, analysis_range=None, bounds=None):
        XYPlot.__init__(self, data, title, label)
        self.analysis_range = analysis_range
        self.bounds = bounds
        self.read_only = False
        self._init_analysis_region()

    def _init_analysis_region(self):
        if hasattr(self, 'analysis_region'):
            return

        self.analysis_region = pg.LinearRegionItem(self.analysis_range)
        self.analysis_region.setBounds(self.bounds)
        ...
        self.pw.addItem(self.analysis_region)
        db = self.bounds[1] - self.bounds[0]
        self.pw.setXRange(self.bounds[0]-0.05*db, self.bounds[1]+0.05*db)
