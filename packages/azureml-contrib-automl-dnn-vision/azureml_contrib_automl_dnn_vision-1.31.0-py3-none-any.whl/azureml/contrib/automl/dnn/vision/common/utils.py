...
from azureml.contrib.automl.dnn.vision.common.constants import (
    ArtifactLiterals, MetricsLiterals, RunPropertyLiterals, SettingsLiterals,
    SystemMetricsLiterals, SystemSettings, TrainingCommonSettings, Warnings, supported_model_layer_info
)
...
from azureml.contrib.automl.dnn.vision.object_detection.common.constants import ModelNames as od_ModelNames
...
def _merge_dicts_with_not_none_values(args_dict: dict, defaults: dict) -> dict:
...
def log_all_metrics(computed_metrics, azureml_run, add_to_logger=False):
...
def log_verbose_metrics_to_rh(train_time, epoch_time, train_sys_meter, valid_sys_meter, azureml_run):
    ...
    metrics[SystemMetricsLiterals.TRAIN_EPOCH_DURATION_SECONDS_MIN] = epoch_time.min
    ...
    if train_sys_meter.gpu_mem_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.TRAIN_GPU_MEM_USED_MB_MIN] = \
    ...
    if train_sys_meter.gpu_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.TRAIN_GPU_USED_PCT_MIN] = \
    ...
    if train_sys_meter.sys_mem_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.TRAIN_SYS_MEM_PCT_MIN] = \
        ...
        metrics[SystemMetricsLiterals.TRAIN_SYS_MEM_SHARED_MB_MIN] = \
        ...
        metrics[SystemMetricsLiterals.TRAIN_SYS_MEM_USED_MB_MIN] = \
    ...
    if valid_sys_meter.gpu_mem_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.VALID_GPU_MEM_USED_MB_MIN] = \
    ...
    if valid_sys_meter.gpu_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.VALID_GPU_USED_PCT_MIN] = \
    ...
    if valid_sys_meter.sys_mem_usage_accumulator:
        ...
        metrics[SystemMetricsLiterals.VALID_SYS_MEM_PCT_MIN] = \
        ...
        metrics[SystemMetricsLiterals.VALID_SYS_MEM_SHARED_MB_MIN] = \
        ...
        metrics[SystemMetricsLiterals.VALID_SYS_MEM_USED_MB_MIN] = \
...
def log_script_duration(script_start_time, settings, azureml_run):
    ...
    if settings.get(SettingsLiterals.LOG_VERBOSE_METRICS, False):
        azureml_run.log(SystemMetricsLiterals.SCRIPT_DURATION_SECONDS, script_duration_seconds)
...
def check_loss_explosion(loss_value):
...
def get_model_layer_info(model_name):
...
def freeze_model_layers(model_wrapper, layers_to_freeze):
    """Freeze some of model layers.

    :param: model_wrapper: Model wrapper containing model for any task
    :type: model_wrapper:  classification.models.classification_model_wrappers.ModelWrapper or
    object_detection.models.object_detection_model_wrappers.ModelWrapper or
    object_detection.models.instance_segmentation_model_wrappers.ModelWrapper or
    ...
    for sublist in nested_layers_to_freeze:
        if isinstance(sublist, tuple):
            ...
            layers_to_freeze.append(sublist)
