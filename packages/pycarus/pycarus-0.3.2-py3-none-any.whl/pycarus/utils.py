...
def get_tsne(
    ...
    data = np.nan_to_num(data)  # type: ignore
    tsne = TSNE(
    ...
    min_val = np.min(results)  # type: ignore
    max_val = np.max(results)  # type: ignore
...
def get_conf_matrix_img(conf_matrix: np.ndarray, size: int = 600) -> Image.Image:
    ...
    """
    c_mat = np.nan_to_num(conf_matrix).astype(np.float32)  # type: ignore
    h, w = c_mat.shape[0], c_mat.shape[1]

    for i in range(h):
        vmin = np.min(c_mat[i, :])  # type: ignore
        vmax = np.max(c_mat[i, :])  # type: ignore
        if vmax > 0:
