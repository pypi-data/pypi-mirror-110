...
def get_animation_wrapper(args):
    productId, location = args
    return get_animation(productId, location)
        ...
        for i, (png, control) in enumerate(im.frames):
            try:
                ...
                if not path.exists():
                    ...
                    img = imread(path)
                    if img.ndim == 2:
                        newimg = np.ones(img.shape[:2]+(3,))
                        newimg[:,:,:] = np.expand_dims(img,2)
                        img = newimg
                    if img.shape[2]==3:
                    ...
                    imsave(path, nim, check_contrast=False)
...
def get_animation_data(num_set, num_workers=0, location="./_tmp"):
    """ total 7107 """
    df = pd.read_csv( dir_path / f'linesticker_animation.csv')
    print("total # set", len(df))
    if num_workers > 0:
        with Pool(num_workers) as p:
            pids = [(row['productId'], location) for i,row in df.iterrows() if i < num_set]
            dataset = [item for datum in tqdm(p.imap_unordered(get_animation_wrapper, pids)) if len(datum)>0 for item in datum]
        return dataset
    else:
        dataset = []
        for i, row in tqdm(df.iterrows()):
            if i >= num_set:
                break
            data = get_animation(row['productId'], location)
            dataset.extend(data)
        return dataset