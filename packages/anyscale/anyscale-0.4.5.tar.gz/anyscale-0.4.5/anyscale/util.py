    ...
    while True:
        ...
        if len(sessions) > 0:
            ...
            if (
                ...
                raise click.ClickException(
                    f"Error while starting session {session_name}: Session startup failed due to an error ({session.state})."
                )
            elif (
                session.state
...
def init_sentry(ctx: click.core.Context) -> None:
    ...
    if anyscale.conf.ANYSCALE_ENV not in ["production", "staging"]:
        return
    ...
    sentry_sdk.init(
        ...
        environment=anyscale.conf.ANYSCALE_ENV,
    )
...
def get_anyscale_version() -> str:
    ...
    if frozen_requirement.editable:
        try:
            ...
            print("Not a git repository, will use standard release versioning")
