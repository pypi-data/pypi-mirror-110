...
def proces(dp, hm_flag, log_flag):
    ...
    df_dict, n_measurements = extr(DATA_PATH)
    ...
    for comparison in comparisons:
        ...
        graph_avg(df_dict, n_measurements, con_data, exp_data, con_repl, exp_repl, DATA_PATH, hm_flag, log_flag, avg=True)
        # Grapah individual wells of replicate
        if exp_repl in replicates:
            graph_indiv(df_dict, n_measurements, exp_data, exp_repl, DATA_PATH, log_flag)
            replicates.remove(exp_repl)
        if con_repl in replicates:
            graph_indiv(df_dict, n_measurements, con_data, con_repl, DATA_PATH, log_flag)
    ...
    if replicates:
        ...
        for repl in replicates:
            ...
            graph_indiv(df_dict, n_measurements, repl_data, repl, DATA_PATH, log_flag)
