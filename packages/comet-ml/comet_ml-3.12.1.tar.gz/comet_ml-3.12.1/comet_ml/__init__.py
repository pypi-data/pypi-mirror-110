...
class Experiment(BaseExperiment):
    ...
    def _on_end(self, wait=True):
        ...
        if successful_clean:
            for logged_artifact in self._uploaded_artifacts:
                LOGGER.info(ARTIFACT_UPLOAD_FINISHED, *logged_artifact)

            # Clean the list to avoid displaying it twice
            self._uploaded_artifacts = []
    ...
    def log_artifact(self, artifact):
        ...
        artifact_id, artifact_version_id = _upsert_artifact(
            artifact, self.rest_api_client, self.id
        )

        for artifact_file in artifact._assets:
        ...
        for remote_artifact_file in artifact._remote_assets:
            self._log_remote_asset(
                artifact_version_id=artifact_version_id,
                critical=True,
                **remote_artifact_file
            )

        self._summary.increment_section("uploads", "artifacts")
        ...
        self._uploaded_artifacts.append(
            (
                logged_artifact.workspace,
                logged_artifact.name,
    ...
    def get_artifact(
        self, artifact_name, workspace=None, version_or_alias=None,
    ):
        ...
        logged_artifact = _get_artifact(
            self.rest_api_client, params, self.id, self._summary
        )
