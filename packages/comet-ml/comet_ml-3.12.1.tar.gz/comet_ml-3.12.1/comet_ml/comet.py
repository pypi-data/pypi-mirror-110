...
from .connection import FileUploadManager, format_messages_for_ws
...
class Streamer(BaseStreamer):
    ...
    def __init__(
        ...
        self.file_upload_manager = FileUploadManager(worker_cpu_ratio)
    ...
    def wait_for_finish(self):
        ...
        if not self.file_upload_manager.all_done():
            ...
            wait_for_empty(
                self.file_upload_manager.all_done,
                self.file_upload_waiting_timeout,
                progress_callback=self.file_upload_manager.log_remaining_uploads,
                sleep_time=5,
            )
