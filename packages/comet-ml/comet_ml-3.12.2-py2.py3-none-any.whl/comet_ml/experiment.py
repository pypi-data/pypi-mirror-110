...
class BaseExperiment(CommonExperiment):
    ...
    def __init__(
        ...
        self._uploaded_artifacts = []  # type: List[Tuple[str, str, str]]
    ...
    def _log_asset(
        ...
        # type: (Any, Optional[str], bool, bool, str, Optional[int], bool, Optional[str], Any, Optional[str], bool) -> Optional[Dict[str, str]]
        if not self.alive:
            return None
        ...
        if artifact_version_id:
            url_params.update({"artifactVersionId": artifact_version_id})
        ...
        if artifact_version_id is None:
            ...
            self._summary.increment_section(
                "uploads", "artifact assets", size=processor.file_size
            )
        return self._get_uploaded_asset_url(asset_id)
    ...
    def _log_remote_asset(
        ...
        if remote_file_name is None:
            ...
            remote_file_name = parse_remote_uri(uri)
        ...
        processor = RemoteAssetUploadProcessor(
            uri, asset_type, url_params, metadata=metadata, critical=critical,
        )
        upload_message = processor.process()
        ...
        if artifact_version_id is None:
            ...
            self._summary.increment_section("uploads", "artifact assets")
        return self._get_uploaded_asset_url(asset_id)
