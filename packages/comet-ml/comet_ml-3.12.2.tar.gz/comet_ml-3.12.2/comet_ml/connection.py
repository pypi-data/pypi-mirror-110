...
def get_http_session(retry_strategy=None):
    # type: (Optional[Retry]) -> Session
    session = Session()
...
def get_thread_session(retry):
    ...
    # resources
    if retry:
        retry_session = THREAD_SESSIONS.__dict__.get(
            "retry_session", None
        ...
        new_retry_session = get_http_session(retry_strategy=get_retry_strategy())
        THREAD_SESSIONS.retry_session = new_retry_session

        return new_retry_session
    else:
        session = THREAD_SESSIONS.__dict__.get(
            "session", None
        )  # type: Optional[Session]
        ...
        THREAD_SESSIONS.session = new_session
...
class LowLevelHTTPClient(object):
    ...
    def __init__(
        ...
        self.retry_session = get_http_session(retry_strategy=get_retry_strategy())
    ...
    def report(
        ...
        try:
            if event_name is not None:
                ...
                session = get_http_session(retry_strategy=get_retry_strategy())
...
def get_http_proxy(target_url):
    ...
    proxies = getproxies()
    # TODO: Support also HTTP proxies and WS+WSS proxies
    http_proxy = proxies.get("https")
    bypass = proxy_bypass(parsed_url.hostname)

    if http_proxy is not None and not bypass:
...
class RestApiClient(BaseApiClient):
    ...
    def _prepare_experiment_asset_request(
        self, asset_id, experiment_key=None, artifact_version_id=None,
    ):
    ...
    def get_artifact_list(
        self, workspace, artifact_type=None,
    ):
...
def send_file(
    post_endpoint, file_path, params, headers, timeout, session, metadata=None,
):
    # type: (str, str, Dict[str, str], Dict[str, str], int, Session, Optional[Dict[Any, Any]]) -> Response
...
def send_file_like(
    post_endpoint, file_like, params, headers, timeout, session, metadata=None,
):
    # type: (str, Any, Dict[str, str], Dict[str, str], int, Session, Optional[Dict[Any, Any]]) -> Response
...
def send_remote_asset(
    post_endpoint, remote_uri, params, headers, timeout, session, metadata=None,
):
    # type: (str, str, Dict[str, str], Dict[str, str], int, Session, Optional[Dict[Any, Any]]) -> Response
...
def upload_file(
    ...
    try:
        response = send_file(
            ...
            metadata=metadata,
            timeout=timeout,
            session=get_thread_session(False),
        )
...
class FileUploadManager(object):
    def __init__(self, worker_cpu_ratio):
        # type: (int) -> None
        self.upload_results = []  # type: List[multiprocessing.pool.AsyncResult]

        pool_size, cpu_count, self._pool = get_thread_pool(worker_cpu_ratio)

        LOGGER.debug(
            "FileUploadManager instantiated with %d threads, %d CPUs, %d worker_cpu_ratio",
            pool_size,
            cpu_count,
            worker_cpu_ratio,
        )

    def upload_file_thread(self, critical=False, **kwargs):
    ...
    def upload_file_like_thread(self, critical=False, **kwargs):
        async_result = self._pool.apply_async(upload_file_like, args=[], kwds=kwargs)
        async_result._critical = critical
        self.upload_results.append(async_result)

    def upload_remote_asset_thread(self, critical=False, **kwargs):
    ...
    def log_remaining_uploads(self):
        # type: () -> None
        LOGGER.info(
            "Still uploading %d file(s)", self.remaining_uploads(),
        )
...
def write_stream_response_to_file(responses, file_object):
    # type: (requests.Response, IO[bytes]) -> None

    # TODO: Tune the chunk_size
