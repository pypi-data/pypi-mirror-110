...
class DataKeeper(object):
    def __init__(self, plain_text=None, cipher_text=None, mask_type=None, field=None, edk_key='default', raw_type=None) -> None:
        # 是否存在脱敏
        if plain_text is not None and isinstance(plain_text, str) and plain_text.count("*") > 0:
    ...
    def plain(self, message=None):
        ...
        if self._raw_type == 'int' or self._raw_type == int:
            return int(plain)
        return plain
