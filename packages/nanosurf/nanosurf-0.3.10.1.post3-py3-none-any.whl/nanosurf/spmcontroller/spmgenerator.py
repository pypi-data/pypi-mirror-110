...
class SPMSineWaveGenerator(nanosurf.spmcontroller._SPMModule):
...
class SPMLockIn(SPMSineWaveGenerator):
    ...
    def source(self, val):
        self.__source = val
        if isinstance(self.__source,nanosurf.spmcontroller.spmanalogin.SPMAnalogFastIn):
            self.__source.target = self
            self._lu.input.value = self._fast_input_channel_index[self.__source.inst_name]
        elif isinstance(self.__source,nanosurf.spmcontroller.spmanalogin.SPMAnalogHiresIn):
            self.__source.target = self
            self._lu.input.value = self._hires_input_channel_index[self.__source.inst_name]
        elif isinstance(self.__source,nanosurf.spmcontroller.spmchannelmux.SPMLockInMux):
            self.__source.target = self
