...
class JJSGF(object):
    ...
    def parameters(self):
        ...
        (Optional) Currently used to specify only which tag-format should be used. There are two possible values: + `\"tag-format\" : \"semantics/1.0\"` for [tag semantics that allow ECMA expressions](https://www.w3.org/TR/semantic-interpretation/#SI3.2.2) + `\"tag-format\" : \"semantics/1.0-literals\"` for [literal tag semantics](https://www.w3.org/TR/semantic-interpretation/#SI3.2.3). </br> Note: in the above links look for ABNF examples which are more similar to JSGF.   # noqa: E501
