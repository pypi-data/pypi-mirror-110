...
class TranscribeApi(object):
    ...
    def asr_transcribe_async_delete(self, session_id, **kwargs):  # noqa: E501
        ...
        Delete transcript and any captured audio (if applicable). Currently it can only be used to delete transcription that has been completed. Calling this method when transcription is in progress will return a 409 (Conflict) error.   # noqa: E501
    ...
    def asr_transcribe_async_post(self, **kwargs):  # noqa: E501
        ...
        Start speech transcription in asynchronous mode. Obtain results later.  This request will return as soon as access to resources is verified.  The response will contain session id, but will not contain result.  Session id can be used to [poll](#operation/asrTranscribeAsyncGet) for intermediate and final results.  If a [callback](#operation/asrTranscribeCallback) URI was registered in the initial request, then the final result will be delivered to that URI.  NOTE, data fields in the polling response, and the fields in the callback request will be the same.   See our Zendesk knowledge-base for examples of using this method: + [Example of word-tree output from async real-time transcription](https://support.voicegain.ai/hc/en-us/articles/360045632531-Example-of-word-tree-output-from-async-real-time-transcription) + [Example of streaming audio via websocket and receiving text via websocket](https://support.voicegain.ai/hc/en-us/articles/360050460931-Example-of-streaming-audio-via-websocket-and-receiving-text-via-websocket) + [Websocket STOMP payload for real-time transcription results](https://support.voicegain.ai/hc/en-us/articles/360045569592-Websocket-STOMP-payload-for-real-time-transcription-results)   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
