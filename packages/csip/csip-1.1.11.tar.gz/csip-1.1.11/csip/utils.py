"""
 * $Id: 1.1+11 utils.py de8100d559ad 2021-06-25 od $
 *
...
class Client(object):
    ...
    def __get0(url: str, conf: Dict):
        """HTTP GET to generate a Client"""
        conf = conf or {}
        timeout = (conf.get('http_conn_timeout', Client.CONN_TIMEOUT), conf.get('http_read_timeout', Client.READ_TIMEOUT))
    ...
    def __post0(url: str, files: Dict, headers: Dict, conf: Dict, sync: bool = True):
        ...
        conf = conf or {}
        if sync:
            timeout = (conf.get('http_conn_timeout', Client.CONN_TIMEOUT), conf.get('service_timeout', Client.SERVICE_TIMEOUT))
        else:
            timeout = (conf.get('http_conn_timeout', Client.CONN_TIMEOUT), conf.get('http_read_timeout', Client.READ_TIMEOUT))
    ...
    def get_catalog(url: str, conf: Dict = None) -> List['Client']:
        ...
        response, http_status = Client.__get0(url, conf)
        if http_status != 200:
            raise Exception("HTTP Error " + http_status)
    ...
    def save_to(self, file: str) -> None:
        ...
        if self.parent:
            d[Client.PARAMETER] = self.parent.data
            d[Client.RESULT] = self.data
        else:
            d[Client.PARAMETER] = self.data
    ...
    def download_data_files(self, files: List[str] = None, dir: str = ".", prefix: str = '', conf: Dict = None) -> None:
        """ Downloads file(s) if this Client object is a valid response and a query url as value.
            If no 'file' argument is provided, all files will be downloaded.
        ...
        for name in files:
            ...
            if http_status == 200:
                with open(os.path.join(dir, prefix + name), 'wb') as outfile:
                    outfile.write(response.content)
            else:
                raise Exception("Error downloading : " + val)
