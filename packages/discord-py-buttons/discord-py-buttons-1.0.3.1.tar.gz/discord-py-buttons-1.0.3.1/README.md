...
These events can be received trought `client.listen('on_eventName')` or can be awaited with `client.wait_for('eventName', filter)`

<details>
<summary>button_press</summary>
...
```py
btn, msg = await client.wait_for('button_press', lambda btn, msg: check here)
```
