...
class BaseModelAdapter:
    ...
    def load_from_snapshot(self, local_path, snapshot_id, **kwargs):
        ...
        if snapshot.bucket.is_remote:
            ...
            if local_path != snapshot.bucket.local_path:
    ...
    def save_to_snapshot(self, local_path, snapshot_name=None, description=None, cleanup=False, **kwargs):
        ...
        #       do we want to override current bucket or just create new local and replace?
    ...
    def prepare_trainset(self, data_path, partitions=None, filters=None, **kwargs):
        ...
        for partition in partitions:
            ...
            self.logger.info("Downloaded {} complete. {} total items".format(partition, len(ret_list)))
        ...
        if with_upload:
            try:
                ...
                if isinstance(all_predictions[0],  entities.AnnotationCollection):
                    ...
                    if not all([pred.type == self.model_entity.output_type for pred in itertools.chain(*all_predictions)]):
    ...
    def dataloop_keras_callback(self):
        ...
        try:
            ...
            raise RuntimeError('{} depends on extenral package. Please install '.format(self.__class__.__name__)) from err
