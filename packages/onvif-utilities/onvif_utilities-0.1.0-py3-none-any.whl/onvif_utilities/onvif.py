...
from onvif_utilities.wsdl import SERVICES
from onvif_utilities.exceptions import ONVIFAuthError, ONVIFError, ONVIFTimeoutError
...
class ONVIFService:
    ...
    def __init__(
        ...
        wsse = UsernameDigestTokenDtDiff(
            user, passwd, dt_diff=dt_diff, use_digest=True
        )
        # Create soap client
...
class ONVIFCamera:
    ...
    def get_definition(self, name, port_type=None):
        ...
        # XAddr for media is fixed:
        if name == 'media':
            xaddr = "{}:{}/onvif/Media".format(
                self.host
                if (self.host.startswith("http://") or self.host.startswith("https://"))
                else "http://%s" % self.host,
                self.port,
