...
def expected_one_wstl(fixtures: Dict[str, List[Dict[str, Any]]],
    ...
    :param resource_path: the resource path. Optional. If None, then /{coll} is used as the resource_path.
    :param include_root: whether to use absolute URLs or just paths.
    :return: a run-time WeSTL document as a dict.
    """
    if resource_path is None:
        resource_path = str(URL(f'/{coll}'))
    ...
    for action, action_name, action_rel, action_url in ((wstl_builder.find_action(a.name), a.name, a.rel, a.url) for a
        ...
        action = {**action,
                  'href': action_url if action_url else (
                      f'{_href(resource_path) / ""}{{id}}' if include_root else f'{URL(resource_path) / ""}{{id}}'),
...
def expected_one(fixtures: Dict[str, List[Dict[str, Any]]], coll: str, wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 resource_path: str = None, include_root=False,
                 get_actions: Optional[List[ActionSpec]] = None) -> List[Dict[str, Any]]:
    """
    ...
    """
    if resource_path is None:
        resource_path_ = str(URL(f'/{coll}'))
    else:
        resource_path_ = resource_path
    id_ = fixtures[coll][0]['id']
    href = _href(resource_path_, id_)
            ...
            if 'item' in targets and 'read' in targets and 'cj' in targets:
                links.append({
                    ...
                    'href': uritemplate.expand(url, {'id': id_}) if url else (
                        str(_href(resource_path_) / id_) if include_root else str(URL(resource_path_) / id_)),
                    'rel': ' '.join(rel)
                })
        return links
...
def expected_one_properties_form(fixtures: Dict[str, List[Dict[str, Any]]],
                                 coll: str,
                                 wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                                 properties_action_name: str,
                                 properties_action_rel: Optional[List[str]] = None,
                                 resource_path: str = None,
    ...
    for modifying the HEAObject's properties.

    :param fixtures: mongodb collection name -> list of HEAObject dicts. Required.
    :param coll: the mongodb collection name to use. Required.
    :param wstl_builder: a runtime WeSTL document builder object. Required.
    :param properties_action_name: the name of the service's properties action. Required.
    :param properties_action_rel: list of rel strings for the action. Optional.
    :param resource_path: the resource path. Optional. If None, then /{coll} is used as the resource_path.
    :param include_root: whether to use absolute URLs or just paths.
    :return: a list of Collection+JSON templates as dicts.
    """
    return _expected_one_form(fixtures, coll, wstl_builder, properties_action_name,
                              properties_action_rel, resource_path, include_root, '/properties')
...
def expected_one_duplicate_form(fixtures: Dict[str, List[Dict[str, Any]]],
    ...
    return _expected_one_form(fixtures, coll, wstl_builder, duplicate_action_name,
                              duplicate_action_rel, resource_path, include_root, '/duplicator')
...
def _expected_one_form(fixtures: Dict[str, List[Dict[str, Any]]],
    ...
    if action is None:
        raise ValueError(f'Action {action_name} does not exist')
    id_ = fixtures[coll][0]['id']
    href = _href(resource_path, id_ + (suffix if suffix else ''))
    data_ = []
    for x, y in fixtures[coll][0].items():
        if x != 'type':
            if isinstance(y, dict):
    ...
    return [{
        'collection': {
            'version': '1.0',
            'href': str(href) if include_root else str(URL(resource_path) / id_),
            'items': [
                {
                    'data': data_,
                    'links': []}],
    ...
    def runtime_actions():
        ...
        for action, action_name, action_rel, action_url in ((wstl_builder.find_action(a.name), a.name, a.rel, a.url) for
            ...
            if action is None:
                raise ValueError(f'Action {action_name} does not exist')
            targets = action.get('target', '').split()
            if ('href' in targets and 'cj' in targets) or 'cj-template' in targets:
                if 'item' in targets:
                    href = action_url if action_url else (
                        str(_href(resource_path) / '') + '{id}' if include_root else str(
                            URL(resource_path) / '') + '{id}')
                else:
                    href = action_url if action_url else (
                        str(_href(resource_path) / '') if include_root else str(URL(resource_path) / ''))
                action['href'] = href
...
def expected_all(fixtures: Dict[str, List[Dict[str, Any]]], coll: str, wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 resource_path: str = None, include_root=False, get_all_actions: Optional[List[ActionSpec]] = None) -> List[Dict[str, Any]]:
    """
        ...
        for action, rel, url in ((wstl_builder.find_action(a.name), a.rel or [], a.url) for a in get_all_actions):
            ...
            if 'item' in targets and 'read' in targets and 'cj' in targets:
                links.append({
                    'prompt': action['prompt'],
                    'href': uritemplate.expand(url, {'id': id_}) if url else (
                        str(_href(resource_path) / id_) if include_root else str(URL(resource_path) / id_)),
                    'rel': ' '.join(rel)
                })
        return links
    ...
    for f in fixtures[coll]:
        ...
        items = [item]
        collection_doc = {'collection': {'href': str(_href(resource_path) / ''),
                                         'items': items,
                                         'version': '1.0'}}
        for action, action_name in ((wstl_builder.find_action(a.name), a.name) for a in get_all_actions):
            if action is None:
            ...
            if 'cj-template' in targets:
                template = {'data': [{'name': i['name'],
                                      'value': i['value'] if 'add' in targets else (f.get(i['name'], None) if i['name'] not in ('meta', 'type') else None),
                                      'prompt': i.get('prompt', ''),
...
def expected_values(fixtures: Dict[str, List[Dict[str, Any]]],
    ...
    return {
        ...
        'expected_one_wstl': expected_one_wstl(fixtures, coll, wstl_builder, get_actions=get_actions,
                                               resource_path=resource_path, include_root=include_root),
        'expected_one': expected_one(fixtures, coll, wstl_builder, resource_path=resource_path,
                                     include_root=include_root, get_actions=get_actions),
        'expected_one_properties_form': expected_one_properties_form(fixtures, coll, wstl_builder,
                                                                     properties_action_name,
                                                                     resource_path=resource_path,
                                                                     include_root=include_root),
        'expected_one_duplicate_form': expected_one_duplicate_form(fixtures, coll, wstl_builder,
                                                                   duplicate_action_name,
                                                                   resource_path=resource_path,
                                                                   include_root=include_root),
        'expected_all_wstl': expected_all_wstl(fixtures, coll, wstl_builder, get_all_actions=get_all_actions,
                                               resource_path=resource_path, include_root=include_root),
        'expected_all': expected_all(fixtures, coll, wstl_builder, resource_path=resource_path,
                                     include_root=include_root, get_all_actions=get_all_actions)
    }


def _href(resource_path: str, id_=None) -> URL:
    url = URL('http://localhost:8080').with_path(resource_path)
    return url / id_ if id_ else url
