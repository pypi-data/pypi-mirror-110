...
async def get_from_wstl(request: web.Request, run_time_wstl_doc: Dict[str, Any]) -> web.Response:
    """
    ...
    :param request: the HTTP request (required).
    :param run_time_wstl_doc: a run-time WeSTL document containing data.
    :return: a Response object with a body containing the object in a JSON array of name-value pair (NVP) objects.
    """
    return await _handle_get_result_from_wstl(request, run_time_wstl_doc)


async def get_all_from_wstl(request: web.Request, run_time_wstl_docs: List[Dict[str, Any]]) -> web.Response:
    """
    ...
    :param request: the HTTP request (required).
    :param run_time_wstl_docs: a list of run-time WeSTL documents containing data.
    :return: a Response object with a body containing the object in a JSON array of name-value pair (NVP) objects.
    """
    return await _handle_get_result_from_wstl(request, run_time_wstl_docs)
...
async def _handle_get_result(request: web.Request, data: Union[Dict[str, Any], List[Dict[str, Any]]]) -> web.Response:
    ...
    :param data: the retrieved HEAObject(s) as a dict or an iterable of dicts, with each dict representing a
    ...
    if data is not None:
        ...
        body = await representor.formats(request, run_time_doc, request.url)
...
async def _handle_get_result_from_wstl(request: web.Request,
                                       run_time_wstl_docs: Union[Dict[str, Any], List[Dict[str, Any]]]) -> web.Response:
    """
    ...
    """
    _logger = logging.getLogger(__name__)
    _logger.debug('Result is %s', run_time_wstl_docs)
    if run_time_wstl_docs is not None:
        if isinstance(run_time_wstl_docs, list):
            run_time_wstl_docs_ = run_time_wstl_docs
        else:
            run_time_wstl_docs_ = [run_time_wstl_docs]
        wstl_builder = request[requestproperty.HEA_WSTL_BUILDER]
        run_time_docs = []
        for run_time_wstl_doc in run_time_wstl_docs_:
            wstl_builder_ = copy.deepcopy(wstl_builder)
            wstl_builder_.data = run_time_wstl_doc['wstl']['data']
            for action in run_time_wstl_doc['wstl']['actions']:
                if not wstl_builder_.has_design_time_action(action['name']):
                    wstl_builder_.add_design_time_action(action)
                wstl_builder_.add_run_time_action(action['name'],
                                                  path=action['href'],
                                                  root=request.app[appproperty.HEA_COMPONENT],
                                                  rel=action.get('rel', None))
            run_time_doc = wstl_builder_()
            _logger.debug('Run-time WeSTL document: %s', run_time_doc)
            run_time_docs.append(run_time_doc)
        representor = representor_factory.from_accepts_header(request.headers[hdrs.ACCEPT])
        _logger.debug('Using %s output format', representor)
        body = await representor.formats(request, run_time_docs, request.url)
        return status_ok(body=body, content_type=representor.MIME_TYPE)
    else:
        return web.HTTPNotFound()
