...
if config.MULTIpr:  # in multi-processing mode ...
    ...
    from sfalmanac.lib.alma_skyfield import ariesGHA, venusGHA, marsGHA, jupiterGHA, saturnGHA, sunGHA, moonGHA, moonVD, sunSD, moonSD, vdm_Venus, vdm_Mars, vdm_Jupiter, vdm_Saturn, ariestransit, stellar_info, planetstransit, moonage, moonphase, equation_of_time, getParams, find_new_moon
    ...
    from sfalmanac.lib.alma_skyfield import ariesGHA, venusGHA, marsGHA, jupiterGHA, saturnGHA, sunGHA, moonGHA, moonVD, sunSD, moonSD, vdm_Venus, vdm_Mars, vdm_Jupiter, vdm_Saturn, ariestransit, stellar_info, planetstransit, twilight, moonrise_set, moonage, moonphase, equation_of_time, getParams, find_new_moon
...
def doublepage(date, page1, pnum, ts, spad, dfcm):
...
def pages(first_day, pnum, ts, spad, dfcm):
    ...
    for i in range(pnum):
        if pnum == 122:	# if Full Almanac for a year...
            ...
            if cmth != pmth:
                ...
                sys.stdout.flush()
            pmth = cmth
        out = out + doublepage(first_day,page1,pnum,ts,spad,dfcm)
        page1 = False
...
def almanac(first_day, pagenum, ts, spad, df180, df360, dfcm):
...
\restoregeometry    % so it does not affect the rest of the pages'''
    ...
    alm = alm + pages(first_day,pagenum,ts,spad,dfcm)
    alm = alm + '''
\end{document}'''
    return alm