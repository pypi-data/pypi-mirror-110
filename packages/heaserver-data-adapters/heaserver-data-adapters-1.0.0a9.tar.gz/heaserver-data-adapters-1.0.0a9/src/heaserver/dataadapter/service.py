...
@action('heaserver-data-adapter-open', path='/dataadapters/{id}')
...
@routes.get('/dataadapters/{id}/properties')
@action(name='heaserver-data-adapters-properties-form', path='/dataadapters/{id}')
async def get_data_adapter_properties_form(request: web.Request) -> web.Response:
    """
    Gets the data adapter with the specified id, including a template for editing the data adapter's properties.
    :param request: the HTTP request.
    :return: the requested data adapter or Not Found.
    """
    return await mongoservicelib.get(request, MONGO_DATA_ADAPTER_COLLECTION)
...
@action(name='heaserver-registry-data-adapters-form', path='/dataadapters/{id}')
async def get_data_adapter_duplicate_form(request: web.Request) -> web.Response:
    """
