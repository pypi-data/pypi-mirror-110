...
import mutwo.converters.symmetrical.tempos    >>> from mutwo.converters import symmetrical
    >>> tempo_point_converter = mutwo.converters.symmetrical.tempos.TempoPointConverter()
    """
...
import mutwo.converters.symmetrical.tempos    >>> import expenvelope
import mutwo.converters.symmetrical.tempos    >>> from mutwo.converters import symmetrical
    >>> from mutwo.parameters import tempos
    >>> tempo_envelope = expenvelope.Envelope.from_levels_and_durations(
    >>>     levels=[mutwo.converters.symmetrical.tempos.TempoPoint(60), 60, 30, 50],
    >>>     durations=[3, 0, 2],
    >>> )
    >>> my_tempo_converter = mutwo.converters.symmetrical.tempos.TempoConverter(tempo_events)
    """
...
import mutwo.converters.symmetrical.tempos        >>> from mutwo.events import basic
import mutwo.converters.symmetrical.tempos        >>> from mutwo.parameters import tempos
        >>> from mutwo.converters import symmetrical
        >>> tempo_events = basic.SequentialEvent(
        >>>     [basic.EnvelopeEvent(3, mutwo.converters.symmetrical.tempos.TempoPoint(60)),  # start with bpm = 60
...
import mutwo.converters.symmetrical.loudness        >>> from mutwo.converters import symmetrical
        >>> loudness_converter = mutwo.converters.symmetrical.loudness.LoudnessToAmplitudeConverter(1)
...
import mutwo.converters.symmetrical.metricities        >>> from mutwo.converters import symmetrical
        >>> metricity_converter = mutwo.converters.symmetrical.metricities.RhythmicalStrataToIndispensabilityConverter()
