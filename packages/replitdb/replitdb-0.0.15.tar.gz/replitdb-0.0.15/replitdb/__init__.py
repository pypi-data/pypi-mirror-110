...
DeprecationWarning("This package has been deprecated please use the official repldb package ( can be found at: https://pypi.org/p/replit ), if you want to keep using it, you can access the clients with _Client and _AsyncClient.")
...
class _AsyncClient():
    ...
    async def _view_json(self, view):
        request = await requests_async.get(self.url + '/' + str(view))
        if(request.status_code==404):
          return None
