...
from sensorialytics.helpers import numeric_types

__all__ = [
    'take_first', 'take_last', 'bin_variable', 'detect_outliers',
    'multiple_replace', 'find_regex', 'fill_empty_with_distr',
    'fill_str', 'norm_cov_matrix'
]
...
def bin_variable(s, min_value, max_value, width, how,
    ...
    if isinstance(s, pandas.Series):
        ...
        raise TypeError('Unsupported input type {} for s'.format(type(s)))
...
def __bin_series(s: pandas.Series, min_value, max_value, width, how,
    ...
    s_binned = s.copy()
    nan_values = numpy.array(s_binned.isna()) | numpy.array(
        (s_binned == errors))
...
def detect_outliers(df, n, features):
    ...
    for col in features:
        ...
        # append the outlier indices for col to the list of outlier indices
...
def multiple_replace(s: pandas.Series, replacing_dict: dict, default=None):
    ...
    :param replacing_dict: dictionary with replacements. Has to be in the form:
     {
        '<replacing_value_1>' : [<list_of_values_to_replace],
        '<replacing_value_2>' : [<list_of_values_to_replace] ...
     }
    ...
    if default is not None:
        s = s.map(
            dict(zip(replacing_dict.keys(), replacing_dict.keys()))).fillna(
            default)
    return s
...
def fill_empty_with_distr(s: pandas.Series, values_to_fill: list,
                          values_to_avoid: list = None):
    """
    ...
    distrib_data = distrib_data[
        ~distrib_data.index.isin(values_to_fill + compl_values_to_avoid)]
    distrib_data_dict = {k: v for k, v in enumerate(distrib_data.index)}

    xk = list(distrib_data_dict.keys())
    pk = distrib_data.values / distrib_data.values.sum()

    distrib = stats.rv_discrete(values=(xk, pk))

    filled_s = s.copy()
    filled_s[filling_index] = [distrib_data_dict[distrib.rvs()] for _ in
                               filling_index]
...
def fill_str(s: pandas.Series, length, fill_value: str = '0', fillna=False):
    if s.isna().any() and not fillna:
        warnings.warn('{} NaNs found in input series. Mapping skipped'.format(
            sum(s.isna())), Warning)

    return s.astype(float).apply(
        lambda x: str(int(x)).rjust(length, fill_value) if not math.isnan(
            x) else ''.rjust(length, fill_value) if fillna else x)
