...
def to_html(entry, *, config: Config, single_indent: int = 2) -> str:
    elements = [
        ...
        indent(3, doilink_to_html(entry)),
...
def doilink_to_html(entry):
    try:
        doi = get_doi(entry)
    except KeyError:
        return ""
    return f'<a class="doi-link" href="https://doi.org/{doi}" title="Link to official version of paper (may be a landing page with more information)"><img alt="" height="17px" width="17px" src="https://www.sosy-lab.org/research/pub/Icons/doi.svg">Publisher\'s Version</a>'
