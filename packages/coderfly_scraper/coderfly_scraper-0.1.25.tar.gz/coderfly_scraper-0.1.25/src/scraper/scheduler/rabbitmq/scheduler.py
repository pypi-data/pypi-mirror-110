...
class RabbitMQScheduler(object):
    ...
    def __init__(self, connection_url, max_idle_time, spider_parse_failed_times, current_requests, *args, **kwargs):
    ...
    def from_settings(cls, settings):
        ...
        current_requests = settings.get('CURRENT_REQUESTS', 1)
        return cls(rabbitmq_url, max_idle_time, spider_parse_failed_times, current_requests)
    ...
    def from_crawler(cls, crawler):
        ...
        crawler.signals.connect(scheduler._request_left_downloader, signals.request_left_downloader)  # 限制从rabbitmq取出数据的量
    ...
    def next_request(self):
        ...
        if self.is_closing:
            return None
        if self.current_requests_count >= self.current_requests:
            return None
