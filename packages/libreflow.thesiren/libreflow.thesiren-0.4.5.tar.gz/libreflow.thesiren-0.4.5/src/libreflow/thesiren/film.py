...
class BuildBlockingScene(BuildBlenderScene):
    ...
    def run(self, button):
        ...
        for dep in self.dependencies.mapped_items():
            ...
            if asset_type in ["chars", "props"]:
                self._shot_data["assets_data"].append((dep.name(), path, asset_type + '_', asset_number))
            else:
                self._shot_data["%s_path" % asset_type] = path
...
class RenderSequencePlayblasts(flow.Action):
    ...
    def add_to_render_pool(self, file, revision_name=None, use_simplify=False):
        ...
        file.render_playblast.revision_name.set(revision_name)
        file.render_playblast.use_simplify.set(use_simplify)
        file.render_playblast.run('Add to render pool')
