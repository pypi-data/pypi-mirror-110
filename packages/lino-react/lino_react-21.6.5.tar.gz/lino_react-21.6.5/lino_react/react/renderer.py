...
class Renderer(JsRenderer, JsCacheRenderer):
                    ...
                    for a in ba.actor.get_toolbar_actions(
                        ...
                        if a.action.combo_group == combo_group\
                            ...
                            if not isinstance(previous, list):
                                previous = [previous]
                            previous.append(a.action.action_name)
                            tba.append(previous)
                        else:  # is normal
                            tba.append(a.action.action_name)
                            combo_group = a.action.combo_group
                    action_descriptor["toolbarActions"] = tba
    ...
    def actor2json(self, v):
        ...
        result = dict(id=v.actor_id,
                      actions_list=self.actions2json(v._actions_list),
        ...
        hotkeys = v.get_actions_hotkeys()
        if hotkeys:
            result.update(hotkeys=hotkeys)
