...
class VadeSpotVehicleDetails:
    ...
    def from_json(j: dict):
        try:
            ...
            try:
                type_ops_raw = j.get("typeOptions", [])
        ...
        try:
            ...
            if isinstance(j, dict):
                for bound in j["bounds"]:
                    if isinstance(j, dict):
                        new_bounds.points.append(VadeSpotPoint(x=bound['x'], y=bound['y']))
                    else:
...
class CurbpaceOutput:
    occupancy_score = None
    vehicle: VadeSpotVehicleDetails = None

    @staticmethod
    def from_json(j: dict):
        try:
            new = CurbpaceOutput()
            new.occupancy_score = j.get("occupancyScore", None)
            new.vehicle = VadeSpotVehicleDetails.from_json(j.get("vehicleDetails", {}))
            return new
        except: return None

    def to_json(self):
        return {
            "occupancyScore": self.occupancy_score,
            "vehicles": self.vehicle.to_json(),
        }
...
class VadeSpotRealtime:
    ...
    curbspace_output: [CurbpaceOutput] = []
    expected_max_curbspace_occupancy: int = None
    ...
    def from_json(j: dict):
        try:
            ...
            new_spot.curbspace_output = []
            for curb_output in j.get("curbspaceOccupancy", []):
                curb_data = CurbpaceOutput.from_json(curb_output)
                if curb_data:
                    new_spot.curbspace_output.append(curb_data)
            new_spot.type = VadeSpotType.from_str(j.get("type", "standard"))
            if len(new_spot.curbspace_output) > 0:
                new_spot.occupied = True
            new_spot.expected_max_occupancy = j.get("expectedMaxCurbspaceOccupancy", None)
            return new_spot
        except Exception as e:
