...
@click.option('-f', '--format', required=False, type=click.Choice(['shell','json', 'yaml', 'csv']), default='shell', show_default=True, help=f"{CLI_PARAMETERS_HELP['common']['format']}")
    ...
    def check_command_usage():
        ...
        if input:
            if key or key_option:
                Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--key'","'--input'"))
        ...
        ### not input
            ...
            if not _key:
                Logger.error(MESSAGES['AtleastOneofTwoNeeded'].format("'--key'","'--input'"))
                Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
                exit(1)

            if value and value_option:
                Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'value'", "'--value'"))
                Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
                exit(1)
            ...
            if not _value:
                Logger.error(MESSAGES['MissingOption'].format("'--value'"))
                Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
                exit(1)
...
def list_parameter(stage, uninherited, query_values, query_all, query, format, working_dir, config, verbosity):
    ...
    def check_command_usage():
        ...
        if query and not format in ['json', 'yaml']:
            Logger.error(MESSAGES['QueryOptionCompatibleFormats'])
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)

        if query_all and format == 'shell':
            Logger.error(MESSAGES['QueryOptionNonCompatibleFormats'])
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)

        if query and query_all:
            Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--query'", "'--query-all'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)

        if query and query_values:
            Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--query'", "'--query-values'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)

        if query_all and query_values:
            Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--query-all'", "'--query-values'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)
    ...
    def check_command_usage():
        ...
        if decrypt:
            if not (query_values or query_all or query):
                Logger.error(MESSAGES['ArgumentsMutualInclusive'].format("Either '--query-values' or '--query-all' or '--query'", "'--decrypt'"))
                Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
                exit(1)
...
def get_config(key, local, _global, working_dir, config, verbosity):
    ...
    def check_command_usage():
        ...
        if local and _global:
            Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--local'", "'--global'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)
...
def set_config(key, key_option, value, value_option, _global, input, working_dir, config, verbosity):
    ...
    def check_command_usage():
        ...
        if value and input:
            Logger.error(MESSAGES['ArgumentsMutualExclusive'].format("'--value'","'--input'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)

        if not (value or input):
            Logger.error(MESSAGES['AtleastOneofTwoNeeded'].format("'--value'","'--input'"))
            Logger.info(MESSAGES['TryHelp'], stress = False, force=True)
            exit(1)
