    ...
    def handle_parse_result(self, ctx, opts, args):
        ...
        for other_param in ctx.command.get_params(ctx):
            ...
            if other_param.human_readable_name in self.not_required_if:
                ...
                if other_opt:
                    if current_opt:
                        Logger.error(MESSAGES['ArgumentsMutalExclusive'].format(str(self.name), str(other_param.human_readable_name)))                      
                        Logger.info(MESSAGES['TryHelpWithCommand'].format(ctx.command_path), stress=False)
                        ctx.abort()
                    else:
                        self.required = None
...
def modify_click_usage_error():
    ...
    def show(self, file=None):
        ...
        if self.ctx:
            ...
            # echo(MESSAGES['TryHelp'].format(self.ctx.command_path), color=self.ctx.color)
            # Logger.info(MESSAGES['TryHelpWithCommand'].format(self.ctx.command_path), force=True)
            Logger.info(MESSAGES['TryHelpWithCommand'].format(self.ctx.command_path), stress=False, force=True)
        else:
