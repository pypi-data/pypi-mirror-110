...
###--------------------------------------------------------------------------------------------
    ...
    def get_key_validator(self):
        ...
        if allowGroups is None:
            Logger.debug(f"'allowGroups' is not set for the parameter provider, defaulted to '{default_spec['allowGroups']}'.")
...
###--------------------------------------------------------------------------------------------
    ...
    def list(self, project, application, stage, uninherited):
        ...
        if prependGroups is None:
            Logger.debug(f"'prependGroups' is not set for the parameter provider, defaulted to '{default_spec['prependGroups']}'.")
            prependGroups = default_spec['prependGroups']

        groupDelimiter = Config.parameter_spec('groupDelimiter')
        if groupDelimiter is None:
            Logger.debug(f"'groupDelimiter' is not set for the parameter provider, defaulted to '{default_spec['groupDelimiter']}'.")
        ...
        if nestedDelimiter is None:
            Logger.debug(f"'nestedDelimiter' is not set for the parameter provider, defaulted to '{default_spec['nestedDelimiter']}'.")
...
###--------------------------------------------------------------------------------------------
    ...
    def get(self, project, application, stage, key):
        ...
        if not found or len(found) == 0:
            raise DSOException(f"Parameter '{key}' not found nor inherited: project={project}, application={application}, stage={Stages.shorten(stage)}, key={key}")
        else:
            # if len(found) > 1:
            ...
            if not found[0]['Type'] in ['String', 'StringList']:
                raise DSOException(f"Parameter not found: project={project}, application={application}, stage={Stages.shorten(stage)}, key={key}")
