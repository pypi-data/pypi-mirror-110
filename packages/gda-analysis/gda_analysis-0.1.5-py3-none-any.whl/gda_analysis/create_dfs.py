...
def main():
    ...
    vcf_data = create_vcf_df(sys.argv[1])
    print("/nplease enter genome studio file path: ")
    gs_data = create_gs_df(sys.argv[2])
    print("/nplease enter path of merged data excel output: ")
    merged_data = join_vcf_and_gs(vcf_data, gs_data)
    merged_data.to_excel(sys.argv[3], encoding='utf-8')
