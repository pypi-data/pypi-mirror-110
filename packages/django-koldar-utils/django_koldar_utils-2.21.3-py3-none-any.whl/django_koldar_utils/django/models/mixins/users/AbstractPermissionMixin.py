...
from django_koldar_utils.django.models.AbstractModelMetaclass import AbstractModelMetaclass

TPERMISSION = TypeVar("TPERMISSION")


class AbstractPermissionsMixin(Generic[TPERMISSION], models.Model, metaclass=AbstractModelMetaclass):
    """
    Add only the methods necessary to support the Group and Permission of a django user implementation.
    It is different from normal PermissionsMixin provided by django because that mixin assume 2 relationships:
    groups and permissions. This mixin does not assume them. This structure also does not assume permission type.

    You mau not that this mixin does not actually store the permissions. This is ebcause this mixin (that should be attached
    to the user) calls all the authentcation backend and, for each of them, call "get_xxx_permissions" methods (if defined)
    to fetch the concrete permissions. In other words, it is the authentication backend that actually specifies the permissions,
    not this mixin
    """

    class Meta(type(Generic[TPERMISSION]), type(models.Model), type(abc.ABC)):
        abstract = True
