...
#                 raise ValueError(f"Failed data type conversion, use BaseObjectStorageDataset(header='infer') if the dataset has a header:  {err}")
...
#     def __init__(self, osds, dtype = None):
#         self.iter = iter(osds)
#         self.dtype = dtype
...
#                 pt_tensor = pt_tensor.to(dtype = self.dtype) if self.dtype is not None else pt_tensor
#                 yield pt_tensor
#                 val = next(self.iter)
#             except ValueError as err:
#                 raise ValueError(f"Failed data type conversion, use BaseObjectStorageDataset(header='infer') if the dataset has a header:  {err}")
            ...
            try:
                ...
                if self.header is None:
                    ...
                    for (skiprows, nrows, uri) in df_read_spec(self.metadata_df, self.row_count, row_start_idx, row_end_idx):
                        if (skiprows == 0):
                            src_df_list.append(pd.read_csv(f"filecache::{uri}", header=None, nrows = nrows, skiprows = 1))
                        else:
