...
######################################################################
 ...
 OPTIONAL INPUT KEYWORDS:
       ...
       SIGMAPSF - Scalar giving the sigma of the PSF of the high resolution
         ...
         SOL - If this keyword has at least 8 elements in input, the Sigma,
               qObs and PA will be used as starting point for the optimization.
 ...
 OPTIONAL OUTPUT KEYWORDS:
         SOL - Output keyword containing a 4xNgauss array with the
               ...
               2) sol[1,*] = Sigma, is the dispersion of the best-fitting
                  Gaussians in pixels.
               3) sol[2,*] = qObs, is the observed axial ratio (q' in the 
                  notation of Cappellari 2002) of the best-fitting Gaussian 
                  components.
...
class mge_fit_sectors_twist:
    ...
    def __init__(self, radius, angle, counts, eps,
        ...
        if sol.size < 8:
            logsigma = _linspace_open(*lrbounds, num=ngauss)  # open grid
        ...
        parinfo = [{'step':0.01, 'limits':lrbounds, 'limited':[1, 1], 'tied':''}
                    for j in range(3*ngauss)]
        ...
        for j in range(2*ngauss, 3*ngauss):
            parinfo[j]['limits'] = pabounds
            parinfo[j]['step'] = 1.0 # degrees: step for numerical derivative
        ...
        mp = mpfit(self._fitfunc, pars, iterfunct=iterfunc,
        ...
        if not self.quiet:
            ...
            for sol in self.sol.T:
                print(('{:13.6e}{:#13.6g}{:13.6f}{:13.4f}').format(*sol))
            print('+++++++++++++++++++++++++++++++++++++++++++++++++++++')
...
# -------------------------------------------------------------------------------
    ...
    def _print(self, myfunct, logsigma, it, fnorm,
               functkw=None, args=None, parinfo=None, dof=None, quiet=0):
        """
...
#----------------------------------------------------------------------------
    ...
    def _fitfunc(self, pars, fjac=None):
        ...
        if self.negative:   # Solution by LAPACK linear least-squares
            ...
            self.weights = optimize.nnls(A, b, maxiter=30*q.size)[0]
