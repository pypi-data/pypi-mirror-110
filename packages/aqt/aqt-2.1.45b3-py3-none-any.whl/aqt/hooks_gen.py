...
class _AddonsDialogWillDeleteAddonsHook:
    ...
    def append(
        self, cb: Callable[["aqt.addons.AddonsDialog", "List[str]"], None]
    ) -> None:
...
class _BrowserDidFetchColumnsHook:
    ...
    def append(self, cb: Callable[["Dict[str, aqt.browser.Column]"], None]) -> None:
        """(columns: Dict[str, aqt.browser.Column])"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["Dict[str, aqt.browser.Column]"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _CurrentNoteTypeDidChangeHook:
    ...
    def append(self, cb: Callable[["NotetypeDict"], None]) -> None:
        """(notetype: NotetypeDict)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["NotetypeDict"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _DebugConsoleDidEvaluatePythonFilter:
    ...
    def append(
        self, cb: Callable[["str", "str", "aqt.forms.debug.Ui_Dialog"], str]
    ) -> None:
...
class _DeckConfDidLoadConfigHook:
    ...
    def append(
        self,
        cb: Callable[["aqt.deckconf.DeckConf", "DeckDict", "DeckConfigDict"], None],
    ) -> None:
...
class _DeckOptionsDidLoadHook:
    ...
    def append(self, cb: Callable[["aqt.deckoptions.DeckOptionsDialog"], None]) -> None:
        """(deck_options: aqt.deckoptions.DeckOptionsDialog)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["aqt.deckoptions.DeckOptionsDialog"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _DefaultSearchFilter:
    ...
    def append(self, cb: Callable[["str", "Card"], str]) -> None:
        """(current_search: str, c: Card)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["str", "Card"], str]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _FieldsDidDeleteFieldHook:
    ...
    def append(
        self, cb: Callable[["aqt.fields.FieldDialog", "anki.models.FieldDict"], None]
    ) -> None:
...
class _FieldsDidRenameFieldHook:
    ...
    def append(
        self,
        cb: Callable[["aqt.fields.FieldDialog", "anki.models.FieldDict", "str"], None],
    ) -> None:
...
class _FocusDidChangeHook:
    ...
    def append(
        self, cb: Callable[["Optional[QWidget]", "Optional[QWidget]"], None]
    ) -> None:
...
class _OperationDidExecuteHook:
    ...
    def append(
        self, cb: Callable[["anki.collection.OpChanges", "Optional[object]"], None]
    ) -> None:
...
class _ReviewerDidAnswerCardHook:
    ...
    def append(
        self,
        cb: Callable[["aqt.reviewer.Reviewer", "Card", "Literal[1, 2, 3, 4]"], None],
    ) -> None:
...
class _StateDidUndoHook:
    ...
    def append(self, cb: Callable[["OpChangesAfterUndo"], None]) -> None:
        """(changes: OpChangesAfterUndo)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["OpChangesAfterUndo"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _UndoStateDidChangeHook:
    ...
    def append(self, cb: Callable[["UndoActionsInfo"], None]) -> None:
        """(info: UndoActionsInfo)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["UndoActionsInfo"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
