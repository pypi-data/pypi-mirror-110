...
from .files_op import (check_file_name_structure, clean_file,
                       get_containing_repo_dir)
from .find_replace import replace_in_files
from .utils import deduce_package_path, multi_startswith
...
@click.option('-s', '--source_path', 'source_path', help='Source absolute or relative path')
@click.option('-t', '--target_path', 'target_path', help='Target absolute or relative path')
@click.option('-p', '--package', 'package_path', default=None)
@click.option('-e', '--elements', 'func_names', multiple=True, required=True)
@click.command()
def refacto(source_path, target_path, func_names=None, package_path=None):
...
#         # parser.add_argument(
#         #     '-s', '--source', help='Source absolute or relative path', required=True, type=str)
#         # parser.add_argument(
#         #     '-t', '--target', help='Target absolute or relative path', required=True, type=str)
#         # parser.add_argument("--elements", "-e", nargs="+", required=True)
#         # parser.add_argument("--package", "-p", default=None)

#         # args = parser.parse_args()
#         # refacto(source_path=args.source, target_path=args.target,
#         #                         func_names=args.elements, package_path=args.package)
#     except:
#         extype, value, tb = sys.exc_info()
#         traceback.print_exc()
#         pdb.post_mortem()
