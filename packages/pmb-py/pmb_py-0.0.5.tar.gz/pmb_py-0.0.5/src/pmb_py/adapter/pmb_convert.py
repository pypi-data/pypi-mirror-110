...
def to_project(item):
    return model.Project(item['id'], item['name'], item['typeId'], item['code'])
...
def to_pmb_block(item):
    return model.Block(item['id'], item['date'], item['duration'], item['task_id'], item['member_id'], item['project_id'], item['type_id'])
