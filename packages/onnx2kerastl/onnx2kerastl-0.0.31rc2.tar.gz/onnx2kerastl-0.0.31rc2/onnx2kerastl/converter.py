...
def onnx_to_keras(onnx_model, input_names,
        ...
        for onnx_i in onnx_inputs:
            if onnx_i.name == input_name:
                ...
                if "." in input_name:
                    ...
                    dot_input_names[input_name] = new_name
                    input_name = new_name
                layers[input_name] = keras.layers.InputLayer(input_shape=input_shape, name=input_name).output
    ...
    for node_index, node in enumerate(onnx_nodes):
        ...
        for i, node_input in enumerate(node.input):
            node_input = dot_input_names.get(node_input,node_input)
