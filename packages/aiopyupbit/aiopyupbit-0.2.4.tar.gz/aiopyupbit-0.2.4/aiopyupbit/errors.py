...
class InValidPriceAsk(UpbitError):
    def __str__(self):
        return "매도 주문가격 단위가 올바르지 않습니다."

class InValidPriceBid(UpbitError):
    def __str__(self):
        return "매수 주문가격 단위가 올바르지 않습니다."
...
async def raise_error(response):
    ...
    if code == 401:
        ...
        if name == "under_min_total_bid":
            raise UnderMinTotalBid()
        elif name == "under_min_total_ask":
            raise UnderMinTotalAsk()
        elif name == "insufficient_funds_bid":
            raise InsufficientFundsBid()
        elif name == "insufficient_funds_ask":
            raise InsufficientFundsAsk()
        elif name == "invalid_price_ask":
            raise InValidPriceAsk
        elif name == "invalid_price_bid":
            raise InValidPriceBid
