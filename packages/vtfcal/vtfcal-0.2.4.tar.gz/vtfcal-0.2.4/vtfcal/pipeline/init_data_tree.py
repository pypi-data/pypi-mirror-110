...
def init_data_tree(indir, legacy_spec0122=False):
    ...
    for short, long_ in channels:
        tree = {
            ...
            "support": {"data_dir": join(indir, "support")},
            "processed": {"data_dir": join(indir, "processed")},
        ...
        asdf_file = add_files_by_keywords(
            asdf_file, "darks", {"DKIST003": "dark"}, raw=True
        )
        ...
        for dkey, head_kw in zip(["flats", "data"], ["gain", "observe"]):
            for m in range(4):
                search_keys = (
                    {"VTF__002": chankey, "VTF_031": m, "DKIST003": head_kw}
                    if legacy_spec0122
                    else {"VTF_002": chankey, "VTF__021": m, "DKIST003": head_kw}
                )
                asdf_file = add_files_by_keywords(
                    asdf_file, f"{dkey} modstate{m}", search_keys, raw=True,
                )
