...
def main():
    ...
    parser.add_argument("-p", "--plain", dest="plain", action="store_true", default=False, help="in case no ansi color use plain output than colorful",)
    ...
    def cutkeys(ds) :
        if not _xdt_keys_included and not _xset_keys_excluded :
            return ds
        if type(ds) is dict :
            ds = {k:v for k, v in ds.items() if (_xdt_keys_included and k.lower() in _xdt_keys_included ) or (_xset_keys_excluded and k.lower() not in _xset_keys_excluded)}
            for k,v in ds.items() :
                if not _xdt_keys_included[k.lower()] :
                    ds[k] = cutkeys(v)
        if type(ds) is list :
            for i,v in enumerate(ds) :
                if type(v) in [dict,list] :
