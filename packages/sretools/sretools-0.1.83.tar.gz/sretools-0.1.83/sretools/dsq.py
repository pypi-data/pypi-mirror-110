...
def main():
    ...
    if _x_args.keys_included :
        _xset_keys_included = set(_x_args.keys_included.split(","))
    else :
        _xset_keys_included = None
    if _x_args.keys_excluded :
        _xset_keys_excluded = set(_x_args.keys_excluded.split(","))
    else :
    ...
    def cutkeys(ds) :
        if not _xset_keys_included and not _xset_keys_excluded :
            return ds
        if type(ds) is dict :
            ds = {k:v for k, v in ds.items() if (_xset_keys_included and k in _xset_keys_included ) or (_xset_keys_excluded and k not in _xset_keys_excluded)}
            for k,v in ds.items() :
                ds[k] = cutkeys(v)
        if type(ds) is list :
            for i,v in enumerate(ds) :
