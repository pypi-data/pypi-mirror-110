...
def main():
    ...
    def choiceexpand(code) :
        ...
        dt = m.group(1)
        keys = m.group(3)
        res = before + "{" + ",".join(["'{}'".format(k)  + ":" + "{}.{}".format(dt,k) for k in keys.split(",")]) + "}" + end
        if _x_args.debug > 2:
            print("# choice expand, before = ",code)
            print("# choice expand, after  = ",res)
        return res
    ...
    def listexpand(code) :
        ...
        res += xcode
        if _x_args.debug > 2:
            print("# list expand, before = ",code)
            print("# list expand, after  = ",res)
        return res
    ...
    def dotexpand(code,keydt=None) :
        ...
        res = res+chgsin
        if _x_args.debug > 2:
            print("# dot expand, before = ",code)
            print("# dot expand, after  = ",res)
        return res
    ...
    if _x_args.interactive :
        try :
            ...
            print_err("[dsq]$ WARN: word completion disabled on combination of {}/{}".format(sys.platform,os.environ.get("TERM")),lvl=2)
            _x_session = None
        _x_dotkey = True
