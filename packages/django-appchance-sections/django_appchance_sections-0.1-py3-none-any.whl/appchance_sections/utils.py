...
from appchance_sections.models import Content as DefaultContent
...
def get_section_model():
    ...
    """
    try:
        return django_apps.get_model(settings.SECTION_MODEL, require_ready=False)
    except ValueError:
        raise ImproperlyConfigured("SECTION_MODEL must be of the form 'app_label.model_name'") from None
    except LookupError:
        raise ImproperlyConfigured(
            "SECTION_MODEL refers to model '%s' that has not been installed" % settings.SECTION_MODEL
        ) from None
...
def get_content_model():
    ...
    try:
        parts = settings.SECTION_CONTENT_MODEL.split(".")
