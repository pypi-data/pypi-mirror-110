...
def loadAndValidate(confPath, overridedata={}):
    ...
    with open(confPath) as ipf:
        t = jinja2.Template(ipf.read())
    ...
    if isinstance(dirs, list):
        log.warning(
            "DeprecationWarning: `directives` are specified as list. We advice to use dictionaries instead."
        )
        for idir in dirs:
            for itag, ival in idir.items():
                if itag == "config":
                    ...
                    if isubConfigTag in conf:
                        raise KeyError(
                            f'sub config with name "{isubConfigTag}" already exists. Rename subconfig'
                        )
                    conf[isubConfigTag] = yaml.load(
                        open(isubConfigPath), Loader=yaml.Loader
                    )
...
class ConfigProvider:
    ...
    def parse(self, overridedata={}):
        ...
        self.config["user"] = loadAndValidate(
            self.userConfigPath, overridedata=overridedata
        )
