    ...
    def __setitem__(self, key, value):
        if not isinstance(value, pd.DataFrame) and not isinstance(value, pd.Series):
            raise DataFieldError(
                'The value "{1}" of the dict entry "{0}" has to be a Dataframe or a Series.'.format(
                    key, value
                )
            )
...
class DataManager(dict):
    ...
    def _addToAccessLog(self, dataSetName, modifier):
        ...
        assert modifier in [
            "r",
            "w",
        ], "access log modifier needs to be read r or write w"
    ...
    def __setitem__(self, fieldname, dataset):
        ...
        assert self._overwriteFlag or fieldname not in self.keys(), (
            'dataset "{}" already in data manager. '
