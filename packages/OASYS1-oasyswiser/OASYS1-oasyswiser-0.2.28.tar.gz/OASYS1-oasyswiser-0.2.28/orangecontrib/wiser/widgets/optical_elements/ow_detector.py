...
class OWDetector(OWOpticalElement, WidgetDecorator):
    ...
    def build_mirror_specific_gui(self, container_box):
        ...
        le_HEW = oasysgui.lineEdit(best_focus_box, self, "ActualBestHew", "HEW at best focus [um]", labelWidth=220,
                               valueType=float, orientation="horizontal")
        ...
        try:
            ...
            QMessageBox.information(self,
                                    ...
                                    "Best Focus Found!\n\nPosition: " + str(self.oe_f2 + (
                                            ...
                                            index_min], 2) / 1e-3)) + " [m]" +
                                    "\nHEW: " + str(
                                        round(self.hews_list[index_min] * 1e6, 4)) + " [um]",
                                    QMessageBox.Ok
                                    )
            ...
            self.plot_histo(best_focus_positions * 1e6,
                            ...
                            title="(BEST FOCUS) Defocus Sweep: " + str(round(
                                self._defocus_sign * self.defocus_list[index_min] / 1e-3, 4)) +
                                  ...
                                  + " [m]" + ", HEW: " + str(self.ActualBestHew) + " [um]",
                            xtitle="Y [um]",
                            ytitle="Intensity",
                            log_x=False,
                            log_y=False)
            ...
            self.plot_canvas[3].setGraphYLabel("Size [um]")
    ...
    def do_find_focus_calculation(self):
        try:
            ...
            self.plot_histo(S * 1e6,
                            ...
                            tabs_canvas_index=2,
                            plot_canvas_index=2,
                            title="(BEST FOCUS) Position: " + str(self.ActualBestDefocus) + " [m] , HEW: " + str(self.ActualBestHew) + " [um]",
                            xtitle="Y [um]",
                            ytitle="Intensity",
                            log_x=False,
                            log_y=False)
        ...
        try:
            ...
            if index == self.best_focus_index:
                title = "(BEST FOCUS) Defocus Sweep: " + \
                        ...
                        " [m], HEW: " + str(round(self.hews_list[index]*1e6, 4)) + " [um]"
            else:
                title = "Defocus Sweep: " + str(1e3 * self._defocus_sign * round(self.defocus_list[index], 4)) + " [mm] " +\
                        " (" + str(index+1) + "/" + str(n_defocus) + "), HEW: " + str(round(self.hews_list[index]*1e6, 4)) + " [um]"
