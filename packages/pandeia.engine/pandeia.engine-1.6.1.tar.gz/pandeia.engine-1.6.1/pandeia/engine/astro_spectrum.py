...
class ConvolvedSceneCube(object):
    ...
    def __init__(self, scene, instrument, background=None, psf_library=None, webapp=False, empty_scene=False):
        ...
        for s in self.source_spectra:
            self.wave = syn.utils.merge_wavelengths(self.wave, s.wave)
        # Also merge in the background wavelengths
        self.wave = syn.utils.merge_wavelengths(self.wave, background.wave)
    ...
    def get_fov_size(self, pixbuffer=SCENE_FOV_PIXBUFFER):
        ...
        readout_pattern = self.instrument.detector["readout_pattern"]
...
class AdvancedPSF(object):
    ...
    def __init__(self, wave, psf_library, instrument, mode, model_scene=None, psf_source_offset=(0, 0),
                 aper_width=None, aper_height=None, multishutter=[(0.0, 0.0)], nslice=None, bg_w=0, single=False, empty_scene=False):
        ...
        npix_x = scene_npix_x / psf_upsamp
        npix_y = scene_npix_y / psf_upsamp
        ...
        if (model_scene is None) or (empty_scene):
            ...
            if single:
                ...
                mini_x = np.int((scene_npix_x - kernel_npix_x)/2) # minimum index of the kernel size within the FOV
                maxi_x = np.int((scene_npix_x + kernel_npix_x)/2) # maximum index of the kernel size within the FOV
                mini_y = np.int((scene_npix_y - kernel_npix_y)/2) # minimum index of the kernel size within the FOV
                maxi_y = np.int((scene_npix_y + kernel_npix_y)/2) # maximum index of the kernel size within the FOV
