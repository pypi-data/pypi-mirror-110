...
def read_dasp_data(name=None,dir_=None,num_shiyan='all',num_tongdao='all'):
    ...
    num_shiyan : TYPE, optional
        要读取的试验号，默认为'all'，如果需要读取部分可输入:[0,100].
    num_tongdao : TYPE, optional
        要读取的通道号，默认为'all'，如果需要读取部分可输入:[1,4].
    Returns
    -------
    ...
    for i in shiyanhao:
        pass
        ls_res_num1[i]=list(set([int(re_obj.findall(s)[-1]) for s in list_name if name+str(i) in s]))
    ls_res_num2={}  
    if num_shiyan=='all':  
        ...
        for i in ls_res_num1:
            pass
            if i in range(num_shiyan[0],num_shiyan[1]+1):
    ...
    if num_tongdao=='all':  
        ...
        for i in ls_res_num2:
            pass
            ls_res_num3[i]=[ s for s in ls_res_num2[i] if s in range(num_tongdao[0],num_tongdao[1]+1)]
