...
class Run():
    ...
    def write_method(f):
        ...
        def wrapper(self, *args, **kwargs):
            if self.read_only:
                print('Run is read only. No actions.')
                return
    ...
    def beta(f):
        @wraps(f)
        def wrapper(self, *args, **kwargs):
            print('This method is still in development and can be unstable.')
            return f(self, *args, **kwargs)
        return wrapper
    ...
    def will_be_depreciated(suggested_method):
        def _method(f):
            @wraps(f)
            def wrapper(self, *args, **kwargs):
                print(f'This method will be removed in a future release. Use {suggested_method} instead.')
                return f(self, *args, **kwargs)
            return wrapper
        return _method
    ...
    @write_method
    def add_artifacts(self, filepath: str, step: int = None):
        ...
        args = (self._id, filename, content_bytes, num_bytes, step)
        self.events_manager.add_event("add_file", args)
    ...
    def add_mlmodel_usage_from_dirpath(self, dirpath:str):
        """Log that this run is using a certain model. 
        """
    ...
    def add_mlmodel(self, source_path: str, name: str, version: str = None, short_description: str = None,
        ...
        """
        ops_upload_mlmodel(source_path=source_path, 
            mlmodel_name=name,
             run_id=self._id, 
             version=version,
    ...
    def add_dataset(self, source_path: str, name: str, version: str = None, short_description: str = None,
        ...
        push_dir: Must be set to True if the source_path is a directory
        """
        ops_upload_dataset(source_path=source_path, 
            dataset_name=name,
             run_id=self._id, 
             version=version,
             short_description=short_description,
    ...
    def add_tf_model(self, model: 'tf.keras.Model', filename: 'str', step: int = None):
        """Add a tensorflow model as an artifact.
        """
        self.add_tf_model_as_artifact(model=model, filename=filename, step=step)
    ...
    def add_tf_model_as_artifact(self, model: 'tf.keras.Model', filename: str, step: int = None):
        ...
        self.add_artifacts(f"{p}/{filename}.zip", step=step)
    ...
    def add_shared_artifact(self, filename: str):
        """Push an artifact to be shared accross the project.
        """
    ...
    def start_recording_logs(self, logger_name=None):
        ...
        if (self.log_recorder is None) or self.logger_name!=logger_name:
