...
def concat_spark_columns(data, name='concat_column', sep='|', return_data=False):
	...
	:rtype: SparkDF
	"""
	string_columns = [col[0] for col in data.dtypes if col[1].startswith('string')]

	data = data.withColumn(
		name, f.concat_ws(
			sep,
			*[
				f.col(col) if col in string_columns else f.lower(f.col(col).cast(StringType()))
...
def concat_pandas_columns(data, name='concat_column', sep='|', return_data=False):
	...
	def concat_row(x):
		return sep.join([
			...
			for value in x.values if value is not None
		])
