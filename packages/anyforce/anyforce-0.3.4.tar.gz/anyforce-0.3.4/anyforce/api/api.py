...
class API(Generic[UserModel, Model, CreateForm, UpdateForm]):
    ...
    @classmethod
    def ids_path(cls):
        return Path(..., title="id", examples={"单个 ID": "1", "多个 ID": "1,2"})
    ...
    def prefetch_query(cls):
        return Query(
            [],
            title="加载动态计算字段",
            description="返回时获取更多字段, 支持采用 prefetch=...&prefetch=... 形式出传入多个",
            examples={"普通字段": "id", "关联字段": "user.id"},
        )
    ...
    def bind(self, router: APIRouter):
        ...
        if self.enable_create:
            ...
            @atomic()
            async def create(
                input: CreateForm = self.get_form_type(CreateForm),
        ...
        if self.enable_get:
            help = "支持采用 condition=...&condition=... 传入多个条件, 使用 JSON 序列化"
            logics = ["and", "or", "not", "not_or"]
            ...
            async def index(
                ...
                condition: List[str] = Query(
                    ...
                    examples={
                        ...
                        "跨表查询": json.fast_dumps(
                            {
                                "user.email": "name@example.com",
                                "user.name.like": "name%",
                            }
                        ),
                        f"条件运算方式, 可选项: {logics}": json.fast_dumps(
                            {
                                ".logic": "or",
                                "name.like": "name%",
                ...
                if condition:
                    for raw in condition:
                        ...
                        condition_q = Q(
                            **{
                                self.model.normalize_field(
                                    k
                                ): safe_condition_value(v)
                                for k, v in kv.items()
                            },
                            join_type=join_type,
                        )
        ...
        if self.enable_update:
            ...
            async def update(
                ...
                for id in ids.split(","):
                    ...
                    id = self.translate_id(current_user, id)
                    obj = await self.q(
                        current_user, self.model.filter(id=id)
                    ).get()
                    raw, m2m_raw = await self.model.save_related(raw)
                    obj.update(raw)
