...
from anyforce.json import fast_dumps as dumps
from anyforce.typing.dataclass import dataclass
...
class Field:
    ...
    def enums(self) -> Tuple[str, List[Any]]:
        if self.type == Type.enum:
            name, choices = self.config.get("name", ""), self.config.get(
                "choices", []
            )
    ...
    def field(self, pk: bool) -> Tuple[str, Dict[str, Any]]:
        ...
        if (
            self.type == Type.str
            or self.type == Type.email
            or self.type == Type.url
        ):
