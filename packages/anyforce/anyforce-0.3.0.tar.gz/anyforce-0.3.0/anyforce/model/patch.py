...
def patch_pydantic(
    from_model: Type[Model], model: Type[PydanticModel], is_form: bool = False
) -> Type[PydanticModel]:
    # 解决数据动态加载的问题
    for k, field in model.__fields__.items():
            ...
            if orig_model:
                if orig_model == from_model:
                    ...
                    field_pydantic_model: Type[Any] = (
                        orig_model.form(from_model)
                        if is_form
                        else orig_model.detail(from_model)
                    )
        ...
        try:
            if field.name == "async_int_field_plus_bigint_field":
                import pdb
