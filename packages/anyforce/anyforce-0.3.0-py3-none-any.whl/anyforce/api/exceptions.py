    ...
    if isinstance(e, exceptions.IntegrityError):
        ...
        for arg in e.args:
            ...
            if arg.find("UNIQUE constraint failed") >= 0:
                ...
                field_name = (
                    field and getattr(field, "description", None) or field_name
                )
                model_name = (
                    model and getattr(model, "title", model.__name__) or "对象"
                )
...
def bind(
    app: FastAPI, e: Type[Exception]
) -> Callable[[Callable[..., R]], Callable[..., R]]:
    return app.exception_handler(e)  # type: ignore


def register(app: FastAPI):
    @bind(app, RequestValidationError)
    ...
    @bind(app, exceptions.BaseORMException)
    async def ormException(
        request: Request, exc: exceptions.BaseORMException
    ) -> ORJSONResponse:
    ...
    @bind(app, EnumMissingError)
    async def enumMissingErrorHandle(
        request: Request, exc: EnumMissingError
    ) -> ORJSONResponse:
        return ORJSONResponse(
    ...
    return (
        requestValidationErrorHandle,
        validationErrorHandle,
        ormException,
        enumMissingErrorHandle,
    )
