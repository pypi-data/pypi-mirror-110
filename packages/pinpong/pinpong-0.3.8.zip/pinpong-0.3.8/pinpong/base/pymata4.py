...
#        time.sleep(0.1)
    ...
    def GD32V_buttonA_is_pressed(self):
        if self._report["buttonA"] == 1:
            return True
        else:
            return False
    
    def GD32V_buttonB_is_pressed(self):
        if self._report["buttonB"] == 1:
            return True
        else:
            return False
...
#            self.dfrobot_dispatch[PrivateConstants.SUB_MESSAGE_UART_READ][bus_num] = ret_value
            ...
            self._report["accelerationX"]=((-1 if data[5] else 1) * ((data[6] & 0x7f) | ((data[7] & 0x7f) << 7)))/1000
            self._report["accelerationY"] = ((-1 if data[8] else 1) * ((data[9] & 0x7f) | ((data[10] & 0x7f) << 7)))/1000
            self._report["accelerationZ"]=((-1 if data[11] else 1) * ((data[12] & 0x7f) | ((data[13] & 0x7f) << 7)))/1000
            self._report["MaccelerationX"]=(-1 if data[14] else 1) * ((data[15] & 0x7f) | ((data[16] & 0x7f) << 7))
            self._report["MaccelerationY"] = (-1 if data[17] else 1) * ((data[18] & 0x7f) | ((data[19] & 0x7f) << 7))
            self._report["MaccelerationZ"]=(-1 if data[20] else 1) * ((data[21] & 0x7f) | ((data[22] & 0x7f) << 7))
