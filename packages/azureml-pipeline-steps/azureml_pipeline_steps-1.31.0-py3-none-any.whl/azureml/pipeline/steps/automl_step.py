...
class AutoMLStep(PipelineStep):
    ...
    def _get_automl_settings(self, context):
        ...
        experiment = Experiment(context._workspace, context.experiment_name)
        ...
        if scenario in constants.Scenarios.VISION_SET:
            # Retrieve curated environment from JOS for vision scenarios
            environment = experiment_state.jasmine_client.get_curated_environment(
                scenario=scenario,
                enable_dnn=experiment_state.automl_settings.enable_dnn,
                enable_gpu=True,
                compute=compute_target.name
            )
            run_config_object.environment = environment
        else:
            # For now we rely on environment utilities to ensure user provided environment has
            # correct dependencies. It would be more correct to use JOS to retrieve CE but
            # VSO: 986826 prevents this right now. Once resolved we can update this. We also
            # should consider requiring user make this call, so they can use the same env for
            # subsequent steps in a single pipeline, and modification from the SDK at submission time
