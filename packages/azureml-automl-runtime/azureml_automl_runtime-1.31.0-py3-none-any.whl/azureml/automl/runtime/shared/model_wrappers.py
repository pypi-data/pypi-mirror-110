...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    # Since we remove nans brought by look-back features in a separate step in training/forecast/model explanation now,
    # the standalone fitted_model.fit() method won't work for regression-based/ensemble models in forecasting.
    # Also, since the featurized data and the raw data has mismatch shapes, fitted_mode.fit() wouldn't work for
    # classical forecasting models which featurizes X internally, with the raw y.
    # If one really want to call fit() function with AutoML featurization, they could feed training data into
    # timeseries transformer, call fit_transform(), and then remove nans brought by look-back features explicitly
    # by calling _remove_nans_from_look_back_features() in timeseries transformer, and give the transformed data
    # to .fit() function of a new regression model instance as a work around. Or, disable look-back features and train
    # classical forecasting models, then fitted_mode.fit() would work, since classical forecasting models don't use the
    # look-back features anyway.
    def fit(self, X: pd.DataFrame, y: np.ndarray) -> None:
        logger.error("The API fit is not supported for a forecast model.")
        raise UserException._with_error(
            AzureMLError.create(
                ForecastFitNotSupported, target="fit",
                reference_code=ReferenceCodes._FORECASTING_FIT_NOT_SUPPORT
            )
        )
