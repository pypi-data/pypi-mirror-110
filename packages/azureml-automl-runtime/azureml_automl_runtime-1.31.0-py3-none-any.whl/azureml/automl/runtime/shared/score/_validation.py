...
def _check_array_values(arr: np.ndarray,
    ...
    if arr.dtype.kind in set('bcfiu'):
        ...
        if validate_type:
            raise ValidationException._with_error(
                AzureMLError.create(
                    AutoMLInternal, target=name,
                    error_details=("{} should have numeric or string type, found type {}. "
                                   "Try converting using {}.astype(str) or {}.astype(int) "
                                   "or {}.astype(float)").format(name, arr.dtype, name, name, name)
                )
            )
