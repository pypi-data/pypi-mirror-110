...
class Source(object):
    ...
    """
    def __init__(self, source_time, span, amplitude=1., name='source'):
        ...
        """ Slightly offset the span such that if the source is zero-size 
...
class ModeSource(Source):
    ...
    def __init__(self, source_time, center, size,
        ...
        if self.norm_ind.size !=1:
            log_and_raise(
                "Exactly one element of ModeSource 'size' must be zero.",
                ValueError
            )
