...
class UNITER_AccuracyMetric(BaseMetric):
    metric_name = 'uniter_accuracy_metric'
...
@DATASET_CONVERTER.register_module()
class UNITER_VCR_DatasetConverter(UNITER_DatasetConverter):

    def __init__(self, post_process_type: str):
        super().__init__(post_process_type=post_process_type)

    def __str__(self):
        return 'uniter_vcr_dataset_converter'
    ...
    def evaluation(self, batch_data, model_outputs, *args, **kwargs):
        ...
        batch_score = {
            'qa_batch_score': model_outputs['qa_batch_score'],
            'qar_batch_score': model_outputs['qar_batch_score'],
            'batch_score': model_outputs['batch_score'],
        }

        return [{'batch_score': batch_score}], [{'batch_size': model_outputs['batch_size']}]


@METRICS.register_module()
class UNITER_VCR_AccuracyMetric(UNITER_AccuracyMetric):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.metric_name = 'uniter {} accuracy'.format(kwargs['name'])
        self.metric_key = kwargs['metric_key']

    def calculate(self, predictions: torch.Tensor, labels: torch.Tensor, **kwargs):
        score = 0.
        datasize = 0
        for preds, bsize in zip(predictions, labels):
            pred = preds['batch_score'][self.metric_key]
            if torch.is_tensor(pred):
                batch_score = pred.item()
            else:
                batch_score = pred
