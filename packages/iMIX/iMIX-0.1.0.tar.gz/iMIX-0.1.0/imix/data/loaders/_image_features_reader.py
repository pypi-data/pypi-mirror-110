...
class ImageFeaturesH5Reader(object):
    ...
    def __init__(self, features_path: str, in_memory: bool = False):
        # self.features_path = bytes(features_path, encoding='utf-8')
            ...
            if self.features[index] is not None:
                ...
                with self.env.begin(write=False) as txn:
                    ...
                    features = np.frombuffer(
                        base64.b64decode(item['features']), dtype=np.float32).reshape(num_boxes, 2048)
                    ...
                    image_location[:, 4] = (image_location[:, 3] -
                                            image_location[:, 1]) * (image_location[:, 2] - image_location[:, 0]) / (
