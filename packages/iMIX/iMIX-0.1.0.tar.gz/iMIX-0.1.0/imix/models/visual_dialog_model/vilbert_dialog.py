...
from ..builder import EMBEDDING, POOLERS, ENCODER, HEADS
...
class BertEmbeddingsDialog(nn.Module):  # BertEmbeddingsDialog
...
class BertImageIntermediate(nn.Module):
    ...
    def __init__(self, config):
        ...
        if isinstance(config.v_hidden_act, str) or (sys.version_info[0] == 2
                                                    and isinstance(config.v_hidden_act, unicode)):
...
class BertConnectionLayer(nn.Module):
    ...
    def forward(self,
                ...
                co_attention_mask=None,
                use_co_attention_mask=False):
        bi_output1, bi_output2, co_attention_probs = self.biattention(input_tensor1, attention_mask1, input_tensor2,
                                                                      attention_mask2, co_attention_mask,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            if count == 0 and self.FAST_MODE:
                ...
                txt_attention_mask = txt_attention_mask.expand(
                    image_embedding.size(0), txt_attention_mask.size(1), txt_attention_mask.size(2),
        ...
        return all_encoder_layers_t, all_encoder_layers_v, (all_attention_mask_t, all_attnetion_mask_v,
                                                            all_attention_mask_c)


@POOLERS.register_module()
class BertTextPooler(nn.Module):  # BertTextPooler
...
@POOLERS.register_module()
class BertImagePooler(nn.Module):  # BertImagePooler
...
class BertLMPredictionHead(nn.Module):
    ...
    def __init__(self, config, bert_model_embedding_weights):
        ...
        # bert_model_embedding_weights = torch.tensor(bert_model_embedding_weights, dtype=torch.float32)
...
class BertPreTrainingHeads(nn.Module):  # BertPreTrainingHeads
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_txt, token_type_ids=token_type_ids, sep_indices=sep_indices, sep_len=sep_len)
...
@EMBEDDING.register_module()
class BertImageEmbeddings(nn.Module):  # BertImageEmbeddings
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    # def forward1(self, input_ids, image_feat, image_loc,
    #              sep_indices=None, sep_len=None, token_type_ids=None,
    #              attention_mask=None, masked_lm_labels=None, next_sentence_label=None, head_mask=None,
    #              random_round_indices=None,
    #              output_nsp_scores=False, output_lm_scores=False, image_attention_mask=None, image_label=None,
    #              image_target=None):
    #
    #     masked_lm_loss = None
    ...
    def forward(self,
        ...
        # else:
        #     return prediction_scores_t, prediction_scores_v, seq_relationship_score, sequence_output_t,
        #            all_attention_mask

        model_output = {}
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            model_output['visual_predict_scores'] = prediction_scores_v
            model_output['image_target'] = image_target
            model_output['text_predict_scores'] = prediction_scores_t.view(-1, self.config.vocab_size)
            model_output['masked_lm_labels'] = masked_lm_labels.view(-1)
            model_output['seq_relationship_scores'] = seq_relationship_score.view(-1, 2)
            model_output['next_sentence_label'] = next_sentence_label.view(-1)
        else:
            model_output['text_predict_scores'] = prediction_scores_t
            model_output['visual_predict_scores'] = prediction_scores_v
            model_output['seq_relationship_scores'] = seq_relationship_score
...
class VILBertForVLTasks(BertPreTrainedModel):
    ...
    def forward(
        ...
        return (vil_prediction, vil_logit, vil_binary_prediction, vision_prediction, vision_logit, linguisic_prediction,
                linguisic_logit)
