...
class Visual7wPointingDataset(Dataset):
    ...
    def __init__(
        ...
        if 'roberta' in bert_model:
            ...
            cache_path = os.path.join(
                dataroot,
                'cache',
                task + '_' + split + '_' + str(max_seq_length) + '_' + str(max_region_num) + clean_train + '.pkl',
            )
    ...
    def __getitem__(self, index):
        ...
        gt_features, gt_num_boxes, gt_boxes, gt_boxes_ori = self._gt_image_features_reader['v7w_' + str(image_id)]
        ...
        target = iou(torch.tensor(mix_boxes_ori[:, :4]).float(), torch.tensor([ref_box]).float())
        target[target < 0.5] = 0
