...
def s3_upload_file(bucketName, key, localFilePath, contentType=' ',tagging = ' '):
    ...
    if tagging == ' ':
        s3_resource.Object(bucketName, key).upload_file(Filename=localFilePath, ExtraArgs={'ContentType':'image/png'})
    else:
        s3_resource.Object(bucketName, key).upload_file(Filename=localFilePath, ExtraArgs={'ContentType':'image/png','Tagging': urlencode(tagging)})
    return "Success"
