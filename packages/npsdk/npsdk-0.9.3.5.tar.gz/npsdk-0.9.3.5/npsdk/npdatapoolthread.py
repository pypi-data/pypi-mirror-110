...
class NpDataPoolThread(threading.Thread):
    ...
    def __init__(self, lock, playmode, nezip_exe_path = ".\\网际风\\"):
        ...
        self.npdatapoolthread_stopevent = threading.Event()
        self.npdatapoolthread_stopevent.clear()
    ...
    def send_and_receive(self):
        ...
        while not self.npdatapoolthread_stopevent.is_set():

            try:
                """
                if self.NpWsClient.m_connected and self.NpWsClient.m_ws:

                    bret, message = self.NpWsClient.receive_wsdata()  #无数据阻塞在这里
                    if bret:
                ...
                #logger.info('npdatapool thread:%s'%time.time())

                if self.NpWsClient.m_connected and self.NpWsClient.m_ws:
                    
                    self.NpWsClient.send_command_pro() #定时发送业务命令
                    ...
                    """
                    events = self.NpWsClient.npsel.select(0.005) 
                ...
                logprtstr = 'send_and_receive() exception: %r'% e
                print(logprtstr); logger.warning(logprtstr)
    ...
    def stop_me(self):
        ...
        self.npdatapoolthread_stopevent.set() #停止线程while循环
        ...
        try:
            ...
            if msgtype == '代码表':
                ...
                for i in range(marketbody.num):
                    ...
                    #logger.info('code:%s market:%s label: %s name:%s'%(stkbody.code, stkbody.market, stkbody.label, stkbody.name))
