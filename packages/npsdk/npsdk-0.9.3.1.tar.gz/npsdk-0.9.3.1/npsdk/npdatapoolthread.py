...
class NpWebsocketClient(object): 
    def __init__(self, parent, playmode = NPSDKAPI_PLAYMODE_NPWEBSOCKET, nezip_exe_path = ".\\网际风\\"):
        ...
        self._wscommands_set = set()
        self._wscommands_dict = {}
    ...
    def connect(self):
        try:
            ...
            if self.m_ws:
                ...
                """               
                self.npsel.register(self.m_ws, selectors.EVENT_READ, self.on_read)
    ...
    注册业务命令
    """
    def register_wscommand(self, wscommand):

        self._wscommands_set.add(wscommand) # 注册websocket commands命令用set集合去除重复命令
        
    """
    给不同的业务命令分配定时器阀值，起始时间戳，并且去除重复命令
    """
    def parse_wscommand_pro(self, command):

        if command in self._wscommands_dict.keys():
            return # 重复命令不处理
        ...
        if type == NPSDKAPI_NEZIP_QUOTE:
            self._wscommands_dict[command] = [NPSDKAPI_TIMER_1SECS, time.time()] 

        # 代码=SH600000&类型=1分钟线&数量=2000
        elif type in NPSDKAPI_NEZIP_KTYPES:
                self._wscommands_dict[command] = [NPSDKAPI_TIMER_3SECS, time.time()]
    ...
    扫描业务命令定时器阀值
    """
    def scan_wscommand_pro(self):

        for cmd in self._wscommands_set:
            self.parse_wscommand_pro(cmd)

        for cmd in self._wscommands_dict.keys():

            #delta = time.time() - self._wscommands_dict[cmd][1]
            #tim = self._wscommands_dict[cmd][0]

            if time.time() - self._wscommands_dict[cmd][1] >= self._wscommands_dict[cmd][0]: #超时
                self.send_command_over_ws(cmd)
                logger.info('send ws command: %s' % cmd)

                self._wscommands_dict[cmd][1] = time.time() # 重新设置起始时间戳

        #print('>>>', self._wscommands_dict)
...
class NpDataPoolThread(threading.Thread):
    ...
    def __init__(self, playmode, nezip_exe_path = ".\\网际风\\"):
        ...
        self.NpWsClient = NpWebsocketClient(self, self._playmode, nezip_exe_path)
    ...
    def start_me(self):
        ...
        1. 循环调用 receive_data_over_ws() 读取数据
        2. select EVENT_READ 触发读取
        方法2 更有效。大吞吐量数据，二者CPU占有率应该差不多。数据量小或者无数据时，方法2更高效。
        """
        ...
        try:
            while True:
                ...
                if self.NpWsClient.m_connected and self.NpWsClient.m_ws:

                    self.NpWsClient.scan_wscommand_pro() #定时发送业务命令
                    ...
                    """
                    events = self.NpWsClient.npsel.select(0.01) 
            ...
            logprtstr = 'start_me() exception: %r'% e
            print(logprtstr); logger.warning(logprtstr)
