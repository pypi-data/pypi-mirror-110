...
class NpSdkApi(object):
    ...
    def run_datapool_thread(self):
        ...
        # 创建数据池线程
        self._npdatapoolthread = NpDataPoolThread(self._npsdk_playmode, self._nezip_exe_path)
    ...
    """
    def fetch_quote_remote(self, symbol, wscommand):
        """ 
        在数据池里查找实时行情业务数据，
        """         
        quote = OEM_REPORT()
        ...
        """
        self._npdatapoolthread.NpWsClient.register_wscommand(wscommand)
    ...
    """
    def fetch_klines_remote(self, symbol, klinetype, wscommand):
        """ 
        在数据池里查找K线业务数据，
        """         
    ...
    def fetch_quote(self, symbol: str) -> OEM_REPORT:
        ...
        #t = time.perf_counter()
        bsuc, quote = self.fetch_quote_remote(symbol, wscommand)
    ...
    def fetch_klines(self, symbol: str, kline_type: str = "1分钟线", data_length: int = 1000) -> pd.DataFrame:
        ...
        bsuc, df_kline = self.fetch_klines_remote(symbol, kline_type, wscommand)
        """
        #t = time.perf_counter()
        if bsuc:
