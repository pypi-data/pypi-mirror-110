...
api = NpSdkApi(debug = True, playmode = 'backtestwebsocket')
print("策略开始运行")
...
while True:
    ...
    if api.is_updated(quote1, ['open', 'high', 'low', 'close']): # 判断实时行情变化
        print("%s 开: %.2f 高: %.2f 低: %.2f 收: %.2f"%(quote1.name, quote1.open, quote1.high, quote1.low, quote1.close))

    if api.is_updated(quote2, 'close'): # 判断实时行情变化
        print("%s 收: %.2f"%(quote2.name,  quote2.close))

    if api.is_updated(quote3, ['volume','amount']): # 判断实时行情变化
