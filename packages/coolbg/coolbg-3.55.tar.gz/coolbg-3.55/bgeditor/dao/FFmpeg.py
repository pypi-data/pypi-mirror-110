...
def add_null_sound(input):
    output=  get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) + '-' + os.path.basename(input)
...
def merge_list_video(vids, is_del=True):
    file_merg_path = get_dir('coolbg_ffmpeg') + str(uuid.uuid4())
    final_clip_path = get_dir('results') + str(uuid.uuid4()) + '-final-' + os.path.basename(vids[0])
    file_merg = open(file_merg_path, "a")
    arrtmp = []
    for vid in vids:
        tmp_clip_path = get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) + '-' + os.path.basename(vid)
...
def create_video_audio(clip_path, audio_path, rs_path=None, is_del=True):
    try:
        ...
        if tmp_clip_path_z is None:
            tmp_clip_path_z = get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) + '-final.mp4'
...
def convert_gif(video_path, is_del=True):
    tmp_clip_path_z = get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) + '-gif.avi'
    cmd=f"ffmpeg -i {video_path}  -r 24 -c:v ffv1 {tmp_clip_path_z}"
    os.system(cmd)
    if is_del:
