...
class DataTransfer:
    ...
    def upload_local_file(
        upload_config, aws_credentials: AwsCredentials, local_file: str, file_suffix: str
    ) -> None:
        key = f"{upload_config.key_prefix}{file_suffix}"
        S3.client_from_credentials(aws_credentials).upload_file(
            Bucket=upload_config.bucket_region(), Key=key, Filename=local_file, ExtraArgs=_ACL
        )
