...
from .. import Connection, encode, decode, AwsCredentialsFetcher


def call_pandas_function(
    function_type: str, function_prefix: Optional[str], function_name: str, *args, **kwargs
) -> Any:
    fetcher = AwsCredentialsFetcher()
    args = [] if args is None else args
    args_encoded = apply_func_on_object_recursively(args, partial(encode, fetcher))
    kwargs = {} if kwargs is None else kwargs
    kwargs_encoded = apply_func_on_object_recursively(kwargs, partial(encode, fetcher))
    ...
    response = Connection.poll_for_answer("compute", job)
