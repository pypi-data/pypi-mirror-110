...
class CameraDisplay(QWidget):
    ...
    def force_resize(self) -> None:
        ...
        # Resize the image
        self.label.setPixmap(self.label.pixmap().scaled(self.sizeHint(),
                                                        Qt.KeepAspectRatio,
...
class CameraSettingsWidget(QWidget):
    ...
    def __init__(self, parent: VideoWidget = None, popup: bool = True):
        ...
        self.config_label.setSizePolicy(QSizePolicy.Maximum,
                                        QSizePolicy.Maximum)
    ...
    def set_config(self, name: str) -> None:
        ...
        if not self._saved and self._previous_config:
            confirm = QMessageBox.question(self, "Confirm Changing Configuration",
    ...
    def save_config(self) -> None:
        ...
        name, ok = QInputDialog.getText(self, "Save Configuration",
                                        ...
                                        QLineEdit.Normal, self.current_config)
        ...
        # Make sure the entered config name is unique
            ...
            confirm = QMessageBox.question(self, "Confirm Configuration Name",
    ...
    class CameraSettingWidget(QWidget):
        def __init__(self, info: dict, parent: "CameraSettingsWidget"):
            ...
            if min_val is not None and max_val is not None:
                ...
                decimals = max(len(str(min_val).split(".")[-1]),
                               len(str(max_val).split(".")[-1]))
                ...
                self.widget.setToolTip(f"minimum = {min_val:,g}, "
                                       f"maximum = {max_val:,g}")
            ...
            if isinstance(self.widget, QSlider):
                self.label = SliderLabel(self.widget, name=self.title, 
                                         unit=self.unit, precision=2)
            else:
                self.label = QLabel()
            ...
            # Set size policy
            self.widget.setSizePolicy(QSizePolicy.MinimumExpanding,
                                      QSizePolicy.Maximum)
...
class CameraStatusBar(QStatusBar):
    ...
    @property
    def fps(self) -> float:
        return getattr(self.camera, "real_fps", 0.)
