...
class LinePlotWidget(PlotWidget):
    def __init__(
        ...
        [w.setAlignment(Qt.AlignRight | Qt.AlignVCenter) for w in 
         (self.fft_min_label, self.fft_max_label)]
...
class LineScanPlotWidget(PlotWidget):
    ...
    def set_image(self, image: np.ndarray) -> None:
        ...
        h, w = image.shape[:2]
        self.plot_item.setXRange(0, w*1.05, padding=0)  # default between 0.02 and 0.1 if not specified
...
class PlotGridWidget(QWidget):
    ...
    def __init__(self, parent = None, title: Optional[str] = None, popup: bool = True):
        ...
        # Create plot widgets
        self.region_plot = LinePlotWidget(parent=self, popup=False, 
                                          title="Region Intensity", show_menubar=False)
        self.region_fft_plot = FFTPlotWidget(parent=self.region_plot, popup=False, 
                                             title="Region Intensity FFT", show_menubar=False)
        self.growth_rate_plot = GrowthRatePlotWidget(parent=self.region_fft_plot, popup=False, 
                                                     title="Growth Rate", show_menubar=False)
        self.profile_plot = LineProfileWidget(parent=self, popup=False, 
                                              title="1D Line Profile", show_menubar=False)
        # self.profile_fft_plot = FFTPlotWidget(parent=self.profile_plot, popup=False,
        #                                       title="Line Profile FFT", show_menubar=False)
        self.line_scan_plot = LineScanPlotWidget(parent=self, popup=False, 
                                                 title="2D Line Scan", show_menubar=False)
        self.plot_widgets = [
            self.region_plot,
