...
class Jedis:
    ...
    # INFO: keys - Return a list of keys matching 'pattern'
    def keys(self, pattern="*"):
        ...
        path = self._path
        fdata = load(path)
        list_keys = list(fdata.keys())
        if pattern == "*":
            return list_keys
        else:
            r = re.compile(pattern)
            newlist = list(filter(r.match, list_keys))
            return newlist
