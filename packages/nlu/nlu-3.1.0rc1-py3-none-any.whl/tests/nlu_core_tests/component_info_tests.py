...
class TestComponentInfo(unittest.TestCase):
    ...
    def test_print_all_default_components_as_markdown(self):
        print(pd.DataFrame(nlu.Spellbook.component_alias_references).T.to_markdown())
    ...
    def test_quick(self):
        import nlu
        p = nlu.load(path='/home/ckl/Documents/freelance/jsl/nlu/nlu4realgit2/tmp/model_test_tests/norm/model',verbose=True)
        df = p.predict("I love peanut butter and jelly")
        for c in df.columns : print(df[c])
        p = nlu.load(path='/home/ckl/Documents/freelance/jsl/nlu/nlu4realgit2/tmp/model_test_tests/udep',verbose=True)
        df = p.predict("I love peanut butter and jelly")
        for c in df.columns : print(df[c])

        #
        # p = nlu.load(path='/home/ckl/Documents/freelance/jsl/nlu/nlu4realgit2/tmp/model_test_tests/norm/modelsent',verbose=True)
        # df = p.predict("I love peanut butter and jelly")
        # for c in df.columns : print(df[c])
