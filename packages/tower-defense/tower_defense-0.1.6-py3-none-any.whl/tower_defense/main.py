...
def draw():
    ...
    if info.placing != '':
        ...
        if 0 <= mx <= 800 and 0 <= my <= 450:
            ...
            original = pygame.transform.scale(pygame.image.load('range.png'), (classObj.range * 2, classObj.range * 2))
    ...
    if info.selected is not None:
        original = pygame.transform.scale(pygame.image.load('range.png'), (info.selected.range * 2, info.selected.range * 2))
        modified = original.copy()
        modified.fill((255, 255, 255, 128), None, pygame.BLEND_RGBA_MULT)
...
def app():
    ...
    IceCircle = pygame.transform.scale(pygame.image.load('ice_circle.png'), (250, 250))
    smallIceCircle = IceCircle.copy()
    smallIceCircle.fill((255, 255, 255, 128), None, pygame.BLEND_RGBA_MULT)

    IceCircle = pygame.transform.scale(pygame.image.load('ice_circle.png'), (350, 350))
