...
class Api(object):
    ...
    def stop_simulation(self, adjust_ms: float):
        response = requests.post(self.host.url('api/simulation/stop'), json=dict(
            adjust_ms=str(adjust_ms)
        ))
    ...
    def start_scenario(self, name, modifiers, meta, options):
