...
class ReportOne:
    ...
    def for_table(self) -> dict:
        ...
        return {
            ...
            'PFCF': utils.to_float(utils.ext_latest_value(self.eval_dict['mil']['가치지표']['PFCF'])),
            'PCR': utils.to_float(utils.ext_latest_value(self.eval_dict['mil']['가치지표']['PCR'])),
        }
...
def eval_spac():
    """
    전체 스팩주를 평가하여 가격이 2000원 이하인 경우 노티한다.
    :return:
    """
    codes = CorpsEval.get_all_codes_in_db()
    ...
    for code in codes:
        ...
        if '스팩' in str(name):
            ...
            if utils.to_int(rc101['주가']) <= 2000:
                ...
                text = str(f"<<< code : {code} name : {name}  price : {rc101['주가']}>>>")
                noti.telegram_to(botname='eval', text=text)
