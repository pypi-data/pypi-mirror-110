...
def get_project_folder(env_path: str = ".env") -> str:
    """Obtains the project directory usinf the PROJECT_DIR value
    from the supplied .env file."""
...
def get_misty(env_path: str = ".env") -> Callable:
    """Obtains a Misty instance using the MISTY_IP_ADDRESS in the supplied
    .env file."""
...
def get_files_in_dir(abs_dir: str) -> List[str]:
    """Lists files in the supplied directory specified with absolute path."""
    return [
...
def get_base_fname_without_ext(fname: str) -> str:
    """Returns the file name without the extension (or extensions if multiple)
...
def query_dict_with_fallback(
    ...
    """Safely queries a dictionary, returns the queried value if it passes
    type check and the fallback otherwise.

    Args:
        data (Dict): the dictionary to query.
        query (str): the query.
        fallback (Any): the fallback value to return if the key does not exist
        or the queried value does not pass the type check.
        required_type (Type, optional): The required type of the queried
        value. None if there is no required type. Defaults to None.
