...
def parse_rgb_value(value: Union[int, str, None]) -> int:
    ...
    Raises:
        ...
        ValueError: if the value is int or str parseable to int but is not
        a valid rgb value.
    ...
    if not (value >= 0 and value <= 255):
        raise ValueError("An rgb value must be \
...
def rgb_values_to_dict(
    ...
    """Returns rgb dictionary in the form required by Misty API from rgb
    values supplied as str or int."""
    return {
...
def validate_rgb_dict(potential_rgb: Dict) -> Dict:
    """Validates a potentially rgb dictionary. Returns the rgb dictionary if
    successful."""
    return {
...
def get_rgb_from_unknown(potential_rgb: Any, allowed_data: Dict = {}) -> Dict:
    """Attempts to parse an rgb dictionary as required by Misty API
    from potential_rgb.

    Args:
        potential_rgb (Any): any data that might be an rgb dict.
        allowed_data (Dict, optional): the dictionary of allowed data
        shortcuts (string keys, dict values). Defaults to {}.

    Raises:
        TypeError: if the type of potential_rgb is not dict or parseable
        to dict.

    Returns:
        Dict: rgb dictionary as required by Misty API.
    """
    ...
    if isinstance(potential_rgb, Dict):
        ...
        raise TypeError(
            "Incorrect type: `%s`, expecting Dict." % type(potential_rgb))
...
def construct_transition_dict(
    ...
    """Constructs a transition dictionary in the form required by Misty API.

    Args:
        data (Dict): the dictionary to transform, must have keys col1 and col2
        that contain a string (data shortcut for a colour) or a dict (rgb
        dictionary) and should have keys transition and time.
        allowed_data (Dict): the dictionary of allowed data shortcuts (string
        keys, dict values).
        fallback_time (int, optional): The default time between colours
        switching in ms. Defaults to 500.
        fallback_trans (str, optional): The default transition type, can be
        "Blink", "Breathe" or "TransitOnce". Defaults to "Breathe".

    Returns:
        Dict: The input dictionary in the form required by Misty API's
        endpoint /api/led/transition.
    """
