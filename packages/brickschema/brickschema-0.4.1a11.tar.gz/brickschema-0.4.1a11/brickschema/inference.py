...
for Allegro with 'pip install brickschema[allegro]"
    ...
    def expand(self, graph):
        ...
        def check_error(res):
            ...
            if exit_code > 0:
                raise Exception(f"Non-zero exit code {exit_code} with message {message}")

        logger.info("setup inputs to docker + connection")
        # setup connection to docker
        tar = self._setup_input(graph)
        # TODO: temporary name so we can have more than one running?
        logger.info("run agraph container")
        agraph = self._client.containers.run(
            "franzinc/agraph:v7.1.0", name=self._container_name, detach=True, shm_size="1G"
        )
        logger.info("should be started; copying input to container")
        ...
        while exit_code > 0:
            time.sleep(1)
            exit_code, _ = agraph.exec_run("ls /agraph/etc/agraph.cfg")
        print(agraph.exec_run("ls /agraph/"))
        print(agraph.exec_run("ls /agraph/etc/"))
        logger.info("starting agraph server")
        check_error(
            agraph.exec_run(
...
/tmp/input.ttl",
        ...
        logger.info("materializing input file")
        check_error(
            agraph.exec_run(
...
--rule all",
        ...
        logger.info("exporting reasoned graph")
        check_error(
            agraph.exec_run(
 ...
 /tmp/output.ttl",
        ...
        logger.info("stopping container + removing")
        agraph.stop()
