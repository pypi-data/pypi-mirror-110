...
def validate_headers(headers):
    return headers is not None and all({k is not None and headers[k] is not None and k.strip() == headers[k].strip(): headers[k] for k in headers})
...
class Subtask:
    ...
    def __init__(self, name: str, max_points: float, is_bonus: bool):
        self.name = name
...
class Ep2Group:
    ...
    def exercise_test_csv(self, ue):
        ...
        if ue_type(ue) == ExerciseType.Normal:
            ...
            return os.path.join(self.ep2.tutor_repo(), self.name, "ex_test_ta.csv")
    ...
    def pre_eval_csv(self, ue: int):
        if ue_type(ue) == ExerciseType.Team:
            return os.path.join(self.ep2.tutor_repo(), self.name, "ta.csv")
        return os.path.join(self.ep2.tutor_repo(), self.name, "exercise_%d.csv" % ue)
...
class Ep2:
    ...
    def __init__(self, verbose: bool = False, config: configparser.ConfigParser = None, sudo: str = None):
        ...
        version, details = get_module_version(ep2_core.common)

        if verbose:
            click.secho("Starting tool version {}".format(version), fg='bright_black')
            click.secho("Fetching latest release version from pypi", fg='bright_black')

        online_version = self.fetch_pypi_version()
        local_version = VersionInfo.parse(version)

        if online_version.is_ok():  # value is version
            if online_version.value > local_version:
                click.secho('You are using an outdated version ({}) of ep2-tutor-tools'.format(local_version), fg='yellow')
                click.secho('Some commands might not work as expected!'.format(local_version), fg='yellow')
                click.secho('Please update the tools by running:', fg='yellow')
                click.echo()
                click.secho('    pip install -U --user ep2-tutor-scripts=={}'.format(online_version.value), fg='yellow', bold=True)
                click.echo()
            elif verbose:
                click.secho('Tools are up to date', fg='bright_black')
        else:   # value is str
            click.secho("Could not fetch latest version from pypi: {}".format(online_version.value), fg='red', bold=True)

    def fetch_pypi_version(self) -> Result[VersionInfo, str]:
        import requests

        response = requests.get('https://pypi.org/pypi/ep2-tutor-scripts/json')
        if response.status_code != 200:
            return Err('HTTP error: {}'.format(response.status_code))
        ...
        try:
            ...
            return Err('JSON Decode Error: {}'.format(e))
        except Exception as e:
            return Err('Other error: {}'.format(e))
    ...
    def git_host(self) -> str:
        ...
        url = urlparse(self.config['Gitlab']['url'])
        return url.hostname
    ...
    def clone_or_update(self, repo_dir, project, tag) -> Result:
        ...
        try:
            ...
            return Err('%s failed with status code: %d (stdout: %s) (stderr: %s)' % (error.command, error.status,
    ...
    def ex_test_repo_name(self, mat_no: str, ue: int):
        if ex_test_type(ue) == ExTestType.Single:
            return '{}u{}'.format(mat_no, ue)
        else:
            return mat_no

    def local_ex_test_repo(self, mat_no: str, ue: int):
        """Returns the local path of students uebungs repo"""
        return os.path.join(self.config.get('Local', 'GitHome'), 'uebung', self.ex_test_repo_name(mat_no, ue))
    ...
    def team_repo(self, path):
        return os.path.join(self.config.get("Gitlab", "RepoPrefix"), 'uebung', path).replace("\\", "/")
    ...
    def local_ta_repository(self, team: str):
        return os.path.join(self.uebung_dir(), team.lower())

    def local_team_repository(self, team: str):
        return os.path.join(self.uebung_dir(), team)

    def create_ex_test_symlink(self, project: str, team: str):
        local_team = self.local_team_repository(team)
        local_repo = self.local_repo(project)

        if os.path.exists(local_team):
            try:
                os.unlink(local_team)
            except IsADirectoryError:
                try:
                    os.rmdir(local_team)
                except OSError:
                    shutil.rmtree(local_team)

        try:
            os.symlink(local_repo, local_team)
        except OSError as e:
            click.echo(
                'User has no rights to create symlinks! See https://stackoverflow.com/a/65504258 to set proper '
                'privileges under Windows 10.')
    ...
    def get_exercise_subtasks(self, ex: int) -> [Subtask]:
        ...
        with open(csv_file, 'r') as infile:
            reader = csv.DictReader(infile, ['subtask', 'max_points', 'bonus'], KEY_INVALID, strict=True)
            ...
            subtasks: [Subtask] = []
            for row in reader:
                subtasks += [Subtask(row['subtask'], float(row['max_points']), row['bonus'] == 'true')]
    ...
    def exercise_test_repo(self, ex: int, student_id: str) -> Optional[str]:
        return os.path.join(self.config.get("Gitlab", "RepoPrefix"), 'uebung', self.ex_test_repo_name(student_id, ex)).replace("\\", "/")

    def grant_access(self, project_path: str, student_id: str) -> Result[None, str]:
        """ Grants maintainer access for `student_id` to the project at `project_path`.

        Maintainer access level is necessary to be able to push to `master`.

        :param project_path: the path to the project (e.g. `<course>/<term>/uebung/<student>
        :param student_id: the student id of the student
        :return: `Ok(None)` if the operation succeeds, `Err(error_msg)` if an error occurs
        """

        try:
            project = self.gitlab.projects.get(project_path.replace("/", "%2F"))
            user = self.gitlab.users.list(username=student_id)[0]

            members = project.members.list()    # lists the directly assigned members (should not need another request)
            if any([member.id == user.id for member in members]):   # member already exists
                member = project.members.get(user.id)
                if member.access_level < gitlab.MAINTAINER_ACCESS:
                    click.secho('setting access level of {} to maintainer for project {}'
                                .format(student_id, project_path), fg='bright_black')
            ...
            return Err('granting {} maintainer access to {} failed: {}'.format(student_id, project_path, e))


def verify_and_normalize_student_id(student_id) -> Result:
    import re
    if re.match('\\d*', student_id) and len(student_id) <= 8 and student_id != '':
