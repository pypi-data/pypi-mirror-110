...
def get_traffic_source(df: DataFrame, user_id_column: str = "player_id") -> DataFrame:
    ...
    projects = ["mk5", "mk1", "mk2", "mk3", "vipclub", "vipclub2", "vipclub", "lara", "vipt", "pobeda"]
...
def get_rates(df, dates_field="date", currencies_field="currency", values_field="sum"):
    unique_dates = set(df[dates_field].dt.date)
    currencies = df[currencies_field].unique()
    projects = ["pobeda", "lara", "vipt", "mk5", "vipclub"]
    rates = MongoDB(random.choice(projects)).query(
    ...
    return merged_rates.assign(usd_sum = lambda x: x[values_field]/x["rate"])