...
class Combobox(ttk.Combobox):
    def __init__(
        ...
        :param validate: The function to use for validation. Will be passed in
         a positional argument with the text as
         a str (like `validate(contents)`) and should return a bool - True if
        ...
        if not isinstance(show, str) and show is not None:
            raise TypeError(
                f"show is not a str! " f"(type passed in: {repr(type(show))})"
            )
        ...
        super().__init__(
            ...
            show=show,
            validatecommand=(parent.register(validate), "%p")
    ...
    def enabled(self) -> bool:
        """
        Get whether this combobox is in normal mode or disabled mode. (grayed
    ...
    def enabled(self, new_state: bool) -> None:
        """
        Set whether this combobox is in normal mode or disabled mode. (grayed
        out and cannot interact with)
    ...
    def read_only(self) -> bool:
        """
        Get whether this combobox is read only mode. (cannot type in selection
        manually - must select from pre-defined values)
    ...
    def read_only(self, new_state: bool) -> None:
        """
        Set whether this combobox is in read only mode (cannot type in
