...
class BoundingBox():
    ...
    def __init__(self, left=None, top=None, right=None, bottom=None, **kwargs):
        ...
        self._width = self.right - self.right
    ...
    def contains(self, bbox):
        if self.area==0 or bbox.area==0:
            return self.top<=bbox.top and self.left<=bbox.left and self.right>=bbox.right and self.bottom>=bbox.bottom
        else:
            return rectangle_a_inside_b((self.left, self.top, self.right, self.bottom), (bbox.left, bbox.top, bbox.right, bbox.bottom))
    ...
    def __eq__(self, other):
        if other is None:
            return False
        return int(self.top) == int(other.top) and int(self.left) == int(other.left) and int(self.right) == int(
            other.right) and int(self.bottom) == int(other.bottom)

    def __hash__(self):
        return hash(('top', int(self.top),'left', int(self.left), 'right', int(self.right),'bottom', int(self.bottom)))
...
def are_in_same_line2(word1, word2, tolerance='auto'):
    if tolerance == 'auto':
        tolerance = max((word1.bottom - word1.top) / 3, (word2.bottom - word2.top) / 3)
...
def join_collated_h_edges(edges):
    ...
    for i in range(1, len(returned_edges)):
        if not np.isclose(edges2[-1].top, returned_edges[i].top, atol=4) or horizontal_overlap(edges2[-1],
                                                                                               returned_edges[i]) == 0:
...
def get_type(value, numerical_equivalent=['-', '_']):
    if isinstance(value, int) or isinstance(value, float):
        return "Numeric"

    if value.strip() in numerical_equivalent:
        return "Numeric"
...
def is_touching_with_bbox(line, bbox, tolerance=1):
    ...
    if line.top==line.bottom: #horizontal line
        return isclose(line.top, bbox.top, abs_tol=1) or isclose(line.top, bbox.bottom, abs_tol=1)
    elif line.left==line.right: #vertical line
        return isclose(line.left, bbox.left, abs_tol=1) or isclose(line.left, bbox.right, abs_tol=1)
    else:
...
def get_widthin_bbox(objs, bbox):
    ...
    if isinstance(objs, dict):
        ...
        sub_obj = [o for o in objs if get_overlap_with_bbox(
            BoundingBox(left=o["left"], top=o["top"], right=o["right"], bottom=o["bottom"]), bbox)]
        return sub_obj
...
def cluster_list2(xs, rel_tol=1e-04, abs_tol=0.0):
    ...
    last = list(xs)[0]
    nb_values = len(last)
    items = tuple(range(nb_values))
    itemg = 'itemgetter' + str(items)
    xs = sorted(xs, key=eval(itemg))
    current_group = [xs[0]]
    last = xs[0]

    for x in xs[1:]:
        truth = [isclose(x[i], last[i], rel_tol=rel_tol, abs_tol=abs_tol) for i in range(0, nb_values)]
        if all(truth):
            current_group.append(x)
        else:
            groups.append(current_group)
...
def make_cluster2(values, rel_tol=1e-04, abs_tol=0.0):
    clusters = cluster_list2(set(values), rel_tol, abs_tol)
...
def cluster_objects2(objs, attrs, rel_tol=1e-04, abs_tol=0.0, min_cluster_size=0):
    if not objs:
        return None

    objs = list(objs)

    if isinstance(attrs[0], int):
        values = [tuple([o[i] for i in attrs]) for o in objs]
    else:
        values = list(map(lambda p: tuple((getattr(p, attr) for attr in attrs)), objs))

    cluster_dict = make_cluster2(values, rel_tol, abs_tol)

    get_0, get_1 = itemgetter(0), itemgetter(1)

    if isinstance(attrs[0], int):
        cluster_tuples = sorted(
            ((obj, cluster_dict.get(tuple([obj[i] for i in attrs]))) for obj in objs), key=get_1
        )
    else:
        cluster_tuples = sorted(
            ((obj, cluster_dict.get(tuple(getattr(obj, attr) for attr in attrs))) for obj in objs), key=get_1
        )
...
def are_equal(bbox1, bbox2):
    return int(bbox1.top) == int(bbox2.top) and int(bbox1.left) == int(bbox2.left) and int(bbox1.bottom) == int(
        bbox2.bottom) and int(bbox1.right) == int(bbox2.right)
...
def words_to_left_edges(words, word_threshold=DEFAULT_MIN_WORDS_VERTICAL):
    ...
    edges = [
                BoundingBox(left=min(w.left for w in words), right=min(w.left for w in words),
                            top=min(w.top for w in words),
                            bottom=max(w.bottom for w in words)) for words in by_left2] + [
                BoundingBox(left=max(w.right for w in words), right=max(w.right for w in words),
                            top=min(w.top for w in words),
...
def vertical_overlap_with_list(a, list_b,tolerance=2):
    overlaps=[vertical_overlap(a, b) for b in list_b if b!=a]
    if overlaps:
        return max(overlaps) > tolerance
    else:
        return False

def horizontal_overlap_with_list(a, list_b,tolerance=2):
    overlaps=[horizontal_overlap(a, b) for b in list_b if b!=a]
    if overlaps:
        return max(overlaps) > tolerance
    else:
        return False

def edge_intersect_text(texts, edge, tollerance=2):
    return [t for t in texts if t.bottom-tollerance > edge.bottom and t.top+tollerance<edge.top and t.left>edge.left and t.right<edge.right]
...
def find_line_structure(tokens):
    ...
    i = 0
    tokens=sorted(tokens, key=lambda x:(x.bottom, x.left))
    while i < len(tokens):
