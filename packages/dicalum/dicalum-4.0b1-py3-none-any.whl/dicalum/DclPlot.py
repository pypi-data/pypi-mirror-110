...
def DclPlot(klog,amin,amax):
    ...
    if (klog==1):
        psm = plt.imshow(GG, cmap=dclcmap, interpolation='none', rasterized=True, norm=LogNorm(vmin=amin, vmax=amax))
    else:
        psm = plt.imshow(GG, cmap=dclcmap, interpolation='none', rasterized=True, vmin=amin, vmax=amax)
    plt.axis('off')
