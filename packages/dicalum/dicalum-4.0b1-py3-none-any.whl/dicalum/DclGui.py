...
class DclFr01(ttk.LabelFrame):
    ...
    def __create_widgets(self):
        ...
        #ttk.Button(self, text="Plot", command=partial(DclPlot,0) ).grid(column=1, row=2)
        #ttk.Button(self, text="LogPlot", command=partial(DclPlot,1) ).grid(column=2, row=2)
        ttk.Button(self, text="dsu Plot", command=lambda: DclPlot(0,float(self.min.get()),float(self.max.get())) ).grid(column=2, row=2)
        ttk.Button(self, text="dsu LogPlot", command=lambda: DclPlot(1,float(self.min.get()),float(self.max.get())) ).grid(column=3, row=2)
        ttk.Button(self, text="MPSAS Plot", command=lambda: DclPlot(2,float(self.min.get()),float(self.max.get())) ).grid(column=4, row=2)
        ttk.Label(self, text="min/max:").grid(column=2, row=1)
        self.max_w = ttk.Entry(self, textvariable=self.min, width=5).grid(row=1, column=3)
        self.max_w = ttk.Entry(self, textvariable=self.max, width=5).grid(row=1, column=4)
...
class DclFr11(ttk.LabelFrame):
    ...
    def __create_widgets(self):
        global f1update
        button = ttk.Button(self, command = DclPlot)
