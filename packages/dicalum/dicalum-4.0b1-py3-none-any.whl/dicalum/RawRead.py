...
def rawread(rfile):
    kam=dclinst.camera
    len=dclinst.lens
    if (pathlib.Path(rfile).suffix.lower() != CamList[kam].exte):
    ...
    R=(R-dark)/(satu-dark)*CamList[kam].rdsu
    G=(G-dark)/(satu-dark)*CamList[kam].gdsu
    B=(B-dark)/(satu-dark)*CamList[kam].bdsu
    R[R < 0.0] = 0.0
    G[G < 0.0] = 0.0
    B[B < 0.0] = 0.0
    V = dcldat.V
    ...
    fRGB = (np.sqrt(np.dstack((R,G,B))) * xss)
    fRGB[fRGB > 255] = 255
    RGB =  np.uint8(fRGB)
    imRGB = im.fromarray(RGB)
    imRGB=imRGB.resize((300,200), im.BILINEAR)
    ddat=DclData(V,R,G,B,D,imRGB,dclex,0)
    return ddat
