...
def get_channel(connection, queue_name, durable=True, confirm_delivery=True, is_delay=False, is_lazy=True):
    ...
    channel.queue_declare(queue=queue_name, durable=durable, arguments=args)
    if confirm_delivery:
