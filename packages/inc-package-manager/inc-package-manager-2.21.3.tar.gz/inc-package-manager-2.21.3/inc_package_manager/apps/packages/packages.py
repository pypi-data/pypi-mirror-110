...
class IncPackageManager(object):
	...
	def install(self, 
		...
		response_object = self.request("/api/packages/download/", {
			"package":package,
			"stable":stable,
			"version":version,
		}, json=False)
		...
		if response_object.status_code != 200:
			return dev0s.response.error(f"Failed to install package {package} {version_str} [{response_object.status_code}] (/api/packages/download/).")	
		...
		if post_install not in [None, False, ""]:
			...
			command = f"bash {tmp_dir.file_path.path}{post_install} --user {dev0s.defaults.vars.user} {post_install_args}"
	...
	# check if a package is purchased.
	def purchased(self,
		# the package id (str) (#1). 
		package,
		# the product's plan (str) (optional).
		plan=None, 
		# the log levenl (int).
		log_level=dev0s.defaults.options.log_level,
		# 
		...
		for key,value in self.subscriptions.items():
			if (
				(plan == None and package == value["product"])
				or
				(plan != None and plan == value["plan"] and package == value["product"])
			):
				purchased = True
				break

		# handler.
		return dev0s.response.success(f"Successfully checked package {package}.", {
			"purchased":purchased,
		})
	...
	def connect(self):
		response = self.request("/api/packages/list/")
		if not response.success: 
		...
		response = self.request("/api/packages/versions/", {"stable":False})
		if not response.success: 
		...
		response = self.request("/api/packages/versions/", {"stable":True})
		if not response.success: 
		...
		response = self.request("/api/subscriptions/list/")
		if not response.success: 
			return dev0s.response.error(f"Failed to download the vandenberghinc subscriptions, error: {response['error']}")
	...
	# list packages.
	def list(self,
		# the search tag (str) (optional).
		#search=None,
		# list with os filter (list) (required) [*, free, paid].
		price=["*"],
		# list with os filter (list) (required) [*, macos, linux, windows].
		os=["*"],
	):

		# checks.
		if isinstance(os, (str, String)): os = [os]
		if isinstance(price, (str, String)): price = [price]
		if os == ["all"]: os = ["*"]
		if price == ["all"]: price = ["*"]
		...
		for package, info in self.packages.items():
			...
			# edit.
			try: info["version"] = self.versions["stable"][package]["newest"]
			except KeyError: info["version"] = None
			info["id"] = package

			# check add.
			add = True
			if add and "*" not in price and "free" not in price and info["free"]:
				add = False
			if add and  "*" not in price and "paid" not in price and not info["free"]:
				add = False
			if add and "*" not in os:
				for i in os:
					if i not in info["os"]:
						add = False
						break
