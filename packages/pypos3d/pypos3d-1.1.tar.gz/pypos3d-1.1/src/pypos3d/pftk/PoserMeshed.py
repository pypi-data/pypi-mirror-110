...
from langutil import RemoveExt, C_FAIL, C_OK, C_ERROR, C_FILE_NOT_FOUND
...
class PoserMeshedObject(StructuredAttribut):
  ...
  def createDefaultChannels(self, fp:'FigurePart'): # grp:'GeomGroup', translation=None, rotation=None, orientation=None, center=None):
    ...
    try:
      ...
      center= fp.center
      if center:
        if center[0]=='(' and center[-1]==')': # Center is given by a Tuple of float
          bc = Point3d(eval(center))
        elif center.lower().startswith('face='): # Can be Face=Material name
          centerFaceVx = fp.geomGroup.getFaceVertexBy(center[5:])
          bc = Point3d.barycentre(centerFaceVx)
        else:
          return fp.status(C_FAIL,'Center definition error for [{:s}]:{:s}', self.getName(), center) 
      else:
        bc = fp.geomGroup.calcBarycentre()
      ...
      if len(tor)>1:
        ...
        if tor[0]=='(' and tor[-1]==')': # The orientation is given by a Tuple of float
          vr = Point3d(eval(tor))
          rotx, roty, rotz = vr.x, vr.y, vr.z
          
        elif tor.lower().startswith('face='): # Can be Face=Material name
          vxtab = fp.geomGroup.getFaceVertexBy(tor[5:])
          rotx, roty, rotz = Point3d.triangle_orientation(vxtab[0], vxtab[1], vxtab[2], AxisOrder=AxisOrder, inDegrees=True)
        else:
          return fp.status(C_FAIL, 'Orientation definition error for [{:s}]:{:s}', self.getName(), tor)
        
      self.setOrientation(Vector3d(rotx, roty, rotz))
      ...
      if fp.translation:
        ...
        trl = fp.translation.lower()
        pos = trl.find(':') # Potential location for a parent actor
        if pos>0:
          parentPMO = self.getFigure().findActor(fp.translation[pos+1:], withIndex=False)
          if not parentPMO:
            return fp.status(C_FAIL, 'Parent [{:s}] not found for [{:s}] translation', fp.translation[pos+1:], self.getName())
        ...
        elif trl.startswith('face='): # Can be Face=Material name
          centerFaceVx = fp.geomGroup.getFaceVertexBy(fp.translation[5:])
          tr = Point3d.barycentre(centerFaceVx).sub(bc)
        
        # Manage symLL[:PartName]
        elif trl.startswith('symxy'):
          tr = Point3d( 0.0, 0.0, -2.0*(bc.z - origin.z))
        elif trl.startswith('symyz'):
          tr = Point3d(-2.0*(bc.x - origin.x), 0.0, 0.0)
        elif trl.startswith('symzx'):
          tr = Point3d( 0.0, -2.0*(bc.y - origin.y), 0.0)
        else:
          return fp.status(C_FAIL,'Translation definition error for [{:s}]:{:s}', self.getName(), fp.translation) 
      ...
      vri = Vector3d()
      if fp.rotation:
        if fp.rotation[0]=='(' and fp.rotation[-1]==')': # Center is given by a Tuple of float
          vri = Vector3d(eval(fp.rotation))
        else:
          return fp.status(C_FAIL, 'Rotation definition error for [{:s}]:{:s}', self.getName(), fp.rotation)
      ...
      ret = fp.status(C_FAIL, 'Definition error for [{:s}]:', self.getName(), str(ve))
