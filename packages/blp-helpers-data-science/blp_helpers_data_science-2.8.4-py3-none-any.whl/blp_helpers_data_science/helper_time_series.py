...
def get_baseline(data, calc_col, viz=False, test_size=0.66, show_explanation=False, drop_na=True, eval_on='train'):
    """Uses persistence_algorithm to calc RMSE to get baseline value for forecasting performance

    :param data: pandas.DataFrame with calc_col
    :param calc_col: column that should be predicted
    :param viz:
    :param test_size:
    :param show_explanation:
    :param drop_na:
    :param eval_on: whether to use train or test set for the persistance model
    :return:
    ...
    """if the data has two lag values, then 
    x = today
    y = tomorrow
    ...
    predictions = eval_data_x

    # calc rmse
    rmse = sqrt(mean_squared_error(eval_data_y, eval_data_x))
    ...
    if viz == eval_on+'+predict':
        sns.set(rc={'figure.figsize': (18, 6)})
        sns.lineplot(data=eval_data_y, color='green', label=eval_on + '_y')
    ...
    residuals = pd.DataFrame(eval_data_y.to_numpy() - predictions.to_numpy())
