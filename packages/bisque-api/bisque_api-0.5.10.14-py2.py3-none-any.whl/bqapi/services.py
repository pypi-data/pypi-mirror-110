...
class BaseServiceProxy(object):
    #  Allows simple access to response codes so that response.status_code == service.codes.ok will work
    codes = requests.codes

    def __init__(self, session, service_name, timeout=DEFAULT_TIMEOUT):
        self.session = session
        self.service_url = session.service_map.get(service_name, None) or  os.path.join(session.c.root , service_name)
...
class ImportProxy(BaseServiceProxy):
    def transfer (self, filename, fileobj=None, xml=None, render=None, callback=None):
        ...
        if fields:
            ...
            #KGK removing seems to break requests_toolbelt=0.8
...
class DatasetProxy (BaseServiceProxy):
    ...
    def create(self, dataset_name, member_list, **kw):
        ...
        Args:
           dataset_name (str): The name of teh
           member_list (list): list ofg uniq ID
        Returns:
           A response or None
        """
        data = self.session.service('data_service')
        ...
        return data.post(data=etree.tostring(dataset))
    ...
    def append_member (self, dataset_uniq, resource_uniq, **kw):
        """Append an element(s) to dataset
        Args:
           dataset_uniq (str): uniq id of dataset
           resource_unque (str|list): uniqs to append
        Returns:
          Response
        """
        if isinstance(resource_uniq, str):
            resource_uniq =  resource_uniq.split(',')
        data = self.session.service('data_service')
        dataset = etree.Element('dataset', uri=dataset_uniq)
        for uniq  in resource_uniq:
            member = etree.SubElement(dataset, 'value', type='object')
            member.text = data.construct (uniq)
        response = data.post(dataset_uniq, data=etree.tostring(dataset))
        return response

    def delete_member (self, dataset_uniq, resource_uniq, **kw):
        """Delete a member(s)..
        Args:
           resource_unque (str|list): uniqs to delete, str ins comma seperated list
        Returns:
           None if nothing deleted, or a response with xml
        """
        removed = set()
        if isinstance(resource_uniq, str):
            resource_uniq =  resource_uniq.split(',')
        data = self.session.service('data_service')
        dataset = data.fetch ( dataset_uniq,  params = {'view':'full'}, render='xml')
        for uniq in resource_uniq:
            members = dataset.xpath ('value[text()="%s"]' % data.construct (uniq))
        ...
        if removed:
            ...
            return data.put (dataset_uniq, data = etree.tostring (dataset))
        return None
