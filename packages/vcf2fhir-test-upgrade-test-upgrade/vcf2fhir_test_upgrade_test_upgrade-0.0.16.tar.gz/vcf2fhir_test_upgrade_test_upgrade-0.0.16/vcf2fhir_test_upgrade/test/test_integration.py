...
import unittest
import os
import vcf2fhir_test_upgrade
import json
from os.path import join, dirname
import shutil
import logging

suite = doctest.DocTestSuite(vcf2fhir_test_upgrade)
...
class Testvcf2fhir_test_upgradeInputs(unittest.TestCase):

    def test_required_vcf_filename(self):
        with self.assertRaises(Exception) as context:
            vcf2fhir_test_upgrade.Converter()
        self.assertTrue(
            'You must provide vcf_filename' in str(context.exception))

    def test_invalid_vcf_filename(self):
        self.assertRaises(FileNotFoundError,
                          vcf2fhir_test_upgrade.Converter, *['Hello', 'GRCh38'])

    def test_required_ref_build(self):
        with self.assertRaises(Exception) as context:
            vcf2fhir_test_upgrade.Converter(os.path.join(
    ...
    def test_valid_ref_build_37(self):
        o_vcf_2_fhir = vcf2fhir_test_upgrade.Converter(os.path.join(
            os.path.dirname(__file__), 'vcf_example1.vcf'), 'GRCh37')
        self.assertEqual(type(o_vcf_2_fhir), vcf2fhir_test_upgrade.Converter)
    ...
    def test_conv_region_only(self):
        ...
        o_vcf_2_fhir = vcf2fhir_test_upgrade.Converter(
            os.path.join(
...
class TestTranslation(unittest.TestCase):
    ...
    def test_with_patient_id(self):
        self.maxDiff = None
        o_vcf_2_fhir = vcf2fhir_test_upgrade.Converter(os.path.join(os.path.dirname(
...
class TestLogger(unittest.TestCase):
    ...
    def test_logger_forks(self):
        ...
        # create logger
        general_logger = logging.getLogger('vcf2fhir_test_upgrade.general')
        invalid_record_logger = logging.getLogger('vcf2fhir_test_upgrade.invalidrecord')
...
suite.addTests(unittest.TestLoader().loadTestsFromTestCase(Testvcf2fhir_test_upgradeInputs))
