...
from unfolded.data_sdk.errors import AuthenticationError
...
def read_fileobj_chunks(
        fileobj: IO[bytes],
        chunk_size: int,
        callback: Optional[Callable[[int], Any]] = None) -> Iterable[bytes]:
...
def compress_fileobj(
        file_in: IO[bytes], file_out: IO[bytes], **kwargs: Any) -> None:
