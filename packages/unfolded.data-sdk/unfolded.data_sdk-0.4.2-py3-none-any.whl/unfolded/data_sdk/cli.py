...
def download_dataset(
        dataset_id: str, output_file: Path, progress: bool) -> None:
...
def upload_file(
        file: Path, name: str, media_type: str, dataset_id: str, desc: str,
        progress: bool) -> None:
...
@click.argument('file', type=PathType(readable=True, file_okay=True))
def update_dataset(
        file: Path, dataset_id: str, media_type: str, progress: bool) -> None:
...
@click.option(
    '--description', type=str, required=False, help="The map description")
@click.option(
    '--map-state',
    type=click.File(),
    required=False,
...
def create_map(
        name: str, description: str, map_state: IO, dataset_ids: str) -> None:
    """Create an Unfolded Studio map
    """
    data_sdk = DataSDK()
    datasets = [
        dataset_id.strip()
        for dataset_id in dataset_ids.split(',')] if dataset_ids else None
...
@click.option(
    ...
    help="Comma separated list of dataset ids to add to the map")
def update_map(
        map_id: str, name: str, description: str, map_state: IO,
