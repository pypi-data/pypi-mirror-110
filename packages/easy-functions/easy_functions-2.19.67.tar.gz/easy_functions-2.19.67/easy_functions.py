...
"""
#更新状态:活跃
cmdlist = ['bigfont','literally','wait','cls','title','find_file','start','call','Vbs','color','mode','shield','choice','Code','mkdir','copy','pause','help','helpcmd','version','log'] #命令列表，以大写字母开头的为类(class)
__all__ = cmdlist
c = 0
d = 0
for i in cmdlist:
    if i.istitle():
        c += 1
d = len(cmdlist) - c
__version__ = str(c) + '.' + str(d) + '.' + '67' #easy_functions版本号
__author__ = 'Jerry\n我的QQ号：2711893794' #作者
...
def log(): #更新日志
    ...
    print('1.修改了功能Code的用法，具体使用helpcmd()查看')
    ...
    print('2.增加了pause功能，具体使用helpcmd()查看')
    print('')
    print('[2021-6-19] [1条更新]')
    print('1.对功能color的报错进行了优化')
    print('')
    print('[2021-6-20] [1条更新]')
    print('升级了功能find_file，使得此功能可以判断文件是否可以被导入')
    print('')
    return
def version(): #版本
    global __version__
...
def helpcmd( cmd ): #文件命令功能解释
    ...
    if cmd == 'find_file':
        print('<',cmd,'>','文件检测功能')
        print("用法：文件名或<import...as...>命令别名.find_file('文件路径')")
        print('如果返回True，则说明文件存在；如果返回"can_import"，则说明文件是可以被导入的；如果返回False，则说明文件不存在')
        return
    if cmd == 'start':
        print('<',cmd,'>','文件运行功能')
