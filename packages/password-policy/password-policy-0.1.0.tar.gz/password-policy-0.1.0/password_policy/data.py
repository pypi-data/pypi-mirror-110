...
def _to_json_compatible_object(obj: Any) -> dict[str,Any]:
  """
...
class PCP():
  ...
  def dumps(self) -> str:
    ...
    # Charsets need to set as null unused charsets from the default charsets
    charsets = self.charsets.copy()
    for key in (ALPHABET_CHARSETS.keys() if 'alphabet' in charsets else DEFAULT_CHARSETS.keys()):
      if key not in charsets:
        charsets[key] = None
    
    return json.dumps({'charsets': charsets, 'rules': [_to_json_compatible_object(rule) for rule in self.rules]})
  ...
  def loads(s: str) -> PCP:
    ...
    if 'rules' not in data:
      rules = [PCPRule._from_json_compatible_object(data)]
    else:
      rules = [PCPRule._from_json_compatible_object(rule) for rule in data['rules']]
  ...
  def validate(self) -> None:
    ...
    for i in range(len(self.rules)):
      self.rules[i]._validate(f'rules[{i}]', self.charsets)
...
class PCPRule():
  ...
  @staticmethod
  def _from_json_compatible_object(data: dict[str,Any]) -> PCPRule:
    """
    ...
    """
    if 'prohibited_substrings' in data:
      data['prohibited_substrings'] = set(data['prohibited_substrings'])
    if 'require' in data:
      data['require'] = set(data['require'])
    if 'require_subset' in data:
      data['require_subset'] = PCPSubsetRequirement._from_json_compatible_object(data['require_subset'])
    if 'charset_requirements' in data:
      data['charset_requirements'] = {
        key: PCPCharsetRequirement._from_json_compatible_object(value) 
...
class PCPSubsetRequirement():
  ...
  @staticmethod
  def _from_json_compatible_object(data: dict[str,Any]) -> PCPSubsetRequirement:
...
`` locations, indexing matches python string indexing.
  ...
  @staticmethod
  def _from_json_compatible_object(data: dict[str,Any]) -> PCPCharsetRequirement:
    """
    ...
    """
    if 'required_locations' in data:
      data['required_locations'] = set(data['required_locations'])
    if 'prohibited_locations' in data:
