...
def validate_python() -> None:
    ...
    if sys.version_info[:3] < REQUIRED_PYTHON_VER:
        print(
            "Claret Assistant requires at least Python "
...
def get_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Claret Assistant: Observe, Control, Automate."
    )
    ...
    parser.add_argument(
        ...
        help="Directory that contains the Claret Assistant configuration",
    )
    parser.add_argument(
        "--safe-mode", action="store_true", help="Start Claret Assistant in safe mode"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Start Claret Assistant in debug mode"
    )
    parser.add_argument(
    ...
    if os.name == "posix":
        parser.add_argument(
            "--daemon", action="store_true", help="Run Claret Assistant as daemon"
        )
...
def check_pid(pid_file: str) -> None:
    """Check that Claret Assistant is not already running."""
    # Check pid file
    try:
    ...
    try:
        ...
        # PID does not exist
        return
    print("Fatal Error: Claret Assistant is already running.")
    sys.exit(1)
...
def try_to_restart() -> None:
    """Attempt to clean up state and start a new Claret Assistant instance."""
    # Things should be mostly shut down already at this point, now just try
    # to clean up things that may have been left behind.
    sys.stderr.write("Claret Assistant attempting to restart.\n")
    ...
    # Now launch into a new instance of Claret Assistant. If this fails we
    # fall through and exit with error 100 (RESTART_EXIT_CODE) in which case
    # systemd will restart us when RestartForceExitStatus=100 is set in the
    # systemd.service file.
    sys.stderr.write("Restarting Claret Assistant\n")
    args = cmdline()
