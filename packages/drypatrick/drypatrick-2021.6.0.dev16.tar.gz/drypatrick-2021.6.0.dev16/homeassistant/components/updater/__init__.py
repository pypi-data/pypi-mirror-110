...
async def async_setup(hass, config):
    ...
    async def check_new_version() -> Updater:
        ...
        if AwesomeVersion(newest) > AwesomeVersion(current_version):
            _LOGGER.debug(
                "The latest available version of Claret Assistant is %s", newest
            )
            update_available = True
        elif AwesomeVersion(newest) == AwesomeVersion(current_version):
            _LOGGER.debug(
                "You are on the latest version (%s) of Claret Assistant", newest
            )
        elif AwesomeVersion(newest) < AwesomeVersion(current_version):
            _LOGGER.debug(
...
async def get_newest_version(hass):
    """Get the newest Claret Assistant version."""
    ...
    try:
        ...
        raise update_coordinator.UpdateFailed(
            "Received invalid JSON from Claret Assistant Update"
        ) from err
