...
def verify_version(value: str):
    ...
    if not validate_custom_integration_version(value):
        raise vol.Invalid(
            f"'{value}' is not a valid version. This will cause a future version of Claret Assistant to block this integration.",
        )
    return value
...
def validate_version(integration: Integration):
    ...
    if not integration.manifest.get("version"):
        integration.add_error(
            "manifest",
            "No 'version' key in the manifest file. This will cause a future version of Claret Assistant to block this integration.",
        )
        return
