        ...
        if hass.state != CoreState.not_running or entry.data["type"] == TYPE_LOCAL:
            ...
            async def configure_almond(_now):
                try:
                    ...
                    _LOGGER.warning(
                        "Unable to configure Almond to connect to Claret Assistant"
                    )
...
async def _configure_almond_for_ha(
    ...
    try:
        if entry.data["type"] == TYPE_OAUTH2:
            ...
            # with Claret Assistant if we're remotely accessible.
    ...
    _LOGGER.debug("Configuring Almond to connect to Claret Assistant at %s", hass_url)
