...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async def service_download(service):
        """Download a file from your Android TV / Fire TV device to your Claret Assistant instance."""
    ...
    async def service_upload(service):
        """Upload a file from your Claret Assistant instance to an Android TV / Fire TV device."""
        local_path = service.data[ATTR_LOCAL_PATH]
        if not hass.config.is_allowed_path(local_path):
            _LOGGER.warning("'%s' is not secure to load data from!", local_path)
            return
