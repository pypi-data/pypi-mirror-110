...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    cli = APIClient(
        ...
        port,
        password,
        client_info=f"Claret Assistant {const.__version__}",
    ...
    # the callback twice when shutting down Claret Assistant.
    ...
    async def send_home_assistant_state_event(event: Event) -> None:
        """Forward Claret Assistant states updates to ESPHome."""
    ...
    async def _send_home_assistant_state(
        ...
        """Forward Claret Assistant states to ESPHome."""
        await cli.send_home_assistant_state(entity_id, new_state.state)
...
async def platform_async_setup_entry(
    ...
    *,
    ...
    def async_list_entities(infos: list[EntityInfo]):
        ...
        # Add entities to Claret Assistant
...
def esphome_state_property(func):
    ...
    prevents writing NAN values to the Claret Assistant state machine.
    """
    ...
    def _wrapper(self):
        ...
        if isinstance(val, float) and math.isnan(val):
            # Claret Assistant doesn't use NAN values in state machine
