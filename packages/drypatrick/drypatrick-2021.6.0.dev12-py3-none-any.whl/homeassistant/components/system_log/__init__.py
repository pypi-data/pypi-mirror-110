...
def _figure_out_source(record, call_stack, hass):
    ...
    # a file in Claret Assistant. Try to figure out where error happened.
    paths_re = r"(?:{})/(.*)".format("|".join([re.escape(x) for x in paths]))
    for pathname in reversed(stack):

        # Try to match with a file within Claret Assistant
...
async def async_setup(hass, config):
    ...
    async def async_shutdown_handler(event):
        """Remove logging handler when Claret Assistant is shutdown."""
