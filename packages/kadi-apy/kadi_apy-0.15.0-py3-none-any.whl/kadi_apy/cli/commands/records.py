...
@records.command()
@apy_command
@id_identifier_options(class_type="record", helptext="to download files from")
...
def get_file(record, file_id, filepath, pattern, force, use_folder):
    """Download one file, all files or files with pattern from a record."""

    record.get_file(filepath, force, file_id, pattern, use_folder)
...
def get_records(manager, user, my_user_id, **kwargs):
    ...
    search = manager.search_resource()
