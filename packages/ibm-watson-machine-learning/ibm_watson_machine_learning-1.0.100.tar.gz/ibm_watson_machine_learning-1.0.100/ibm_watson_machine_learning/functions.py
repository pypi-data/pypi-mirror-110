                ...
                if not self._client.CLOUD_PLATFORM_SPACES and not self._client.ICP_PLATFORM_SPACES:
                    if self.ConfigurationMetaNames.RUNTIME_UID not in meta_props and self.ConfigurationMetaNames.SOFTWARE_SPEC_UID not in meta_props:
                        ...
                        if not self._ICP:
                            ...
                            check_runtime = requests.get(self._client.service_instance._href_definitions.get_runtime_href(runtime_uid),  headers=self._client._get_headers(),verify=False)
...
""".format(code, function.__name__)
    ...
    def download(self, function_uid, filename='downloaded_function.gz', rev_uid=None):
        ...
        artifact_url = self._client.service_instance._href_definitions.get_function_href(artifact_uid)
        if self._client.WSD:
            import urllib
           ...
           # function_details = self.get_details(function_uid)
            ...
            if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_PLATFORM_SPACES:
                artifact_content_url = self._client.service_instance._href_definitions.get_function_href(artifact_uid) + '/code'
            else:
                artifact_content_url = self._client.service_instance._href_definitions.get_function_latest_revision_content_href(artifact_uid)
