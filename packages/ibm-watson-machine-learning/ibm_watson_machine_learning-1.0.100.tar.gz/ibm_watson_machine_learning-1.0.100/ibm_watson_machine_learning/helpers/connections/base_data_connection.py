...
class BaseDataConnection(ABC):
    ...
    def _download_training_data_from_data_asset_storage(self) -> 'DataFrame':
        ...
        if self._wml_client is not None:
            ...
            asset_response = requests.get(self._wml_client.service_instance._href_definitions.get_data_asset_href(asset_id),
            ...
            if 'handle' in asset_details['attachments'][0]:
                ...
                artifact_content_url = (f"{self._wml_client.service_instance._href_definitions.get_wsd_model_attachment_href()}"
                                        f"{attachment_url}")
