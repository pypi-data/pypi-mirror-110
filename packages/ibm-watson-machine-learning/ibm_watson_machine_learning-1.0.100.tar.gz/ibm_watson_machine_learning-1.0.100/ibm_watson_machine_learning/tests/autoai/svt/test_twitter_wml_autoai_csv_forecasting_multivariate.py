...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_06_get_train_data(self):
        ...
        TestAutoAIRemote.X_values = self.train_data.drop(['value1', 'value2'], axis=1)[:10].values
        print(self.train_data.head())
        TestAutoAIRemote.y_values = self.train_data[['value1', 'value2']][:10]
    ...
    def test_33_score_batch_deployed_model(self):
        scoring_params = self.service_batch.run_job(
            payload=self.train_data.drop(columns=['day']),
            background_mode=False)
