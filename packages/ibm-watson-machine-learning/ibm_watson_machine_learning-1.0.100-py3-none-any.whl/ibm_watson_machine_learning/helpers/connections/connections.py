...
from ibm_watson_machine_learning.utils.autoai.errors import (
    ...
    NotExistingCOSResource, InvalidDataAsset, CannotReadSavedRemoteDataBeforeFit
)
...
class DataConnection(BaseDataConnection):
    ...
    def __init__(self,
        ...
        if data_join_node_name is None:
            ...
            if self.type == DataConnectionTypes.S3 or (
                    self.type == DataConnectionTypes.CA and hasattr(location, 'path')):
    ...
    def _from_dict(cls, _dict: dict) -> 'DataConnection':
        ...
        if _dict['type'] == DataConnectionTypes.S3:
            ...
            if _dict['location'].get('path') and _dict['location'].get('bucket'):
                data_connection: 'DataConnection' = cls(
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            if os.environ.get('USER_ACCESS_TOKEN') is None and self._check_if_connection_asset_is_s3():
                ...
                try:
                    ...
                    raise ConnectionError(f"Unable to access data object in cloud object storage with credentials supplied. "
                                          f"Error: {cos_access_exception}")
            else:
                try:
...
class AssetLocation(BaseLocation):
    ...
    def _get_connection_id(self, client) -> str:
        ...
        attachment_content = attachment.json()

        if 'connection_id' in attachment_content:
            return attachment_content['connection_id']
