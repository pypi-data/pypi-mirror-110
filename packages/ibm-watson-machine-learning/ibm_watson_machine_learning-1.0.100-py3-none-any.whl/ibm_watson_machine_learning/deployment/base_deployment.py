...
class BaseDeployment(ABC):
    ...
    def _promote_model(self,
                       ...
                       space_id: str):
        """
        Protected method to promote a model from project to deployment space.
        Returns id of promoted model.
        """

        promote_payload = {"spaceId": space_id,
                           "projectId": project_id,
                           "assetType": "wml_model",
                           "assetDescription": "Model promoted by ibm_wml client"}
        promote_href = self._source_workspace.wml_client.service_instance._href_definitions.promote_asset_href(model_id)

        response = requests.post(
            promote_href,
            headers=self._source_workspace.wml_client._get_headers(),
            json=promote_payload,
            verify=False
        )
        promotion_details = self._source_workspace.wml_client.repository._handle_response(200, u'promote model asset', response)

        try:
            promoted_model_id = promotion_details['promotedAsset']['asset_id']
        except KeyError as key_err:
            raise ModelPromotionFailed(project_id, space_id, promotion_details, reason=key_err)
    ...
    def _publish_model(self,
        ...
        if self._source_workspace.project_id and self._source_workspace.project_id in str(meta_props):
            ...
            project_asset_id = self._source_workspace.wml_client.repository.get_model_uid(published_model_details)

            asset_id = self._promote_model(model_id=project_asset_id,
                                           project_id=self._source_workspace.project_id,
                                           space_id=self._target_workspace.space_id)
