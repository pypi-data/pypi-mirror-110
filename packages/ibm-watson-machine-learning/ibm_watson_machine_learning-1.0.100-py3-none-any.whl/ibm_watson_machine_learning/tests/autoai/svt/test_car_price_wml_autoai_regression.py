...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def _check_optimizer_parameters(self, parameters):
        ...
        self.assertEqual(parameters.get('daub_give_priority_to_runtime'), self.daub_give_priority_to_runtime)
        self.assertEqual(parameters.get('train_sample_rows_test_size'), self.train_sample_rows_test_size)
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            daub_give_priority_to_runtime=self.daub_give_priority_to_runtime,
            train_sample_rows_test_size=self.train_sample_rows_test_size
        )
    ...
    def test_14a_get_pipeline_params_specific_pipeline_parameters(self):
    ...
    def test_14b_get_pipeline_params_best_pipeline_parameters(self):
        best_pipeline_params = self.remote_auto_pipelines.get_pipeline_details()
        self.assertIn(f"holdout_{self.scoring_method}", str(best_pipeline_params))

    def test_15_check_params_of_realoaded_pipeline_optimizer(self):
