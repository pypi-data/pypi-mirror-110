...
class TunnelClient(Client):
    ...
    def __init__(self, name: str, tunnel_name: str, config_file_path: str):
        ...
        # self._tunnel.register_cb(TunnelClient._on_data)
    ...
    def req(self, path_tag, msg, msg_type, req_id):
        ...
        with self._cv:
            if not self._cv.wait_for(lambda: not self._is_req, Fist.REQ_WAIT_TIME / 1000 + 2):
        ...
        with self._cv:
            if not self._cv.wait_for(lambda: self._last_req_res is not None, Fist.REQ_WAIT_TIME / 1000):
                self._is_req = False
