...
class Video:
    ...
    def __init__(self, url: str, quality: str = "720p"):
        try:
            ...
            data = BeautifulSoup(content, "html.parser").findAll("ul", href=False, attrs={"class": ""})
            ...
            data = BeautifulSoup(content, "html.parser").findAll("meta", href=False, attrs={"itemprop": "thumbnailUrl"})
            ...
            data = BeautifulSoup(content, "html.parser").findAll("div", href=False, attrs={"class": "date_added"})
            ...
            data = BeautifulSoup(content, "html.parser").findAll("div", href=False, attrs={"id": "directed_by"})
            self.director = None if not data else data[0].text.split(':')[-1].strip()

            file_name = self.url.split('/')[-1]

            response = requests.post("https://www.radiojavan.com/videos/video_host", params={"id": file_name})
            quality = "lq" if self.quality == "480p" else "hq" if self.quality == "720p" else "hd"
            self.download_link = f"{response.json()['host']}/media/music_video/{quality}/{file_name}.mp4"
