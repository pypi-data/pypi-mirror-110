...
class Postgres(object):
    ...
    def __init__(self, conf=None, conf_path=None):
        """
        生成数据库连接self.cursor
        :param conf: 对应数据库配置，默认从配置文件读取
        :param conf_path: yaml配置文件绝对路径（可放在环境下任意路径），与file_path字段不同时存在，文件格式参考同目录下的db.yaml
        """
        if not conf:
    ...
    def is_table_exist(self, table_str):
        ...
        print(f'{table_str} is_table_exist: {exist}')
        return exist
    ...
    def find(self, table, filter_dict, columns):
        """
        查询所有满足条件的结果
        ...
        sql = f"select {columns_str} from {table} where {filter_str}"
        print('sql:', sql)
