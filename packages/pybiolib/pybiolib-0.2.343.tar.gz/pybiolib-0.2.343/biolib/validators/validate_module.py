...
from biolib.validators.validator_utils import AllowedYAMLEnvironments, normalize, custom_executors, \
...
def validate_unsupported_task_fields(name, task_data, error_dict, yaml_version):
    ...
    if not isinstance(task_data, dict):
        error_dict['unsupported_fields'] = [f'Module {name} is the wrong type. Modules can only be a YAML dict in version {yaml_version}']
        return
