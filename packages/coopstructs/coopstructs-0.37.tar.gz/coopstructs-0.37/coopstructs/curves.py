...
class CubicBezier(Curve):
    ...
    def as_circular_arcs(cls, curve, tolerance: float = 10):
        ...
        if curve.origin != curve.end_point:
            circle, deviation = curve.biarc_circle()
            if circle is None:
                pass
    ...
    def biarc_circle(self) -> (Circle, float):
        ...
        max_delta = 0
        for point in self.compute_bezier_points([(x.x, x.y) for x in self.control_points], 10):
            point = Vector2(point[0], point[1])
            dist_from_center = (point - biarc_circ.center).length()
            deviation = abs(biarc_circ.radius - dist_from_center)
            if deviation > max_delta:
                max_delta = deviation

        return biarc_circ, max_delta
...
class CurveBuilder:
    ...
    def save_curve(self):
        ...
        for curve in new_curves:
            new_id = self.id_provider()
            new_curve_dict[new_id] = curve
            self.curves[new_id] = curve
        self.current_points = []
