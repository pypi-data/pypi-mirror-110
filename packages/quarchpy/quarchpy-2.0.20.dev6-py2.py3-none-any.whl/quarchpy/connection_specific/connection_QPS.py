...
class QpsInterface:
    ...
    def sendCmdVerbose(self, cmd, timeout=10):
        cmd = cmd + "\r\n"
        ...
        while response.rfind('\r\n>') == -1:
            ...
            if time.time() - start > timeout:
                logging.warning("Hit timeout during QPS read")
                break

        pos = response.rfind('\r\n>')
        if pos == -1:
            logging.warning("Did nto retrieve trailing '\\r\\n>' from QPS read, returned full response")
