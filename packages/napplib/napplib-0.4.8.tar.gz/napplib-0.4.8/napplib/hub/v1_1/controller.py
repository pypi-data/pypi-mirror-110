...
class HubController:
    ...
    @classmethod
    def get_total_products_by_store(server_url, token, store_id):
        headers = dict()
        headers['Authorization'] = f'Bearer {token}'
        
        url = f'/totalProductsByStore/{store_id}'
        
        response = requests.get(f'{server_url}{url}', headers=headers)
        
        return json.loads(response.content)

    @classmethod
    def get_products_not_exists(server_url, token, store_id, products=[]):
        headers = dict()
        headers['Authorization'] = f'Bearer {token}'
        ...
        response = requests.get(f'{server_url}{url}', headers=headers, data=payload)
        
        return json.loads(response.content)

    @classmethod
    def post_update_inventory(server_url, token, store_id, marketplace_id, products=[]):
        headers = dict()
        headers['Authorization'] = f'Bearer {token}'
        ...
        response = requests.post(f'{server_url}{url}', headers=headers, data=payload)
