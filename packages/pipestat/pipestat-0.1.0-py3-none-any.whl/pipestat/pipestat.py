...
class PipestatManager(dict):
    ...
    def db_column_relationships_by_result(self) -> Dict[str, str]:
        ...
        *Note: this is an experimental feature*
        ...
        def _validate_rel_section(result_id):
            if not all(
                [
                    k in list(self.schema[result_id][DB_RELATIONSHIP_KEY].keys())
    ...
    def db_url(self) -> str:
        ...
        parsed_creds = {k: quote_plus(str(v)) for k, v in list(creds.items())}
    ...
    def _create_table_orm(self, table_name: str, schema: Dict[str, Any]):
        ...
        def _auto_repr(x: Any) -> str:
            ...
            """
            attr_strs = [
                f"{k}={str(v)}" for k, v in list(x.__dict__.items()) if not k.startswith("_")
            ]
        ...
        for result_id, result_metadata in list(schema.items()):
    ...
    def _report_db(
        ...
        if not self.check_record_exists(
            ...
            with self.session as s:
                ...
                for result_id, result_value in list(values.items()):
