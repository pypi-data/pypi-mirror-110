...
CN_KEY = gpg.Key(
    ...
    sub_keys=(
        gpg.SubKey(
            ...
            key_capabilities={gpg.KeyCapability.Encrypt(False)},
        ),
    ),
    origin=KEYSERVER,
)
...
class TestGPG(unittest.TestCase):
    ...
    def test_parse_search_keyserver(self):
        response = io.BytesIO(
            b"""info:1:1
...
@unittest.skipUnless(KEY_SERVER is not None and WITH_GPG, "Key Server Tests")
class TestGPGKeyserver(IntegrationTest):
    def test_send_search(self):
        ...
        self.s.send_keys(key_a.key_id, key_b.key_id, keyserver=KEY_SERVER)

        keys = keyserver.search_keyserver(key_a.uids[0].full_name, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_a)])

        keys = keyserver.search_keyserver(key_b.uids[0].full_name, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])

        # By fingerprint
        keys = keyserver.search_keyserver(key_b.fingerprint, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])

        # By ID
        keys = keyserver.search_keyserver(key_b.key_id, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])
    ...
    def test_recv_keys(self):
        try:
            with tempfile.TemporaryDirectory() as gpg_temp_dir:
                ...
                temp_store.send_keys(key_a.key_id, key_b.key_id, keyserver=KEY_SERVER)
        except FileNotFoundError:
            pass
        self.s.recv_keys(key_a.key_id, key_b.fingerprint, keyserver=KEY_SERVER)
