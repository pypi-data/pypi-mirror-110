...
from vectice.api._utils import calculate_duration
from vectice.api.project import ProjectApi
from vectice.entity import DatasetVersion, ModelVersion, CodeVersion
from vectice.entity.job_artifact import ArtifactType
...
class RuleApi(ProjectApi):
    ...
    def start_run(
        self, job: Job, run: Optional[JobRun] = None, inputs: Optional[List[Artifact]] = None
    ) -> Dict[str, Any]:
        if run is None:
            run = JobRun()
        if inputs is None:
    ...
    def stop_run(self, run: dict, outputs: Optional[List[Artifact]] = None):
        ...
        if run.get("endDate") is None:
            ...
            run["duration"] = calculate_duration(now, run["createdDate"])
        if outputs is None:
            outputs = []
        else:
