...
class MlflowAdapter(Adapter):
    ...
    def start_run(self, inputs: Optional[List[Artifact]] = None):
        ml_run = mlflow.start_run()
        job, run, output_artifacts = self._extract_from_mlflow(ml_run)
        self._runnable_job = RunnableJob(job, run)
        return super().start_run(inputs)
    ...
    def end_run(self, *args, **kwargs):
        ...
        job, run, output_artifacts = self._extract_from_mlflow(ml_run)
        self._active_job.update(job.__dict__)
        self._active_run.update(run.__dict__)
        self._mlflow_client.set_tag(ml_run.info.run_id, VECTICE_RUN_TAG, self._active_run["id"])
        super().end_run(output_artifacts)
    ...
    def __save_run(
        ...
        super().end_run(mapped_output_artifacts)
        return run_id
