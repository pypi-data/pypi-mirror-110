...
class Root(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.loaded_files = {} #{'file_path' : [region_id1, region_id2,...]}
    ...
    def _do_regions_math(self, math):
        ...
        checked_regions = [self.loaded_regions.get_by_id(reg_id) for reg_id in checked_ids]
        if not datahandler.Region.check_compatibility(checked_regions):
            self.winfo_toplevel().display_message("The chosen regions are incompatible (in energy interval or step). "
                                                  "Can't do math.")
            return
        if len(checked_ids) == 0:
            self.winfo_toplevel().display_message("No regions have been chosen. "
                                                  "Choose regions for doing math.")
            return

        if math == math_options[0] and len(checked_ids) < 2:
            self.winfo_toplevel().display_message("Addition operation requires at least two regions. "
                                                  "Choose at least two regions for doing math.")
            return
        if (math == math_options[1] or math == math_options[2]) and len(checked_ids) != 2:
            self.winfo_toplevel().display_message("Subtraction of more than two regions is ambiguous. "
                                                  "Choose exactly two for doing math.")
            return
        if math == math_options[3]:
            formula = simpledialog.askstring("Type formula",
                                             "Write a formula using '+-*/().', numbers and letters A-Z for the regions",
                                             parent=self)
            if len([ch for ch in formula if ch not in "+-/*. ()"+string.ascii_uppercase+string.digits]) > 0:
                self.winfo_toplevel().display_message("It is only possible to use the following characters in the formula. "
                                                      "+-/*. ()A-Z and numbers")
                return
        ...
        new_region_id = simpledialog.askstring("Choose new region name (unique)",
                                               f"The following regions will be used\n{txt_regions_list}",
                                               parent=self)
        ...
        try:
            ...
            if math == math_options[3]:
                ...
                for i in range(len(checked_regions)):
                    new_formula = new_formula.replace(string.ascii_uppercase[i], f"checked_regions[{i}].get_data('final')")
                try:
                    new_data = eval(new_formula)
                except NameError:
                    self.winfo_toplevel().display_message(f"The formula '{formula}' couldn't be evaluated. "
                                                          "Check and try again using +-/*. ()A-Z and numbers")
                    return
                new_region_info = copy.deepcopy(checked_regions[0].get_info())
                new_region_info["Sweeps Number"] = '1'
                new_region_info["File Name"] = new_region_id
                new_region = datahandler.Region(checked_regions[0].get_data('energy'), new_data,
                                                info=new_region_info,
                                                add_dimension_flag=False,
                                                add_dimension_data=None,
                                                conditions=None,
                                                excitation_energy=checked_regions[0].get_excitation_energy(),
                                                id_=new_region_id,
                                                fermi_flag=checked_regions[0].get_flags(flagname=datahandler.Region.region_flags[2]),
                                                flags=None)
                new_region.add_correction(f"Math: {formula} for Regions: {checked_ids}")
            ...
            self.winfo_toplevel().display_message("Oops! Something went wrong...")
    ...
    def generate_main_menu(self):
        ...
        self.math_menu.add_command(label="Use formula",
                                   command=lambda: self._do_regions_math(math='Use formula'))
        # self.file_menu.add_separator()
    ...
    def load_file_list(self, file_list: list, file_type):
        ...
        #If some files have already been loaded, ask if the user wants to reload
        duplicates = [fn for fn in file_list if fn in self.loaded_files.keys()]
        if len(duplicates) > 0:
            answer = messagebox.askquestion("Reloading files", f"The following files have already been loaded. Do you want "
                                                               f"to reload them?\n{', '.join([os.path.basename(d) for d in duplicates])}")
        if len(file_type) == len(file_list):
            for i, file_name in enumerate(file_list):
                if (file_name in duplicates and answer == 'yes') or not file_name in duplicates:
                    loaded_ids[file_name] = self.loaded_regions.add_regions_from_file(file_name, file_type[i], overwrite=True)
        else:
            for file_name in file_list:
                if (file_name in duplicates and answer == 'yes') or not file_name in duplicates:
                    loaded_ids[file_name] = self.loaded_regions.add_regions_from_file(file_name, file_type[0], overwrite=True)
        last_region_id = ""
        if loaded_ids:
            for key, val in loaded_ids.items():
                ...
                if val:
                    # In order to not add already existing names to the browser tree
                    if not key in self.loaded_files:
                        self.gui_widgets["BrowserPanel"].spectra_tree_panel.add_items_to_check_list(os.path.basename(key), val)
                        self.loaded_files[key] = val
                    else:
                        #If the file has already been loaded. Now it can be reloaded.
                        new_val = [v for v in val if v not in self.loaded_files[key]]
                        if len(new_val) > 0:
                            self.gui_widgets["BrowserPanel"].spectra_tree_panel.add_items_to_check_list(os.path.basename(key), new_val)
                            self.loaded_files[key] += new_val
    ...
    def show_about(self):
        messagebox.showinfo("About", f"SpecQP v{specqp.__version__}\n"
                                     "Mikhail Shipilin\n"
