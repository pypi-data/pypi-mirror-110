...
class Region:
    ...
    def do_math(first, second, math, ydata='final', truncate='True'):
        ...
        new_region = Region(first.get_data('energy'), counts,
                            ...
                            id_=f"{first.get_id()} {math} {second.get_id()}",
...
class RegionsCollection:
    ...
    def add_regions(self, new_regions, overwrite=False):
        ...
        :param overwrite: Overwrites already existing regions.
        ...
        for new_region in new_regions:
            ...
            if self.is_duplicate(new_id) and not overwrite:
    ...
    def add_regions_from_file(self, file_path, file_type=DATA_FILE_TYPES[0], overwrite=False):
        ...
        try:
            if file_type == DATA_FILE_TYPES[0]:
                ids = self.add_regions(load_scienta_txt(file_path), overwrite=overwrite)
            elif file_type == DATA_FILE_TYPES[1]:
                ids = self.add_regions(load_specs_xy(file_path), overwrite=overwrite)
            elif file_type == DATA_FILE_TYPES[2]:
                ids = self.add_regions(load_csv(file_path), overwrite=overwrite)
        except OSError:
