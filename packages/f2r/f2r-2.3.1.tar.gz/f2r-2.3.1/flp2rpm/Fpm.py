...
class Fpm:
  ...
  def run(self, name, version, deps, devel_deps, extra_deps, RPMS_GENERATED = []):
    ...
    if len(version_split) == 2:
        ...
        iterationArg = ['--iteration', version_split[1].replace('local', '', 1) + '.' + self.release_suffix]
    else:
    ...
    subdirs = ['bin', 'lib', 'lib64', 'etc', 'include', 'bin-safe', 'libexec', 'WebDID', 'share', 'plugins', 'cmake', 'sbin']
    paths = []
    ...
    if paths:
        fpmCommand.extend(['--exclude=*/modulefiles', '--exclude=*/profile.d'])
    ...
    for depName, depVersion in deps.items():
      if depName not in self.skipDeps:
        if (depVersion != "from_system"):
            countDashes = depVersion.count('-')
            if countDashes > 1:
                depVersion = depVersion.replace('-', '_', countDashes - 1)
            depsArg.extend(['--depends', self.packagePrefix + depName + ' >= ' + depVersion.replace('local', '', 1)])
