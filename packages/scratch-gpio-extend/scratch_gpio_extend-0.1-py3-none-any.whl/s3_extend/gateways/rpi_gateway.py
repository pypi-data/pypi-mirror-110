...
import adafruit_dht # dht 센서import
...
class RpiGateway(GatewayBase):
    ...
    def set_mode_sonar(self, topic, payload):  # SEt해주고 아래 read에서 읽는다.
        """
        ...
        self.dht = adafruit_dht.DHT11(trigger)
        # self.sonar = Sonar(self.pi, trigger, echo) # 트리거핀 에코핀정하고
    ...
    def read_sonar(self):
        ...
        time.sleep(0.5)
        # test = 1234
        # sonar_time = self.sonar.read() # 센서읽어와서
        # distance = sonar_time / 29 / 2 # 거리 처리
        # distance = round(distance, 2) # 거리처리
        payload = {'report': 'sonar_data', 'value': temp} # payload에담아서 distance를 내보낸다. 여기다가 dht_sensor값을 보내주면됨
        self.publish_payload(payload, 'from_rpi_gateway') # Scratch로 내보낸다
