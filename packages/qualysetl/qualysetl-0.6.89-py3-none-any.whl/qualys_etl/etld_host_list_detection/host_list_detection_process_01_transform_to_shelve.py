    ...
    try:
        ...
        for xml_file in xml_file_list:
            with open(xml_file, 'r', encoding='utf-8') as t_xml_file:
                with shelve.open(str(shelve_file), flag='n') as shelve_database:
...
def end_msg_host_list_detection_shelve():
    ...
    etld_lib_functions.logger.info(
        f"count host ids added to shelve: {count_items_added_to_shelve:,} for {str(shelve_file)}")
    for xml_file in xml_file_list:
        etld_lib_functions.log_file_info(xml_file, 'in')
...
def setup_shelve_vars():
    ...
    xml_dir = etld_lib_config.host_list_detection_xml_dir
