...
# 한번만 실행할 수 있게 make_dart_dict() 함수 외부에서 선언한다.
...
class 자산재평가실시결정(DartSubject):
    ...
    def process(self):
        ...
        c103_자본총계y = load_db.Corps(self.intro['code'], col='c103재무상태표y').find_c103(page='재무상태표y', title='자본총계')
        if self.echo:
            pprint.pprint(self.data)
