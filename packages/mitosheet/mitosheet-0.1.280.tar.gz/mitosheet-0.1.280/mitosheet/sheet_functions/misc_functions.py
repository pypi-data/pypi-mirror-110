...
def FILLNAN(series: pd.Series, replacement: pd.Series) -> pd.Series:
    ...
    """
    # Make copy of the series as to not fill the nan values in the series itself
    series_copy = series.copy(True)
    nan_indexes = series_copy.isna()
    for idx in range(len(nan_indexes)):
        if nan_indexes[idx]:
            series_copy[idx] = replacement[0]
