...
def main():
    ...
    if scan_type:
        ...
        while status != "finished" and status != "failed":
            scan_status, scan_percentage = check_scan(scan_id, project_id, apikey, baseurl)

            if scan_percentage is None and scan_status not in ["finished", "failed", "queued", "running", "processed"]:
                # fail to find scan
                retry += 1
                logger.info(scan_status + f"({retry}/10)")

                if retry > 10:
                    logger.error("Fail to get scan infomation, please manually check scan status on website.")
                    exit(1)
                else:
                    time.sleep(10)
                    continue
