...
async def sync_all_commands(client : typing.Union[discord.ext.commands.Bot, discord.ext.commands.AutoShardedBot, discord.Client, discord.AutoShardedClient], case_sensitive=True, loading_message="Loading", send_hidden=False, hidden_commands=[], choices={}, error_function=None):
    ...
    for x in client.commands:
        commands.append(_create_info(x, choices))
    ...
    for x in commands:
        w = await _post(f"https://discord.com/api/v9/applications/{client.user.id}/commands", json_dict=x, headers=_get_headers(client))
        try:
            if w["name"] == x["name"]:
    ...
    slash_commands = await _get(f"https://discord.com/api/v9/applications/{client.user.id}/commands", headers=_get_headers(client))
    name_list = []
    for x in commands:
        name_list.append(x["name"])
    for x in slash_commands:
        if x["name"] not in name_list or x["name"] in hidden_commands:
            t = await _delete(f"https://discord.com/api/v9/applications/{client.user.id}/commands/{x['id']}", headers=_get_headers(client))
            if t == "":
