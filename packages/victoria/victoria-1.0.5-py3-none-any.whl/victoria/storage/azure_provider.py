...
class AzureStorageProvider(provider.StorageProvider):
    ...
    def store(self, data: Union[IOBase, str, bytes], key: str) -> None:
        try:
            ...
            logging.error("ERROR: Error occurred while try to perform store operation for key %s", key)
            raise ex
    ...
    def retrieve(self, key: str, stream: IOBase):
        try:
            ...
            logging.error("ERROR: Error occurred while try to perform retrieve operation for key %s", key)
            raise ex
