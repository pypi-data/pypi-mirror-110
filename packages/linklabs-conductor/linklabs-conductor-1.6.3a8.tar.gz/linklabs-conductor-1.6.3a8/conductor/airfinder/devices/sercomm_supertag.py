...
class SupertagProfile(ConfigurationProfile):
    ...
    def __init__(self, **kwargs):
        self.values = [
            kwargs.get('sym_heartbeat'),
            kwargs.get('stationary_location_update_rate'),
            kwargs.get('moving_location_update_rate'),
            kwargs.get('mailbox_check_interval'),
...
class SercommSupertag(Supertag):
    ...
    @property
    def moving_st_location_update_rate(self):
        return self._make_prop(int, "stModeLocUpdateRate_Moving")

    @property
    def stationary_st_location_update_rate(self):
    ...
    def profiles(self):
        ...
        for i in range(0, 5):
            profiles_arr.append(SupertagProfile(
                sym_heartbeat=self._md.get(f"PRO_{i}_symModeHeartbeatInterval"),
                stationary_location_update_rate=self._md.get(f"PRO_{i}_symModeLocInterval_Stationary"),
                moving_location_update_rate=self._md.get(f"PRO_{i}_symModeLocInterval_Moving"),
                mailbox_check_interval=self._md.get(f"PRO_{i}_symModeMailboxInterval"),
                stationary_network_scan_interval=self._md.get(f"PRO_{i}_networkScanInterval_Stationary"),
                moving_network_scan_interval=self._md.get(f"PRO_{i}_networkScanInterval_Moving"),
                acc_enable=self._md.get(f"PRO_{i}_accelerometerEnable"),
                acc_duration=self._md.get(f"PRO_{i}_accelerometerMovementDuration"),
                acc_threshold=self._md.get(f"PRO_{i}_accelerometerThreshold"),
                beacon_flags=self._md.get(f"PRO_{i}_beaconFlags")
            ))
        return profiles_arr
