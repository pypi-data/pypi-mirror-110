...
class UsersManagementClient(object):
    """Approw Users Management Client"""
    ...
    def create(self, userInfo, keep_password=False, custom_data=None):
        ...
        Args:
            userInfo (dict): 用户信息
            keep_password (bool): 该参数一般在迁移旧有用户数据到 Approw 的时候会设置。开启这个开关，password 字段会直接写入 Authing 数据库，
                                    Approw 不会再次加密此字段。如果你的密码不是明文存储，你应该保持开启，并编写密码函数计算。
        ...
        if not isinstance(userInfo, dict):
            raise ApprowWrongArgumentException('userInfo must be a dict')
    ...
    def search(
        ...
        if department_opts:
            if not isinstance(department_opts, list):
                raise ApprowWrongArgumentException('department_opts must be a list')

        if group_opts:
            if not isinstance(group_opts, list):
                raise ApprowWrongArgumentException('group_opts must be a list')
    ...
    def get_udf_value_batch(self, user_ids):
        if type(user_ids).__name__ != "list":
            raise ApprowWrongArgumentException('empty user id list')
    ...
    def set_udf_value(self, user_id, data):
        ...
        if not isinstance(data, dict):
            raise ApprowWrongArgumentException('data must be a dict')
    ...
    def set_udf_value_batch(self, data):
        ...
        """
        if not isinstance(data, dict):
            raise ApprowWrongArgumentException('data must be a list')

        for k, v in data.items():
            if not isinstance(v, dict):
                raise ApprowWrongArgumentException('invalid data input')
