...
def arg_parse():
    ...
    indir = args.indir
    path = args.path
    return nucleotide,genbank,resultdir,databases,coverages,identities,indir,path
...
def parse_genbank(genbank,resultdir):
    ...
    for gb_seq in gb_seqs:
        index1 += 1
        new_id = "bacantsequence"+str(index1)
...
def process_dir(blastn_path,final_indir,final_resultdir):
    ...
    nucleotide,genbank,resultdir,databases,coverages,identities,indir,path = arg_parse()
    if path:
    ...
    ResDB = database[0]
    IntegronDB = database[1]
    TransposonDB = database[2]
    ...
    for file in os.listdir(indir):
        ...
        if TransposonDB:
            if path:
                ...
                database = curent_dir+'/TransposonDB/Transposon'
            transposon(nucleotide,outdir,blastn_path,cov2,ident2,database)
        ...
        if indir:
            ...
            if TransposonDB:
                if path:
                    ...
                    database = curent_dir+'/TransposonDB/Transposon'
                transposon(nucleotide,resultdir,blastn_path,cov2,ident2,database)
            if IntegronDB:
                while 1:
