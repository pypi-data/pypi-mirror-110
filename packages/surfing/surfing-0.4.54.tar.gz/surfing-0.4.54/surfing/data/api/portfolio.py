...
class PortfolioDataApi(metaclass=Singleton):
    ...
    def get_product_price_part(self, begin_date, end_date, time_para, product_list: Tuple[str] = (), manager_id=None):
        try:
            ...
            if manager_id == None:
                ...
                hf_info = basic.get_fof_info_strategy_exsit(fof_id_list=product_list,manager_id=manager_id)
            ...
            _df1 = derived.get_fof_nav_public_adj(fof_id_list=real_ids,start_date=begin_date,end_date=end_date)
            if not _df1.empty > 0:
                _df1 = _df1.pivot_table(index='datetime',columns='fund_id',values='nav')
            _df2 = derived.get_fof_nav(manager_id=manager_id, fof_id_list=real_ids)
            if not _df2.empty > 0:
                _df2 = _df2.pivot_table(index='datetime',columns='fof_id',values='adjusted_nav')
                if not _df1.empty:
                    _df = _df1.append(_df2).drop_duplicates()
                else:
                    _df = _df2.copy()
            else:
                _df = _df1.copy()
    ...
    def product_recent_rate_data(self, product_list, manager_id=None):
        try:
            ...
            result = []
            real_ids = product_list
            _df1 = derived.get_fof_nav_public_adj(fof_id_list=real_ids,start_date='20091231')
            if not _df1.empty > 0:
                _df1 = _df1.pivot_table(index='datetime',columns='fund_id',values='nav')
            _df2 = derived.get_fof_nav(manager_id=manager_id, fof_id_list=real_ids)
            if not _df2.empty > 0:
