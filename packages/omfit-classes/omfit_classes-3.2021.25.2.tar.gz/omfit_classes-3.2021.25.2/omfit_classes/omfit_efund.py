...
__all__ = ['OMFITmhdin', 'OMFITdprobe', 'OMFITnstxMHD']
...
class OMFITmhdin(OMFITnamelist):
    ...
    def to_omas(self, ods=None, update=['pf_active', 'magnetics']):
        """
            ...
            for group in self:
                if group not in ['all']:
                    for item in self[group]:
                        self[group][item]['mds_tree'] = mds_tree_map[self[group][item]['mds_tree']]
