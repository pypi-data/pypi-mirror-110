...
from azureml.train.automl._constants_azureml import CodeGenOutputPaths
    ...
    try:
        ...
        if automl_settings_obj.enable_code_generation:
            try:
                ...
                with open('autogenerated_script.py', 'w') as f:
                    f.write(code)
                current_run.upload_file(CodeGenOutputPaths.Script, 'autogenerated_script.py')
                logger.info('Script has been generated, output saved to {}'.format(CodeGenOutputPaths.Script))

                notebook = notebook_generator.generate_script_run_notebook(current_run._experiment)
                with open('script_run_notebook.ipynb', 'w') as f:
                    f.write(notebook)
                current_run.upload_file(CodeGenOutputPaths.ScriptRunNotebook, 'script_run_notebook.ipynb')
                logger.info('Script has been generated, output saved to {}'.format(
                    CodeGenOutputPaths.ScriptRunNotebook))
