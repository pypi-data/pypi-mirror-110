...
class AutoMLModelExplainForecastingDriver(AutoMLModelExplainDriver):
    ...
    def setup_model_explain_train_data(self) -> None:
        ...
        dataset_transformers = self._dataset.get_transformers()
        Contract.assert_value(dataset_transformers, 'dataset_transformers')
        Contract.assert_true(
            tst_key in dataset_transformers,
            'Timeseries_transformer cannot be found from transformers.',
            log_safe=True
        )
        Contract.assert_type(
            dataset_transformers[tst_key],
