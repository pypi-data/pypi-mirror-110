...
#            s = self.scoringstripes(result, EV, mask)
#            s = s[0]
#            result.insert(result.shape[1], 'Stripiness', s, True)
#            res = self.RemoveRedundant(result, 'Stripiness')
            res = self.RemoveRedundant(result, 'mean')
    ...
    def StripeSearch(self, submat, num, start, end, M, perc, chr, framesize, start_array, end_array):
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            for b in np.arange(0.5, 1.01, 0.1):  # b: brightness parameter
                ...
                #blur = cv.filter2D(adj, -1, kernel) # temporary deletion
                #blur = np.clip(blur, a_min=0, a_max=1) # temporary deletion
                # plt.subplot(111), plt.imshow(blur), plt.title('Blurry'), plt.show()
                gray = cv.cvtColor(np.float32(adj), cv.COLOR_RGB2GRAY) # temporary modification
                ...
                for ud in [1, 2]:
                    ...
                    for c in range(Xsize - 1):
                        ...
                        if abs(m - n) > 1 and abs(m - n) <= self.maxW:
                            if abs(m - n) > 4 :
                                Pair.append((n + 0, m - 3)) # modified 06/22/21
                            else:
                                Pair.append((n, m))
                            ...
                            if ud == 1:
                                if abs(m-n) > 4:
                                    MAX = m-3 # MAX = X[c + 1] # modified 06.22.21
                                else:
                                    MAX = m
                            else:
                                if abs(m-n) > 4:
                                    MIN = n+0 # MIN = X[c] # modified 06.22.21
                                else:
                                    MIN = n
                            MIN_vec.append(MIN)
                            MAX_vec.append(MAX)
            ...
            result = self.RemoveRedundant(result, 'mean')
    ...
    def RemoveRedundant(self, df, by):
        ...
        if by != 'size' and by != 'score' and by != 'pvalue' and by !='mean':
            raise ValueError('"by" should be one of "size", "pvalue" and "score" and "mean"')
        df_size = df.shape
        row_size = df_size[0]
        if row_size == 0:
            return df
