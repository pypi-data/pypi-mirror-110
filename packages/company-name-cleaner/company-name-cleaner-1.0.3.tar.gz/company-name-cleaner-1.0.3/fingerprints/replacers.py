...
def build_replacer(to_ascii: bool = True) -> Callable[[str], str]:
    replacements: Dict[str, str] = {}
    with open(DATA_PATH, "r") as fh:
        types = json.load(fh).get("types", {})
        # Compile person prefixes into a regular expression.
        for form, canonical in types.items():
            form = clean_strict(form, to_ascii=to_ascii)
            canonical = clean_strict(canonical, to_ascii=to_ascii)
...
def get_replacer(to_ascii: bool = True) -> Callable[[str], str]:
    return build_replacer(to_ascii)


def replace_types(text: Optional[str], to_ascii: bool = True) -> Optional[str]:
    """Chomp down company types to a more convention form."""
    if text is None:
        return None
    return get_replacer(to_ascii)(text)
