...
class Charts(MPLVisualization):
	...
	def launch(self, title):
		...
		self.fig = plt.figure()
		# if self.gsargs is not None: self.fig.add_gridspec(self.gsargs[0], self.gsargs[1], **self.gsargs[2])
	...
	def addPlot(self, name, label, type=None, selected=True, **kwargs):
		# if self.maxplots is not None and len(self.plots)>=self.maxplots: raise ValueError(f'Current gridspec allows a maximum of {self.maxplots} plots.')
	...
	#Store the arguments here because the figure isn't created until launch time
	# def gridspec(self, nrows=1, ncols=1, **kwargs):
	# 	self.gsargs = (nrows, ncols, kwargs)
	# 	self.maxplots = nrows * ncols
