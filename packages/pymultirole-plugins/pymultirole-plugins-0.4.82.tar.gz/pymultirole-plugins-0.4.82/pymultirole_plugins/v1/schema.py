...
class Annotation(Span):
    ...
    text: Optional[str] = Field(None, description="Covering text of the annotation", example="Kairntech")
    score: Optional[float] = Field(None, description="Confidence score of the annotation", example=0.87)
...
class Category(BaseModel):
    ...
    score: Optional[float] = Field(None, description="Confidence score of the category", example=0.87)
...
class Sentence(Span):
    ...
    categories: Optional[List[Category]] = Field(None, description="Sentence categories")
...
class AnnotatorDocumentExample(BaseModel):
...
class ProcessorDocumentExample(BaseModel):
    text: str = Field("Mandatory text")
    identifier: Optional[str] = Field("Optional identifier")
    title: Optional[str] = Field("Optional title")
    metadata: Optional[Dict[str, Any]] = Field({})
    sentences: Optional[List[Sentence]] = Field([])
    annotations: Optional[List[Annotation]] = Field([Annotation(start=0, end=10, labelName="Optional annotation")])
    categories: Optional[List[Category]] = Field([Category(labelName="Optional category")])
