...
class OneSampleT():
    ...
    def calculate(self, **kwargs):
        ...
        self.ci_mean = [ci.ci_mean(self.frame.iloc[:,0])]
...
class Wilcoxon():
    ...
    def calculate(self, **kwargs):
        self.array_a = self.frame.iloc[:,0]
        self.array_b = self.frame.iloc[:,1] if self.ncols == 2 else None
...
class TwoSampleT():
    ...
    def calculate(self, **kwargs):
        self.array_a = self.frame.iloc[:,0]
        self.array_b = self.frame.iloc[:,1]
...
class Anova():
    ...
    def calculate(self, **kwargs):
        ...
        self.statistic, self.p_value = stats.f_oneway(*self.frame.dropna().T.values)
        self.passed = True if self.p_value >= self.alpha else False
...
class KruskalWallis():
    ...
    def calculate(self, **kwargs):
        ...
        self.statistic, self.p_value = stats.kruskal(*self.frame.dropna().T.values)
        self.passed = True if self.p_value >= self.alpha else False
...
def _equal_means_plot(plot_obj, **kwargs):
    ...
    for row in range(0, plot_obj.ncols):
        ...
        # plot CI values
        ci_digits_str = bluebelt.core.helpers._format_digits([ci_mean[0], ci_mean[1]], digits=digits)
        ax.text(ci_mean[0], 1.5, f'{ci_mean[0]:{ci_digits_str}} ', **style.equal_means_plot.text_ci_min)
        ax.text(ci_mean[1], 1.5, f' {ci_mean[1]:{ci_digits_str}}', **style.equal_means_plot.text_ci_max)
        ...
        # add xticks to xtick list for formatting
        if row == 0:
            xticks = local_xticks
            xlims = ax.get_xlim()
    ...
    for row in range(0, plot_obj.ncols):
        ...
        ax.xaxis.set_major_formatter(FormatStrFormatter(f'%{bluebelt.core.helpers._format_digits(xticks, digits=digits)}'))
        ax.set_xlim(xlims)

    # add suptitle
    fig.suptitle(t=f'{plot_obj.test} for {bluebelt.core.helpers._get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.equal_means_plot.suptitle)
...
class FTest():
    ...
    def plot(self, **kwargs):
        return _equal_variance_plot(self, **kwargs)
...
class Levene():
    ...
    def calculate(self, **kwargs):
        self.statistic, self.p_value = stats.levene(*self.frame.dropna().T.values)
        self.passed = True if self.p_value >= self.alpha else False
...
class Bartlett():
    ...
    def calculate(self, **kwargs):
        self.statistic, self.p_value = stats.bartlett(*self.frame.dropna().T.values)
        self.passed = True if self.p_value >= self.alpha else False
...
class EqualVariance():
    """
    Test a dataframe for equal variance over the dataframe columns.
...
def _equal_variance_plot(plot_obj, **kwargs):
    ...
    for row in range(0, plot_obj.ncols):
        ...
        # calculate bin width
        bin_width = (plot_obj.frame.values.max() - plot_obj.frame.values.min()) / bins

        # 1. histogram ############################################
        ax.hist(array, bins=np.arange(plot_obj.frame.values.min(), plot_obj.frame.values.max() + bin_width, bin_width), **style.equal_variance_plot.histogram)
        ax.text(0.05, 0.9, array.name, transform=ax.transAxes, **style.equal_variance_plot.text)
        ax.set_yticks([])

        # add xlims to xlims list for formatting
        if row == 0:
            xlims = ax.get_xlim()
            ax.set_title(f't={plot_obj.statistic:1.2f} p={plot_obj.p_value:1.2f} ('+r'$H_0$'+f' is {"accepted" if plot_obj.p_value > plot_obj.alpha else "rejected"})', **style.equal_variance_plot.title)
    ...
    # add suptitle
    fig.suptitle(t=f'{plot_obj.test} for {bluebelt.core.helpers._get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.equal_variance_plot.suptitle)
...
# distribution
@bluebelt.core.decorators.class_methods
class AndersonDarling2():
    """
    Anderson-Darling test for distribution testing
    """
    
    def __init__(self, series, dist='norm', alpha=0.05, **kwargs):
    ...
    def calculate(self, **kwargs):
        ...
        AD, critical_values, significance_level = stats.anderson(self.series, dist=self.dist)
    ...
    for row in range(0, nrows):
        for col in range(0, ncols):
            ...
            if id < plot_obj.ncols:
                ...
                ax.text(0.99, 0.01, f'statistic: {plot_obj.statistics[id]:1.2f}\np-value: {plot_obj.p_values[id]:1.2f}', transform=ax.transAxes, **style.distribution_plot.stat_text)
            ...
            # ticks
            # ax.tick_params(axis="x", top=False, direction='in', pad=-18)
            # ax.tick_params(axis="y", right=False, direction='in', pad=-18)
            
    # add suptitle
    fig.suptitle(t=f'Probability plot of {bluebelt.core.helpers._get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.distribution_plot.suptitle)
