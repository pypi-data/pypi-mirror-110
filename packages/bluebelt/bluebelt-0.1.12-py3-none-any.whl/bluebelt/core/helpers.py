...
def _get_boxplot_outliers(series, whis=1.5):
    low, q1, q3, high = get_boxplot_quantiles(series=series, whis=whis)

    return series[series < low].append(series[series > high]).values


# histogram
def _bins(series, points, min_bins=10, max_bins=20):
...
def _get_digits(x, digits=6):
    prtd = max(digits, math.ceil(math.log10(abs(x))))
    prtd -= math.ceil(math.log10(abs(x)))
    prtd = min(digits, prtd)
    return prtd

def _format_digits(x, digits=6):
    if isinstance(x, list):
        return _format_digits(max([_get_digits(i, digits=digits) for i in x if i is not None]), digits=digits)
