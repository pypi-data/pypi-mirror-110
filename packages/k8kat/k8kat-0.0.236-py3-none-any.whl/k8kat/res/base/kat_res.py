...
from kubernetes.client import ApiClient, V1OwnerReference
...
# --
  ...
  def owner_ref(self, target_kind: str) -> V1OwnerReference:
    finder = lambda ref: ref.kind == target_kind
    candidates = self.raw.metadata.owner_references
    return next(filter(finder, candidates), None)
...
def auto_namespaced_kat_cls(res_name_or_kind: str) -> Optional[Type[KatRes]]:
  ...
  if definition:
    class NsdKatShell(KatRes):
      ...
      @classmethod
      def is_namespaced(cls) -> bool:
        return definition.get('namespaced')
      ...
      def k8s_verb_methods(cls) -> Dict[str, Callable]:
        def _list_namespaced(namespace, **kwargs):
          return rest_backend.list_namespaced_resources(
            cls.res_name_plural,
            definition['versioned_apigroup'],
            namespace,
            **kwargs
          )

        def _list_cluster_scoped(**kwargs):
          return rest_backend.list_cluster_scoped_resources(
            cls.res_name_plural,
            definition['versioned_apigroup'],
            **kwargs
          )
        ...
        return dict(
          list=_list_namespaced if cls.is_namespaced() else _list_cluster_scoped,
          delete=_delete
        )
    return NsdKatShell
  else:
...
def from_dict(dict_repr: Dict):
  ...
  for guess in class_guesses:
    try:
      getattr(kubernetes.client.models, guess)
      print(f"win for {guess}")
      return api.deserialize(mocked_kube_http_resp, guess)
    except:
      pass
