...
complete --command yt-dlp --long-option compat-options --description 'Options that can help keep compatibility with youtube-dl and youtube-dlc configurations by reverting some of the changes made in yt-dlp. See "Differences in default behavior" for details'
...
complete --command yt-dlp --long-option concurrent-fragments --short-option N --description 'Number of fragments of a dash/hlsnative video that should be download concurrently (default is %default)'
...
complete --command yt-dlp --long-option auto-number
complete --command yt-dlp --long-option title
complete --command yt-dlp --long-option literal
...
complete --command yt-dlp --long-option cache-dir --description 'Location in the filesystem where youtube-dl can store some downloaded information (such as client ids and signatures) permanently. By default $XDG_CACHE_HOME/youtube-dl or ~/.cache/youtube-dl'
...
complete --command yt-dlp --long-option ignore-no-formats-error --description 'Ignore "No video formats" error. Usefull for extracting metadata even if the video is not actually available for download (experimental)'
...
complete --command yt-dlp --long-option print --short-option O --description 'Simulate, quiet but print the given fields. Either a field name or similar formatting as the output template can be used'
...
complete --command yt-dlp --long-option check-formats --description 'Check that the formats selected are actually downloadable (Experimental)'
...
complete --command yt-dlp --long-option allow-unplayable-formats --description 'Allow unplayable formats to be listed and downloaded. All video post-processing will also be turned off'
...
complete --command yt-dlp --long-option audio-format --description 'Specify audio format to convert the audio to when -x is used. Currently supported formats are: best (default) or one of best|aac|flac|mp3|m4a|opus|vorbis|wav'
...
complete --command yt-dlp --long-option embed-metadata --description 'Embed metadata including chapter markers (if supported by the format) to the video file (Alias: --add-metadata)'
complete --command yt-dlp --long-option no-embed-metadata --description 'Do not write metadata (default)  (Alias: --no-add-metadata)'
...
complete --command yt-dlp --long-option exec --description 'Execute a command on the file after downloading and post-processing. Similar syntax to the output template can be used to pass any field as arguments to the command. An additional field "filepath" that contains the final path of the downloaded file is also available. If no fields are passed, "%(filepath)s" is appended to the end of the command'
