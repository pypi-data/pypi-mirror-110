...
def create_dataset(tfrecords,
    ...
    if encoding:
        parsed_dataset = raw_dataset.map(
            lambda example: _parse_tfrecords_encoded(
                example, height, width, depth, keys, one_hot_labels, metadata),
...
def _parse_tfrecords_encoded(example, height, width, depth, keys,
    ...
    if metadata:
        return img_source, label, img_filename
    else:
