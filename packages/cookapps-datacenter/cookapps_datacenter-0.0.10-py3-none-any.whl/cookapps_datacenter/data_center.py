    ...
    class Athena:
        ...
        def run_query(self, query: str, database: str) -> DataFrame:
            ...
            if result['QueryExecution']['Status']['State'] == 'SUCCEEDED':
                ...
                try:
                    print("bucket => {}".format(self.__athena_config.s3_bucket))
                    print("s3 key => {}".format(s3_key))
                    test = self.__s3_client.Bucket(self.__athena_config.s3_bucket).download_file(s3_key, local_filename)
                ...
                if os.path.isfile(local_filename):
                    query_result_df = pd.read_csv(local_filename)
