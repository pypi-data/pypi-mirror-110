...
from typing import Dict, Tuple, List, Iterator, Generator, AsyncGenerator, Optional
...
class SQLAlchemy:
    ...
    Usage 2: Without context manager
       s = SQLAlchemy()
       setup_options = s.setup2(...)
       s.reflect_schema(...)
       session = s.session_scope2(...)
       session.query(...)
       s.dispose2(setup_options)
    """
    ...
    def setup(self, options: Dict = {}):
        ...
        try:
            setup_engines_options = self.setup2(options=options)
            yield self
        finally:
            self.dispose2(setup_engines_options=setup_engines_options)

    def setup2(self, options: Dict = {}):
        setup_engines_options = []
    ...
    def _setup(self, instance: SQLAlchemyDBConfig = None, options: Dict = {}):
        (
            ...
            instance.engine,
        ) = self._create_engine(options=options)
    ...
    def dispose2(self, setup_engines_options: List = []):
    ...
    def session_scope(
        ...
        for session in self._session_scope_generator(
            auto_flush=auto_flush, expire_on_commit=expire_on_commit
        ):
            yield session

    def _session_scope_generator(
        self, auto_flush=True, expire_on_commit=True
    ) -> Generator[Session, None, None]:
    ...
    def session_scope2(self, auto_flush=True, expire_on_commit=True) -> Session:
        """Provide a transactional scope around a series of database operations."""
        session_maker = self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit
        )
    ...
    async def async_session_scope(
        ...
        """Provide a transactional scope around a series of database operations."""
        async for session in self._async_session_scope_generator(
            auto_flush=auto_flush, expire_on_commit=expire_on_commit
        ):
            yield session

    async def _async_session_scope_generator(
        self, auto_flush=True, expire_on_commit=True
    ) -> AsyncGenerator[Session, None]:
        session_maker = self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit
        ...
        async for session in self._async_session_scope(session_maker=session_maker):
            yield session

    async def async_session_scope2(self, auto_flush=True, expire_on_commit=True) -> Session:
        """Provide a transactional scope around a series of operations."""
        session_maker = self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit
        )
    ...
    async def _async_session_scope(self, session_maker: None) -> AsyncGenerator[Session, None]:
