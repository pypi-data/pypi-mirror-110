...
class RegistryClient:
    ...
    def rebuild_send_heart_beat(self, overridden_status=""):
        # print('resend heartbeat for eureka registry at', str(datetime.datetime.now()))
        with self.__net_lock:
            dc_eureka_servers = copy.deepcopy(self.__eureka_servers)
            ok = False
            for eureka_server in self.__eureka_servers:
                if ok:
                    break
                url = str(eureka_server).strip()
                retry_times = 3
                while retry_times > 0:
                    retry_times -= 1
                    try:
                        send_heart_beat(url, self.__instance["app"],
                                        self.__instance["instanceId"],
                                        self.__instance["lastDirtyTimestamp"],
                                        status=self.__instance["status"],
                                        overridden_status=overridden_status)
                        ...
                        if e.code == 404:
                            _logger.warning("Eureka server [%s] has lost heartbeat, retrying ..." % url)
                            self.__instance["status"] = INSTANCE_STATUS_UP
                            self.__instance["overriddenstatus"] = INSTANCE_STATUS_UNKNOWN
                            self.__instance["lastUpdatedTimestamp"] = str(_current_time_millis())
                            self.__instance["lastDirtyTimestamp"] = str(_current_time_millis())
                            _register(url, self.__instance)
                            self.__alive = True
                            _logger.warning("Eureka server [%s] has owned heartbeat again ..." % url)
                        else:
                            _logger.warning('exception code out of 404, unexpected exception: %s' % e)

                        curr_index = self.__eureka_servers.index(eureka_server)
                        dc_eureka_servers = \
                            self.__eureka_servers[curr_index + 1:] + self.__eureka_servers[:curr_index + 1]

                    except Exception as e:
                        _logger.warning('exception type out of urllib2.HTTPError, unexpected exception: %s' % e)
                        curr_index = self.__eureka_servers.index(eureka_server)
                        dc_eureka_servers = \
                            self.__eureka_servers[curr_index + 1:] + self.__eureka_servers[:curr_index + 1]
...
class DiscoveryClient:
    ...
    def __generate_service_url(self, instance, prefer_ip, prefer_https):
        ...
        # ipAddr -> ip_addr
        # hostName -> host_name
        if instance.port.port and not instance.secure_port.enabled:
            schema = "http"
            port = instance.port.port
        elif not instance.port.port and instance.secure_port.enabled:
            schema = "https"
            port = instance.secure_port.port
        elif instance.port.port and instance.secure_port.enabled:
            if prefer_https:
                schema = "https"
                port = instance.secure_port.port
            else:
        ...
        host = instance.ip_addr if prefer_ip else instance.host_name
