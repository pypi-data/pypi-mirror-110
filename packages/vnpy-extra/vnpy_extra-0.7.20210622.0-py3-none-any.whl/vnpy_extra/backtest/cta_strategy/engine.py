...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_by_trade(self, df: DataFrame = None):
            ...
            if trade_data.offset.value == Offset.OPEN.value:
                ...
                if volume == 0:
                    self.logger.warning(
                        ...
                        self.strategy, trade_data.symbol, trade_data.tradeid, trade_data.direction.value,
        ...
        # 总盈利 / 总亏损 （累计盈利亏损比）
        acc_profit_loss_ratio = abs(offset_acc_profit / offset_acc_loss)

        # 平均盈利 / 平均亏损
        average_profit = offset_acc_profit / len(offset_acc_pnl_s[offset_acc_pnl_s > 0])
        average_loss = offset_acc_loss / len(offset_acc_pnl_s[offset_acc_pnl_s < 0])
        average_profit_loss_ratio = abs(average_profit / average_loss)
    ...
    def show_chart(self, df: DataFrame = None,
        ...
        title_traces_dic["Daily Price"] = [daily_price, (fee_line, True), (profit_fee0_line, True), (profit_line, True)]
