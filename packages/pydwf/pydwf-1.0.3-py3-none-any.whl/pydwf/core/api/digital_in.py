...
class DigitalIn(AbstractDwfDeviceSubApi):
    ...
    def status(self, read_data_flag: bool) -> DwfState:
        ...
        Executing this method initiates a status request to the |DigitalIn| instrument and receives its response.
    ...
    def triggerGet(self) -> Tuple[int, int, int, int]:
        ...
        Returns:
            Tuple[int, int, int, int]: The currently active *level_low*, *level_high*, *edge_rise*,
    ...
    def triggerLengthSet(self, min_length: float, max_length: float, sync_mode: int) -> None:
        ...
        Parameters:
            ...
            sync_mode (int): Synchronization mode:
                ...
                * 2 — PWM: the *min_length* parameter specifies sthe ampling time and the *max_length* parameter
                  specifies the timing length.
    ...
    def triggerMatchSet(self, pin: int, mask: int, value: int, bit_stuffing: int) -> None:
        ...
        Configure the deserializer. The bits are left shifted. the mask and value should be specified according to this,
        in MSB-first order.
