...
class AnalogIn(AbstractDwfDeviceSubApi):
    ...
    def bitsInfo(self) -> int:
        ...
        Note:
            The Analog Discovery 2 uses an |Analog Devices AD964| two-channel ADC. It converts 14-bit samples at a rate
            of up to 125 MHz. So for the Analog Discovery 2, this method always returns 14.
    ...
    def channelRangeInfo(self) -> Tuple[float, float, int]:
        """Report the possible voltage ranges of the |AnalogIn| input channels, in Volts.
    ...
    def triggerSourceGet(self) -> DwfTriggerSource:
        """Get the |AnalogIn| instrument trigger source.

        Returns:
            DwfTriggerSource: The currently selected trigger source value.

        Raises:
            DwfLibraryError: An error occurred while retrieving the active trigger source.
        """
    ...
    def triggerPositionInfo(self) -> Tuple[float, float, int]:
        """Get the |AnalogIn| instrument trigger position info.
    ...
    def triggerPositionSet(self, trigger_position: float) -> None:
        """Set the |AnalogIn| instrument trigger position.
    ...
    def triggerPositionGet(self) -> float:
        """Get the |AnalogIn| instrument trigger position.
    ...
    def triggerAutoTimeoutInfo(self) -> Tuple[float, float, int]:
        """Get the |AnalogIn| instrument trigger auto-timeout info.
    ...
    def triggerAutoTimeoutSet(self, trigger_autotimeout: float) -> None:
        """Set the |AnalogIn| instrument trigger auto-timeout value.

        Parameters:
            trigger_autotimeout (float): The auto timeout setting, in seconds.

        Raises:
            DwfLibraryError: An error occurred while setting the trigger auto-timeout value.
        """
        result = self.lib.FDwfAnalogInTriggerAutoTimeoutSet(self.hdwf, trigger_autotimeout)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()

    def triggerAutoTimeoutGet(self) -> float:
        """Get the |AnalogIn| instrument trigger auto-timeout value.
        ...
        """
        c_trigger_autotimeout = typespec_ctypes.c_double()
        result = self.lib.FDwfAnalogInTriggerAutoTimeoutGet(self.hdwf, c_trigger_autotimeout)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
        trigger_autotimeout = c_trigger_autotimeout.value
        return trigger_autotimeout
    ...
    def triggerHoldOffInfo(self) -> Tuple[float, float, int]:
        """Get |AnalogIn| trigger detector holdoff info, in seconds.
        ...
        """
        c_trigger_holdoff_min = typespec_ctypes.c_double()
        c_trigger_holdoff_max = typespec_ctypes.c_double()
        c_trigger_holdoff_num_steps = typespec_ctypes.c_double()

        result = self.lib.FDwfAnalogInTriggerHoldOffInfo(
            self.hdwf,
            c_trigger_holdoff_min,
            c_trigger_holdoff_max,
            c_trigger_holdoff_num_steps)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()

        if not c_trigger_holdoff_num_steps.value.is_integer():
            raise PyDwfError("Bad c_trigger_holdoff_num_steps value.")

        trigger_holdoff_min = c_trigger_holdoff_min.value
        trigger_holdoff_max = c_trigger_holdoff_max.value
        trigger_holdoff_num_steps = int(c_trigger_holdoff_num_steps.value)

        return (trigger_holdoff_min, trigger_holdoff_max, trigger_holdoff_num_steps)

    def triggerHoldOffSet(self, trigger_holdoff: float) -> None:
        """Set the |AnalogIn| trigger detector  holdoff value, in seconds.

        Parameters:
            trigger_holdoff (float): The holdoff setting, in seconds.

        Raises:
            DwfLibraryError: An error occurred while setting the trigger holdoff value.
        """
        result = self.lib.FDwfAnalogInTriggerHoldOffSet(self.hdwf, trigger_holdoff)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()

    def triggerHoldOffGet(self) -> float:
        """Get the current |AnalogIn| trigger detector holdoff value, in seconds.
        ...
        """
        c_trigger_holdoff = typespec_ctypes.c_double()
        result = self.lib.FDwfAnalogInTriggerHoldOffGet(self.hdwf, c_trigger_holdoff)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
        trigger_holdoff = c_trigger_holdoff.value
        return trigger_holdoff

    def triggerTypeInfo(self) -> List[DwfAnalogInTriggerType]:
        """Get the |AnalogIn| trigger detector trigger type info.

        Returns:
            List[DwfAnalogInTriggerType]: A list of trigger types that can be configured.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger type info.
        """
        ...
        trigger_type_list = [trigger_type for trigger_type in DwfAnalogInTriggerType
                             if trigger_type_bitset & (1 << trigger_type.value)]
        return trigger_type_list

    def triggerTypeSet(self, trigger_type: DwfAnalogInTriggerType) -> None:
        """Set the |AnalogIn| trigger detector trigger type.

        Parameters:
            trigger_type (DwfAnalogInTriggerType): The trigger type to be configured.

        Raises:
            DwfLibraryError: An error occurred while setting the trigger type.
        """
    ...
    def triggerTypeGet(self) -> DwfAnalogInTriggerType:
        """Get the |AnalogIn| trigger detector trigger type.

        Returns:
            DwfAnalogInTriggerType: The currently configured trigger type.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger type.
        """
        c_trigger_type = typespec_ctypes.DwfAnalogInTriggerType()
        result = self.lib.FDwfAnalogInTriggerTypeGet(self.hdwf, c_trigger_type)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
        trigger_type = DwfAnalogInTriggerType(c_trigger_type.value)
        return trigger_type

    def triggerChannelInfo(self) -> Tuple[int, int]:
        """Get the |AnalogIn| trigger detector channel info.
        ...
        Raises:
            DwfLibraryError: An error occurred while getting the trigger channel info.
        """
    ...
    def triggerChannelSet(self, trigger_channel_index: int) -> None:
        ...
        Raises:
            DwfLibraryError: An error occurred while setting the trigger channel.
        """
    ...
    def triggerChannelGet(self) -> int:
        ...
        Raises:
            DwfLibraryError: An error occurred while getting the trigger channel.
        """
    ...
    def triggerFilterInfo(self) -> List[DwfAnalogInFilter]:
        """Get |AnalogIn| trigger detector filter info.

        Returns:
            List[DwfAnalogInFilter]: A list of filters that can be configured for the trigger.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger filter info.
        """
    ...
    def triggerFilterSet(self, trigger_filter: DwfAnalogInFilter) -> None:
        """Set the |AnalogIn| trigger detector filter.

        Parameters:
            trigger_filter (DwfAnalogInFilter): The trigger filter to be selected.

        Raises:
            DwfLibraryError: An error occurred while setting the trigger filter.
        """
    ...
    def triggerFilterGet(self) -> DwfAnalogInFilter:
        """Get the |AnalogIn| trigger detector filter.

        Returns:
            DwfAnalogInFilter: The currently configured trigger filter.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger filter.
        """
        c_trigger_filter = typespec_ctypes.DwfAnalogInFilter()
    ...
    def triggerLevelInfo(self) -> Tuple[float, float, int]:
        """Get the |AnalogIn| trigger detector level info, in Volts.

        Returns:
            Tuple[float, float, int]: The valid range of trigger level values that can be configured.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger level info.
        """
        c_trigger_level_min = typespec_ctypes.c_double()
        c_trigger_level_max = typespec_ctypes.c_double()
        c_trigger_level_num_steps = typespec_ctypes.c_double()

        result = self.lib.FDwfAnalogInTriggerLevelInfo(
            self.hdwf,
            c_trigger_level_min,
            c_trigger_level_max,
            c_trigger_level_num_steps)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()

        if not c_trigger_level_num_steps.value.is_integer():
            raise PyDwfError("Bad c_trigger_level_num_steps value")

        trigger_level_min = c_trigger_level_min.value
        trigger_level_max = c_trigger_level_max.value
        trigger_level_num_steps = int(c_trigger_level_num_steps.value)

        return (trigger_level_min, trigger_level_max, trigger_level_num_steps)

    def triggerLevelSet(self, trigger_level: float) -> None:
        """Set the |AnalogIn| trigger detector level, in Volts.
    ...
    def triggerLevelGet(self) -> float:
        """Get the |AnalogIn| trigger detector level, in Volts.
    ...
    def triggerHysteresisInfo(self) -> Tuple[float, float, int]:
        ...
        if not c_trigger_hysteresis_num_steps.value.is_integer():
            raise PyDwfError("Bad c_trigger_hysteresis_num_steps value.")
    ...
    def triggerConditionInfo(self) -> List[DwfTriggerSlope]:
        ...
        Returns:
            List[DwfTriggerSlope]: A list of possible detector condition values.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger condition info.
        """
    ...
    def triggerConditionSet(self, trigger_condition: DwfTriggerSlope) -> None:
        """Set the |AnalogIn| trigger detector condition.
    ...
    def triggerConditionGet(self) -> DwfTriggerSlope:
        """Get the |AnalogIn| trigger detector condition.
    ...
    def triggerLengthInfo(self) -> Tuple[float, float, int]:
        ...
        Raises:
            DwfLibraryError: An error occurred while getting the trigger length info.
        """
        ...
        result = self.lib.FDwfAnalogInTriggerLengthInfo(
            self.hdwf,
            c_trigger_length_min,
            c_trigger_length_max,
            c_trigger_length_num_steps)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()

        if not c_trigger_length_num_steps.value.is_integer():
            raise PyDwfError("Bad c_trigger_length_num_steps value.")
    ...
    def triggerLengthSet(self, trigger_length: float) -> None:
        ...
        Raises:
            DwfLibraryError: An error occurred while setting the trigger length.
        """
    ...
    def triggerLengthGet(self) -> float:
        ...
        Raises:
            DwfLibraryError: An error occurred while getting the trigger length.
        """
    ...
    def triggerLengthConditionInfo(self) -> List[DwfAnalogInTriggerLengthCondition]:
        ...
        Returns:
            List[DwfAnalogInTriggerLengthCondition]: The valid trigger length condition values that can be selected.
    ...
    def triggerLengthConditionSet(self, trigger_length_condition: DwfAnalogInTriggerLengthCondition) -> None:
        """Set the |AnalogIn| trigger detector length condition.

        Parameters:
            trigger_length_condition (DwfAnalogInTriggerLengthCondition):
                The trigger length condition to be configured.

        Raises:
            DwfLibraryError: An error occurred while setting the trigger length condition.
        """
    ...
    def triggerLengthConditionGet(self) -> DwfAnalogInTriggerLengthCondition:
        ...
        Returns:
            DwfAnalogInTriggerLengthCondition: The currently configured trigger length condition.

        Raises:
            DwfLibraryError: An error occurred while getting the trigger length condition.
        """
