...
class DynamoCrudRepository(CrudRepository):
    ...
    def __init__(
        ...
        if drop_table:
            try:
                if hasattr(self.table, "table_status") and (
                    ...
                    self.if_debug_print("Table found and will be deleted")
                    self.table.delete()
            except Exception as ex:
                print(ex)
        ...
        try:
            if (
                not hasattr(self.table, "table_status")
                or self.table.table_status == "DELETING"
            ) and self.create_table_if_not_exists:
                self.if_debug_print("Table not found and will be created")
                self.create_table()
...
if __name__ == "__main__":
    ...
    sample_crud = DynamoCrudRepository(
        SampleEntityClass, print_debug=True, use_mock_db=True, drop_table=True, create_table_if_not_exists=True
    )
