...
class DynamoCrudRepository(CrudRepository):
    ...
    def __init__(self, entity_type: T, drop_table: bool = False, print_debug: bool = False):
        ...
        try:
            ...
            raise RuntimeError("Not able to find the DynamoDB Table:", self.table_name)
        ...
        if drop_table:
            try:
                if hasattr(self.table, "table_status") and (
                    ...
                    if self.print_debug:
                        print("Table found and will be deleted")
                    self.table.delete()
            except Exception:
                pass
        ...
        try:
            if not hasattr(self.table, "table_status"):
                if self.print_debug:
                    print("Table not found and will be created")
                self.create_table()
    ...
    def remove_by_keys(self, keys: dict):
        try:
            ...
            warnings.warn("Not able to delete item with keys", keys, "from table", self.table_name)
    ...
    def find_by_keys(self, keys: Union[dict, list]):
        ...
        try:
            ...
            try:
                ...
                warnings.warn("Not able to query" + str(self.table) + "with keys" + str(keys))
                return None
    ...
    def save(self, obj: T):
        ...
        try:
            ...
            warnings.warn("Not able to put item" + str(item_params) + "in table" + str(self.table_name))
    ...
    def remove_all(self):
        ...
        try:
            ...
            raise ResourceWarning("Not able to remove all items from table", self.table_name)
        return True
    ...
    def remove_all_by_keys(self, keys_list: Iterable[dict]):
        try:
            ...
            raise ResourceWarning("Not able to remove items with keys", keys_list, "from table", self.table_name)

    def exists_by_keys(self, keys: Union[dict, list]):
        if isinstance(keys, list):  # Produce dict from list
            key_list = keys
            keys = {self.entity_type.dynamo_pk(): key_list[0], self.entity_type.dynamo_sk(): key_list[1]}
        try:
            response = self.table.get_item(Key=keys)
            if "Item" in response:
                return True
    ...
    def save_all(self, entities: Iterable[T]):
        try:
            ...
            raise ResourceWarning("Not able to save item list", entities, "into table", self.table_name)
...
def _wrap_class(
    ...
    ignore_attributes = (
        ignore_attributes if ignore_attributes is not None else []
    )
...
if __name__ == "__main__":
    counter = Counter()
    #global_params = DynamoDBGlobalParameters.get_instance(use_aws_cli_credentials=False)
    sample_crud = DynamoCrudRepository(SampleEntityClass, drop_table=True, print_debug=True)
