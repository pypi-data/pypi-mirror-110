...
def thomson_starter(k0, kz, dz, zs, theta1):
    ...
    idx = np.nonzero(np.abs(kz) >= np.abs(k0)) 
    a = np.empty(kz.shape)
    a[idx] = 0
    idx = np.nonzero(np.abs(kz) < np.abs(k0)) 
...
class TransmissionLoss():
    ...
    def calc(self, source_depth=None, rec_depth=[.1], vertical=False, aperture=86, 
                    progress_bar=True, nz_max=250, nr_max=250, size_limit=True,
                    return_field=False):
        """ Calculate the transmission loss in the horizontal plane at 
            ...
            Args:
                ...
                size_limit: bool
                    Limit of the size of the output arrays to the dimensions
                    specified in nz_max and nr_max. If False, nz_max and nr_max 
                    have no effect
                return_field: bool
                    Instead of returning the transmission loss, TL, return 
                    the complex pressure field, f, where TL = 20 * np.log10(np.abs(f)).
                    Default is False.
        ...
        self._init_output(num_sources=len(source_depth), rec_depth=rec_depth, nz_max=nz_max, nr_max=nr_max, size_limit=size_limit)
        ...
        if not return_field: tl_h = -20 * np.log10(np.abs(tl_h))
        q = self.grid.q
        if len(q) > 1: q = np.squeeze(q)    
        q = np.fft.fftshift(q)
        ...
        if not self._do_vertical: 
            ...
            if not return_field: tl_v = -20 * np.ma.log10(np.abs(tl_v))  # OBS: this computation is rather slow
    ...
    def _update_bathy(self, r):
        ...
        # temp fix to ensure that bathy returns array even when x and y have length 1
        if np.ndim(zb) == 0: zb = np.array([zb]) 
    ...
    def _init_output(self, num_sources, rec_depth, nz_max=250, nr_max=250, size_limit=False):
