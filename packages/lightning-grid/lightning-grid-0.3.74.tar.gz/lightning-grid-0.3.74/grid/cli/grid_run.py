...
def _check_is_valid_extension(ctx, _param, value):
    ...
    if value is not None:
        ...
        if not any(value.endswith(e) for e in valid_file_types):
...
@rich_click.argument(
    'script', type=click.Path(exists=True), callback=_check_is_valid_extension, help='The name of the script to run.'
)
@rich_click.argument('script_args', nargs=-1, type=click.UNPROCESSED, help='Arguments to be passed to the script.')
@click.pass_context
def run(
    ctx, script: str, entrypoint: str, config: Optional[Dict], name: Optional[str], cluster: Optional[str],
