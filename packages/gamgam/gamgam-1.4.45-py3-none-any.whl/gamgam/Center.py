...
# Var Area
global isGameRun, Console, StandardColor, SceneNow, Scenes, Objects,  ObjectWaiting, ComponentWaiting
isGameRun = True
SceneNow = 0
Scenes = []
object_level = 12
...
class TimeManager:
    ...
    def invoke(self,function, condition = True, repeat = False, delay:int = 1):
        if condition:
            self.invokeFunctions.append([function, t.time(), delay, repeat])
...
global Input, Console, StandardColor, default_icon, SoundManager, Time, fps_limit, Frame
Input = InputManager()
...
def run():
    global isGameRun, NowCamera, Objects, fps_limit, ObjectWaiting, ComponentWaiting
...
def find_GameObject_Num(game_object_id: str, scene:int = SceneNow):
    ...
    if num == 0:
        for i in range(len(GameObjects[scene])):
            if GameObjects[scene][i].name == game_object_id:
                num += 1
    return num
