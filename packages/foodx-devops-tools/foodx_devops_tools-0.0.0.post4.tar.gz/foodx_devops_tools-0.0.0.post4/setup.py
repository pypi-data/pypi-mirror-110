#!/usr/bin/env python
# setup.py generated by flit for tools that don't yet use PEP 517

from distutils.core import setup

packages = \
['foodx_devops_tools', 'foodx_devops_tools.pipeline_config']

package_data = \
{'': ['*']}

install_requires = \
['click == 7.1.2', 'pydantic == 1.8.2', 'ruamel.yaml == 0.17.9']

extras_require = \
{'dev': ['build_harness == 0.1.1', 'pre_commit == 2.7.1'],
 'doc': ['sphinx == 3.2.1', 'sphinx_rtd_theme == 0.5.0'],
 'test': ['pytest == 6.1.1', 'pytest-cov == 2.10.1', 'pytest-mock == 2.0.0']}

entry_points = \
{'console_scripts': ['release_flow = '
                     'foodx_devops_tools.release_flow:flit_entry',
                     'release_id = foodx_devops_tools.release_id:flit_entry']}

setup(name='foodx_devops_tools',
      version='0.0.0.post4',
      description='Foodx DevOps pipeline utilities.',
      author='FoodX Technologies',
      author_email='support@foodxtech.com',
      url='https://github.com/Food-X-Technologies/foodx_devops_tools',
      packages=packages,
      package_data=package_data,
      install_requires=install_requires,
      extras_require=extras_require,
      entry_points=entry_points,
      python_requires='>=3.8',
     )
