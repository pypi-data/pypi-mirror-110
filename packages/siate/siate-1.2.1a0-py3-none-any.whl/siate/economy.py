...
def compare_economy(tickers,measures,fromdate,todate,power=0,twinx=False):
    ...
    if isinstance(tickers,list): 
        ...
        if security_num == 0:
            print("  Error(compare_economy): country code not provided!")
            return
    ...
    if isinstance(measures,list): 
        ...
        if measure_num == 0:
            print("  Error(compare_economy): measure(s) not provided!")
            return
    ...
    if (security_num == 1) and (measure_num >= 2):
        ...
        footnote=ectranslate(measure1)+': '+list(df1['units'])[0]+', '+list(df1['freq'])[0]+ \
            ...
            '\nData source: OECD|IMF|WB|FRED, '+str(today)
        ...
        footnote='Notes: '+list(df1['units'])[0]+', '+list(df1['freq'])[0]+ \
            '\nData Source: OECD|IMF|WB|FRED, '+str(today)
        ...
        print("  Error(compare_economy): comparative contents not understandable")
        return None,None
...
def internal_growth_rate_df(df0,fieldlist=['VALUE','scope','factor','freq','name','units']):
    ...
    if freq in ['Daily']:
        date0=date0+'-'+str(df.index[0].day)+''
        date1=date1+'-'+str(df.index[-1].day)+''     
    
    #显示增长率
    #print('\n')
    print('From '+date0+' to '+date1+'：')
    if freq != 'Annual':
        print('    '+ectranslate(scope)+': '+ectranslate(factor)+' '+ectranslate(freq)+' chain growth：',round(item_gr*100.0,4),'\b%')
    print('    '+ectranslate(scope)+': '+ectranslate(factor)+' annual growth：',round(annual_gr*100.0,4),'\b%')
    return 
