...
def guide(data, K=1, tail=1, truncated_pareto = True, purity=0.96, clonal_beta_var=1., number_of_trials_clonal_mean=100.,
    ...
    for kr in pyro.plate("kr", len(karyos)):
        ...
        if theoretical_num_clones[kr] == 2:
            ...
            b_2_min = torch.cat([bmin_clonal.repeat(2), bmin_subclonal.repeat(K)], 0).repeat(counts_clones[2],1).transpose(1,0)
            b_2_max = torch.cat([bmax_clonal.repeat(2), bmax_subclonal.repeat(K)], 0).repeat(counts_clones[2],1).transpose(1,0)
            ...
            # get lower bound for number of trials
            # get lower bound for number of trials
            b_1_min = torch.cat([bmin_clonal.repeat(1), bmin_subclonal.repeat(K)], 0).repeat(counts_clones[1],
                                                                                             1).transpose(1, 0)
            b_1_max = torch.cat([bmax_clonal.repeat(1), bmax_subclonal.repeat(K)], 0).repeat(counts_clones[1],
                                                                                             1).transpose(1, 0)

            if K != 0:

                a_1_k = dist.Uniform(k_means_1 - 0.03, k_means_1 + 0.03).sample().reshape([K, counts_clones[1]])
                a_1_k = torch.max(a_1_k,torch.tensor([0.00001]))
