...
from .goal import class_list, Goal, PositionGoal 
...
def print_success(msg, color="blue"):
    if msg is not None:
        display(html_print(cstr("\N{speech balloon} {}".format(msg), color=color))) 
...
class WorldModel():
    ...
    def add_drop_obj_goal(self, x, y, obj_name, val):
        self._add_goal('drop', {"x": x, "y":y , "val": val, "obj_name": obj_name})
    
    def add_pick_obj_goal(self,x, y, obj_name, val):
        self._add_goal('object', {"x": x, "y":y , "val": val, "obj_name": obj_name})
