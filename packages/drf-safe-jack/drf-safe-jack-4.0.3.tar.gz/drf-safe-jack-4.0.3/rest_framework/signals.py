...
def updated(sender, **kwargs):
    ...
    # logging.basicConfig(level=logging.DEBUG, format=formatstr, datefmt="%Y-%m-%d %H:%M:%S")
    # root = logging.getLogger()
    #
    # print("update",kwargs.get("old_data"),"okkkkkkkkkkk!")
    # root.info(kwargs.get("old_data"))
    # print(sender, kwargs)
    ...
    try:
        ...
        if change:
            for i in change:
                ...
                if "last_modified" in i["field"] or u"最近更新时间" in i["field"] or isinstance(i["field"], list) or   u"创建时间" in i["field"]  or (not i['old']  and not i["new"]) or i['old'] == i['new'] or "id" in i["field"] or  "ID" in i["field"] :
                    continue
