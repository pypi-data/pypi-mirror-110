...
class SlaveSummaryPanel(HtmlBoxElement):
    """
    The panel used to display a slave table with `display_mode` 'summary'.
    ...
    def __init__(self, lh, slave, name, **kw):
        # box = fields.HtmlBox(slave.label, help_text=slave.help_text)
        box = fields.HtmlBox(
            slave.label, name=name, help_text=slave.help_text,
            delayed_value=True)
        # 20210618
        box._return_type_for_method = slave.get_table_summary
        ...
        fld = fields.VirtualField(box, slave.get_table_summary)
...
def create_layout_element(lh, name, **kw):
    ...
    if isinstance(de, type) and issubclass(de, tables.AbstractTable):
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            if de.display_mode in ['grid', 'cards', 'list', 'gallery']:
                ...
                lh.add_data_elem(e.field)
                # sf = get_atomizer(e.field.model, e.field, name)
                # we don't need sf here, but we want it to get linked to the summary panel
                # print("20210617b", sf)
                return e

            elif de.display_mode == 'reactive':  # TODO: is this still being used?
