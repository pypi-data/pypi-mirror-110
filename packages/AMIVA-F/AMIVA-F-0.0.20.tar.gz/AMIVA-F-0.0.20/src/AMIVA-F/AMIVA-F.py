...
class Entry_formular:
    def __init__(self, master):
        ...
        self.header = tk.Label(self.frame, text="AMIVA-F analysis of mutations in variants of human Filamin C", font="Helvetica 16 bold", fg="black")
        self.header.grid(row=0, column=0, sticky="nswe", ipady=5)
        self.label1 = tk.Label(self.frame, text="Please enter your mutation of interest into the entry field below. \n"
                                                "Accepted format: e.g M82K \n"
                                                "for Methionine being substituted by Lysine in the mutation at position"
                                                " 82 according to UniProt Q14315")
        self.label1.grid(row=1, column=0, ipadx=10, columnspan=2, sticky="news")
        ...
        self.lblexplain = tk.Label(self.frame, text="After entering your desired mutation"
                                                    ", the green button will calculate and fill in all parameters")
        self.lblexplain.grid(row=6, column=0)
        self.button = tk.Button(self.frame, text="Calculate everything for me!"
                                , relief="ridge", borderwidth=4, bg="green", fg="white",
                                command=self.hydrophobicity_atomnumb)
    ...
    def hydrophobicity_atomnumb(self):
        ...
        aa_wt = entry_start[0:1]
        aa_mut = entry_start[-1:]
        global pos
        pos = entry_start[1:-1]
...
class Entryfield:
    def __init__(self, master):
        ...
        for idx, entries in enumerate(attributeshown[:6]):
            ...
            self.label.grid(column=0, row=idx, sticky="e")
            self.entry.grid(column=1, row=idx, sticky="w")
        ...
        self.clearbtn = tk.Button(self.frame5, text="Clear", command=self.clear_btn, borderwidth=4, width=10)
        self.clearbtn.grid(row=1, column=3, sticky="w")

        self.subbutnlbl = tk.Label(self.frame5, text="Click here to generate a prediction ready WEKA file -------->")
        ...
        self.tutbutton = tk.Button(self.frame5, text="Read Tutorial", command=self.manual, borderwidth=4, width=20)
        self.tutbutton.grid(row=1,column=4, sticky="w")
        ...
        self.extndinfo = tk.Label(self.frame5, text="Eibe Frank, Mark A. Hall, and Ian H. Witten (2016).\n The WEKA Workbench. "
                                                    ...
                                                    "\n\nFor this part you require a generated template file (blue button above)")
        ...
        self.frame6.grid(row = 16, column=0, pady=(50,10), columnspan=4)

        self.frame6_1 = self.frame6 = tk.Frame(self.frame)
        ...
        self.wekabtn = tk.Button(self.frame6_1, text="Prediction on\n pathogenicity", borderwidth=4, bg="red", fg="white",
        ...
        self.wekabtnmutantpdb = tk.Button(self.frame6_1, text="Clear", borderwidth=4, command=self.clearweka)
        self.wekabtnmutantpdb.grid(row=3, column=2, sticky="we")
        self.wekabtnoutputfile = tk.Button(self.frame6_1, text="Outputfile mutation predictions", borderwidth=4, command=self.outputfilemutant)
        self.wekabtnoutputfile.grid(row=3, column=3, sticky="we")
        ...
        self.wekaentryid.grid(row=5, column=1, sticky="nsew")
        self.wekaentrydomaininfo = tk.Entry(self.frame6, width=10)
        self.wekaentrydomaininfo.grid(row=5, column=2, sticky="nseww")
        self.wekaentrydis = tk.Entry(self.frame6, width=15)
        self.wekaentrydis.grid(row=5, column=3, sticky="nswe")
        #self.wekaentryconf = tk.Entry(self.frame6, width=5)
        #self.wekaentryconf.grid(row=5, column=4, sticky="nsew")

        self.wekaentryidlbl = tk.Label(self.frame6, text="mutation", font="Helvetica 13")
        self.wekaentryidlbl.grid(row=4, column=1, pady=(20,0))
        self.wekaentrydomainlbl = tk.Label(self.frame6, text="domain", font="Helvetica 13")
        self.wekaentrydomainlbl.grid(row=4, column=2, pady=(20, 0))
        self.wekaentrydiseaselbl = tk.Label(self.frame6, text="pathogenicity", font="Helvetica 13")
        self.wekaentrydiseaselbl.grid(row=4, column=3, pady=(20, 0))
    ...
    def manual(self):
        ...
        window.title("Manual for AMIVA-F")
        s = tk.Scrollbar(window)
        exp = tk.Text(window, width=200, height=30)
        s.pack(side=tk.RIGHT, fill=tk.Y)
        exp.pack(side=tk.LEFT, fill=tk.Y)
        s.config(command=exp.yview)
        exp.config(yscrollcommand=s.set)
    ...
    def outputfilemutant(self):
        ...
        dirtarget = "./WEKA_associated_files/" + mutfilename + ".txt"
