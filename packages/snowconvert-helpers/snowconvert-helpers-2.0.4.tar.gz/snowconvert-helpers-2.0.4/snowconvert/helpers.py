...
def get_from_args_or_environment(arg_pos, env_variable_name, args):
    """Gets the argument from the position specified or gets the environment variable name passed.

        arg_pos
            The argument position to be used from the arguments parameter.
        env_variable_name
            The name of the environment variable to be obtained.
        args
...
def exec(sql_string, using=None, con=None):
    ...
    try:
        ...
        if current_error_count >= max_errors:
            print(colored(f'*** Failure: reached max error count {max_errors}', 'red'))
            quit_application(1)
...
class Export:
    ...
    @staticmethod
    @deprecated(reason="This function will be removed soon", version='1.9')
