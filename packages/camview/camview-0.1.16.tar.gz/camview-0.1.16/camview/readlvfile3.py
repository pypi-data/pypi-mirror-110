...
def lvSummary(filenames,o2flow=1,coflow=2,coconc=0.1,kelvin=True,desc=None):
    ...
    table=[]
    headerRow=['Filename','Start time','Temperature Range','Current Range','Pressure','O<sub>2</sub> Partial', 'CO Partial','O<sub>2</sub>:CO Ratio','Description']
    ...
    for j,file in enumerate(filenames):
        ...
        dataRow=[file,lvData['timeline'],\
                 ...
                 '{} mbar'.format(o2Partial),'{} mbar'.format(coPartial),'{:.1f}:1'.format(np.round(o2Partial/coPartial,1))]
        if desc is not None:
            dataRow.append(desc[j])
