...
async def start_gateway(descriptor: FileDescriptor, grpc_port: int, http_port: int = 7070):
  try:
    ...
    channel = insecure_channel(f'localhost:{grpc_port}', options=client_options)
    config = Config()
    ...
    logging.info(f'started gateway on port {http_port}')
