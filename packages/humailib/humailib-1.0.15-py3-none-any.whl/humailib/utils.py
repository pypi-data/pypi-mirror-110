...
def get_papermill_param(param, type_str='str'):
    
    if type_str == 'int':
        return param if param is not None else 0
    elif type_str =='float':
        return param if param is not None else 0.0
    elif type_str == 'list':
...
def keep_subset_customers(dataframes, cid_column, k, seed=1):
    ...
    for df in dataframes_in:
        ...
        dataframes_out.append( df.drop(index=df[df.keep == 0].index), sort=True, ignore_index=True )
