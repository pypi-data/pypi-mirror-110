...
class Bdrmapit(FirstHopMixin, LastHopsMixin, VRFMixin, RegexMixin, DebugMixin, HelpersMixin):
    ...
    def __init__(self, graph: Graph, as2org: AS2Org, bgp: BGP, ixpasns=None, strict=False, skipua=False, hidden_reverse=True, norelpeer: Set[int]=None, interfaces: Collection[Interface]=None):
        ...
        self.nodes = sorted(chain(self.routers_succ, self.interfaces_pred), key=lambda x: len(x.succ if isinstance(x, Router) else x.pred), reverse=True)
    ...
    def router_heuristics(self, router: Router, isucc: Interface, origins: Set[int], iasns: TCounter[int]):
        ...
        rsucc_asn = self.rupdates.asn(rsucc) if rsucc in self.rupdates else rsucc.interfaces[0].asn  # subsequent router AS annotation
    ...
    def annotate_interface(self, interface: Interface):
        ...
        for rpred, num in edges.items():
            asn = self.rupdates.asn(rpred) if rpred in self.rupdates else rpred.interfaces[0].asn
            if asn <= -100:
                ixpasns = self.ixpasns.get(asn)
                if ixpasns is not None and interface.asn in ixpasns:
                    asn = interface.asn
        ...
        if len(votes) == 1:
            ...
            if interface.asn in votes:
                if votes[interface.asn] / sum(votes.values()) >= .25:
                    return interface.asn, 33
            maxvote = max(votes, key=votes.__getitem__)
            if votes[maxvote] / sum(votes.values()) >= .75:
                return maxvote, 35
            # if len(votes) >= 3:
            #     return interface.asn, 40
            rels = [asn for asn in votes if
                    interface.asn == asn or self.bgp.rel(interface.asn, asn) or asn in self.norelpeer]
            if not rels:
                rels = votes
            if debug.DEBUG:
    ...
    def graph_refinement(self, nodes=None):
        if nodes is None:
            nodes = self.nodes
        pb = Progress(len(nodes), increment=100000)
        for node in pb.iterator(nodes):
            if isinstance(node, Router):
                asn, utype = self.annotate_router(node)
                self.rupdates.add_update_direct(node, asn, self.as2org[asn], utype)
            else:
                asn, utype = self.annotate_interface(node)
                self.iupdates.add_update_direct(node, asn, self.as2org[asn], utype)
