...
class MLMethod(ClassificationMethod):
    ...
    def __init__(self, model_id: Union[UUID, str], threshold: Optional[float] = None):
        ...
        if threshold is not None and threshold < 0:
            raise DtlError(
                "Invalid threshold. Confidence scores are positive numbers. Please set "
                "a threshold within that range."
            )

    def __repr__(self):
        return "MLMethod(\n " f"model_id= {self.model_id}, " f"threshold= {self.threshold}\n)"
...
class Classifier(object):
    ...
    def __repr__(self):
        return (
            "Classifier(" + (f"id={self.id!r}, " if self.id is not None else "") + f"name={self.name!r}, "
