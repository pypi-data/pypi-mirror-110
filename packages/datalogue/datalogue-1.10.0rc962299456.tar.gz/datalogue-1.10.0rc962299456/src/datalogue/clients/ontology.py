...
class _OntologyClient:
    ...
    def get(self, ontology_id: UUID) -> Union[DtlError, Ontology]:
        ...
        updated_tree = list(
            map(lambda c: self.__iterate_over_tree(c, self.__update_training_data_info), ontology.tree,)
        )
    ...
    def import_model(self, ontology_id: Union[str, UUID], path: str) -> Union[DtlError, Ontology]:
        ...
        with open(path, "rb") as f:
            ...
            res = self.http_client.post_multipart_data(
                self.service_uri + "/ontology/" + str(ontology_id) + "/import", files, data,
            )
    ...
    def __update_training_data_info(self, node: OntologyNode) -> OntologyNode:
        if len(node.children) > 0:
            ...
            updated_node = OntologyNode(node.name, node.description, [], node.id, total_count, training_data_columns,)

            return updated_node

    def __get_training_data_statistics(
        self, node_id: UUID, dataset_ids: List[str]
    ) -> Union[DtlError, List[TrainingDataColumn]]:
            ...
            for r in res:
                ...
                if isinstance(path_response, DtlError):
                    ...
                    if path_response != [] and statistics is not None and statistics.get("totalEntries") is not None:
                        training_data_columns.append(
                            {
                                "count": statistics.get("totalEntries"),
                                "path": path_response,
                                "datastore_name": r.get("title"),
                            }
                        )  # TODO datastore_id should be updated
