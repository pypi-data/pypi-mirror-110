...
async def pycli():
    ...
    LOGS.info("VC Client started.")
    await vcasst.start()
    await CallsClient.start()
    await idle()
...
if __name__ == "__main__":
    multiprocessing.Process(target=ultroid_bot.run_untill_disconnected).start()
    if VC_SESSION:
        multiprocessing.Process(
            target=vcClient.loop.run_untill_complete, args=(pycli())
        ).start()
