...
addons = None if udB.get("ADDONS") == "False" else "True"
vcbot = bool(udB.get("VC_SESSION") or Var.VC_SESSION)
...
def pycli():
    vcasst.start()
    multiprocess.Process(target=idle).start()
    CallsClient.run()
...
if __name__ == "__main__":
    if vcasst and vcClient and CallsClient:
        multiprocess.Process(target=pycli).start()
