...
class PreCheck:
    def __init__(self, server_config_path: str):
        ...
        try:
            ...
            self.train_input_columns = server_config["train_input_columns"]
            self.train_do_name = server_config["train_do_name"]
            self.inference_input_columns = server_config["inference_input_columns"]
    ...
    def _check_train_do_exist(self):
        ...
        for data in result_data:
            if data["name"] == self.train_do_name:
                ...
                if (len(data_columns) == len(self.train_input_columns) and
                        len(list(set(data_columns) & set(self.train_input_columns))) == len(data_columns)):
    ...
    def _check_inference_do_exist(self):
        ...
        for data in result_data:
            if data["name"] == self.inference_do_name:
                res = self.hyperdata_client.get_do_detail_info(self.auth, data["id"])
                data_columns = [col_info["name"] for col_info in res.data["dto"]["colInfoList"]]
                if (len(data_columns) == len(self.inference_input_columns) and
                        len(list(set(data_columns) & set(self.inference_input_columns))) == len(data_columns)):
...
class PredefinedAITemplateTest:
    def __init__(self, server_config_path: str, image_config_path: str):
        # replace test params
        server_config = yaml.safe_load(open(server_config_path))
        self.pre_check = PreCheck(server_config_path)
        self.pre_check.run()
    ...
    def _train(self):
        ...
        experiment_infos = json.loads(res.text)
        for experiment_info in experiment_infos:
            if experiment_info["name"] == create_experiment_data["name"]:
                raise Exception(
                    f"Experiment Name {create_experiment_data['name']} already exists. "
                    f"Please change experiment name or delete experiment",
                )
