...
def update_directories_api(api_name, max_days=1):
    ...
    create_directories([temp_folder, local_folder, PLACEHOLDER_FOLDER])
    update_placeholders()
...
def update_placeholders():
    ...
    for file in list_files(url, IMG_EXTENSIONS):
        if check_too_old(PLACEHOLDER_FOLDER, max_age):
            download_install_media(url, TEMP_FOLDER, 'placeholders')
