...
def parse_flights(flights, destinations, response):
    ...
    for flight_info in response.get('flights', []):
        ...
        flight['departure_time'] = get_departure_time(flight)
        flight['flight_number'] = get_flight_number(flight)
        flight['status'] = get_flight_status(flight)
