...
class Test_graph_length_score_no_transformation(unittest.TestCase):

    @classmethod
    def setUpClass(self):
        sf_enum = ScoringFunctionComponentNameEnum()
        csp_enum = ComponentSpecificParametersEnum()
        ts_parameters = ComponentParameters(component_type=sf_enum.GRAPH_LENGTH,
                                            name="graph length",
                                            weight=1.,
                                            smiles=[],
                                            model_path="",
    ...
    def test_graph_length_1(self):
        ...
        values = np.array([3,10,11,13,21])
        score = self.sf_state.get_final_score(smiles=smiles)
        npt.assert_array_almost_equal(score.total_score, values, 2)

class Test_graph_length_score_with_double_sigmoid(unittest.TestCase):
    ...
    def test_graph_length_1(self):
        ...
        values = np.array([0.000, 0.500, 0.863, 0.996, 0.137])
        score = self.sf_state.get_final_score(smiles=smiles)
        npt.assert_array_almost_equal(score.total_score, values, 2)
