...
class Test_pip_hlm_clint(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_scores = [1.171, 0.715, 1.038]
...
class Test_pip_hlm_clint_transformation(unittest.TestCase):
    def setUp(cls):
        ...
        enum = ScoringFunctionComponentNameEnum()
        parameters = create_c_lab_component(enum.HLM_CLINT_PIP)
        parameters.specific_parameters[csp_enum.TRANSFORMATION_TYPE] = transf_type.STEP
        parameters.specific_parameters[csp_enum.LOW] = 1.0
        if not os.path.isdir(MAIN_TEST_PATH):
            os.makedirs(MAIN_TEST_PATH)

        cls.query_smiles = [celecoxib, 'CCC', "CCCCC"]
        cls.expected_raw_scores = [1.171, 0.715, 1.038]
