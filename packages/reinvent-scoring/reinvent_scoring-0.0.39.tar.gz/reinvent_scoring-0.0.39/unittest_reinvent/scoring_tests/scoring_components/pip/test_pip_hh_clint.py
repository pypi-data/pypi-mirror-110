...
class Test_pip_hh_clint(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        enum = ScoringFunctionComponentNameEnum()
        csp_enum = ComponentSpecificParametersEnum()
        parameters = create_c_lab_component(enum.HH_CLINT_PIP)
...
class Test_pip_hh_clint_transformation(unittest.TestCase):
    def setUp(cls):
        csp_enum = ComponentSpecificParametersEnum()
        transf_type = TransformationTypeEnum()
        enum = ScoringFunctionComponentNameEnum()
        parameters = create_c_lab_component(enum.HH_CLINT_PIP)
        parameters.specific_parameters[csp_enum.TRANSFORMATION_TYPE] = transf_type.LEFT_STEP
        parameters.specific_parameters[csp_enum.LOW] = 1.33
        if not os.path.isdir(MAIN_TEST_PATH):
            os.makedirs(MAIN_TEST_PATH)

        cls.query_smiles = [celecoxib, 'CCC', "CCCCC"]
        cls.expected_raw_scores = [1.3, 1.06, 1.324]
        cls.expected_scores = [1.0, 1.0, 1.0]
        cls.component = PiPLogPredictionComponent(parameters)
