...
class Test_pip_caco2_intr(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        enum = ScoringFunctionComponentNameEnum()
        csp_enum = ComponentSpecificParametersEnum()
        parameters = create_c_lab_component(enum.CACO2_INTR_PIP)
        ...
        cls.expected_scores = [1.7, 1.1, 1.1]
...
class Test_pip_caco2_intr_transformation(unittest.TestCase):
    def setUp(cls):
        enum = ScoringFunctionComponentNameEnum()
        parameters = create_c_lab_component(enum.CACO2_INTR_PIP)
        if not os.path.isdir(MAIN_TEST_PATH):
        ...
        cls.query_smiles = [celecoxib, 'CCC', "CCCCC"]
        cls.expected_raw_scores = [1.7, 0.9, 1.]
        cls.expected_scores = [0.004, 0.002, 0.001]
        cls.component = PiPLogPredictionComponent(parameters)
