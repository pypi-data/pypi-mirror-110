...
class Test_pip_caco2_efflux(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_scores = [0.8, 1.6, 1.5]
...
class Test_pip_caco2_efflux_transformation(unittest.TestCase):
    def setUp(cls):
        ...
        csp_enum = ComponentSpecificParametersEnum()
        parameters = create_c_lab_component(enum.CACO2_EFFLUX_PIP)
        parameters.specific_parameters[csp_enum.HIGH] = 3
        parameters.specific_parameters[csp_enum.LOW] = 0

        if not os.path.isdir(MAIN_TEST_PATH):
            os.makedirs(MAIN_TEST_PATH)

        cls.query_smiles = [celecoxib, "CCC", "CCCCC"]
        cls.expected_scores = [0.795, 1.587, 1.55]
