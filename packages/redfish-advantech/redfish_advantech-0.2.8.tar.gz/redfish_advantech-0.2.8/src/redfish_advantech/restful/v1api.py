...
class redfish_advantech:
    ...
    def rfRequest(self, log=True):
        if (self.authToken == ''):  # for login only
            headers = {'Accept': '*/*',
        ...
        # except:
        #    self.logger.error(
        #        'Unknown exception when create the http self.connection and exit()')
        #    exit()
    ...
    def logout(self):
        ...
        if (self.authToken != ''):
            ...
            if response.status not in [200, 202, 204]:
                self.logger.info("Invalid session resource: %s, return code: %d" % (self.url, response.status))
    ...
    def disconnect(self):
        ...
        if (self.connection):
            try:
                ...
                if (ret == None):
                    self.logger.info('http self.connection closed successfully')
                else:
                    logging.error('http self.connection closed failed with ', ret)
            except:
                logging.error('Unknown exception when close the http self.connection')
        else:
                self.logger.info('http self.connection is not connected. No need to close it.')
        self.connection = None
    ...
    def getSystems0EthernetInterfaces(self):
        if not (self.urlEthernetInterfaces == ''):
