...
class RestClientBase(object):
    ...
    def _rest_request(self, path, method='GET', args=None, body=None,
        ...
        """
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": rest_request: {} {}".format(method, path))
        headers = self._get_req_headers(headers)
        reqpath = path.replace('//', '/')
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": headers：", headers) # CH Huang
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": reqpath：", reqpath) # CH Huang
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": body：", body) # CH Huang
        ...
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": args：", args) # CH Huang
        if args:
            if method == 'GET':
        ...
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": restreq：", restreq) # CH Huang
        print(inspect.getframeinfo(inspect.currentframe()).lineno, ": === end of restreq") # CH Huang
        ...
        while attempts <= self._max_retry:
            ...
            try:
                while True:
                    ...
                    print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Inside '_rest_request' checking response details")
                    #print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp.status=", resp.status)
                    #print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp.msg=", resp.msg)
                    #print(inspect.getframeinfo(inspect.currentframe()).lineno, ": === End of resp.msg")
                    #print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp.headers=", resp.headers)
                    #print(inspect.getframeinfo(inspect.currentframe()).lineno, ": === End of resp.headers")
                    # redirect handling
                    if resp.status not in list(range(300, 399)) or \
                        resp.status == 304 or skip_redirect is True:
                        if skip_redirect is True:
                            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Skipping redirects")
                        break
                    newloc = resp.getheader('location')
                    newurl = urlparse(newloc)
                    #if resp.status == 303: # replace this line by next line by author
                    if resp.status in [301, 302, 303]:
                        method = 'GET'
                        body = None
                        # headers['X-Auth-Token'] = resp.getheader('X-Auth-Token') ## add by CH Huang
                ...
                print(inspect.getframeinfo(inspect.currentframe()).lineno, ": restresp：", restresp) # CH Huang
                print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp：", resp) # CH Huang
    ...
    def login(self, username=None, password=None, auth=AuthMethod.SESSION):
        ...
        if auth == AuthMethod.BASIC:
            ...
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Inside 'login' after _rest_request")
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp.status=", resp.status)
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": resp.getheaders()=", resp.getheaders())
            ...
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Key {}".format(self.__session_key))
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Location {}".format(self.__session_location))
            print(inspect.getframeinfo(inspect.currentframe()).lineno, ": Exit 'login'")
...
def restful_client(base_url=None, username=None, password=None,
                                default_prefix='/redfish/v1/',
                                sessionkey=None, capath=None,
                                cafile=None, timeout=None,
                                max_retry=None):
