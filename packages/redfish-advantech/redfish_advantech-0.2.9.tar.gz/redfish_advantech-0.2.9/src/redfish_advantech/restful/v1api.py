...
class redfish_advantech:
        ...
        try:
            if (log):
                ...
                if (self.payload == None):
                    if (self.logger.isEnabledFor(logging.DEBUG) and self.get_logVerbose() >= 0):
    ...
    def login(self):
        ...
        if (self.logger.isEnabledFor(logging.DEBUG) and self.get_logVerbose() >= 1):
    ...
    def getSystems0LogServicesLogEntries(self):
        if (self.urlLogEntries != ''):
            ...
            response = self.rfRequest()
            result = response.read().decode(errors='replace')
            if (self.logger.isEnabledFor(logging.DEBUG) and self.get_logVerbose() >= 2):
                self.logger.debug(result)
            if (response.getcode() == 200):
                json_data = json.loads(result)
                for i in json_data.items():
