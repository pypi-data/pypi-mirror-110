...
def route_feedback(
    ...
    # TODO: Add grade info at this stage?
...
def put_inflight(course, semester, username, phase, psid, taskid):
    ...
    istring = "{username}-{phase}-{psid}-{taskid}-{timestamp}-{rand}".format(
        username=username,
        phase=phase,
        psid=psid,
        taskid=taskid,
    ...
    logfile = unused_filename(
        flask.safe_join(logs_folder(course, semester), istring + ".log")
    )

    reportfile = unused_filename(
        flask.safe_join(reports_folder(course, semester), istring + ".json")
    )
...
def get_feedback_summary(
    ...
    # TODO: What if get_inflight fails?
