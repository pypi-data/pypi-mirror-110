...
class Rubric:
    ...
    def evaluate(self, task_info, username, submission_target):
        ...
        if os.path.isdir(submission_target):
            ...
            submission_root, default_file = os.path.split(submission_target)
        base_context = {
            "task_info": task_info,
            "username": username,
        ...
        all_filenames = set([default_file])
...
def overall_evaluation(foundational, core, extra):
    ...
    if core_score == len(core):
        ...
        if len(extra_unaccomplished) == 0:
            ...
            return (
                ...
                (
                  ...
                  + "<p>You accomplished {} of {} extra goals.</p>"
                ).format(
                    len(extra) - len(extra_unaccomplished),
                    len(extra)
                )
            )
        ...
        return (
            "almost complete",
            "<p>You accomplished nearly all of the {} core goals.</p>".format(
                len(core)
            )
        )
        ...
        if core_score >= half:
            return (
                "partially complete",
                (
                    "<p>You accomplished at least {} of the {} core"
                  + " goals.</p>"
                ).format(half, len(core))
            )
        else:
            return (
                "incomplete",
                (
                    "<p>You accomplished less than {} of the {} core"
                  + " goals.</p>"
                ).format(half, len(core))
            )
...
class JointGoal(Goal):
    ...
    def evaluate_in_context(self, context=None):
        ...
        for subgoal in self.parts:
            ...
            result = subgoal.evaluate(task, sub_context)
            goals_evaluated += 1
...
class FailGoal(Goal):
    ...
    def evaluate_in_context(self, context=None):
        ...
        context = context or {}
        self.result = {}
        self.result.update(self.goal.evaluate(task, context))
...
class PreconditionGoal(Goal):
    ...
    def evaluate_in_context(self, context={}):
        ...
        """
        pre = self.precondition.evaluate(task, context)
        if pre.get("status") != "accomplished":
            ...
            self.result = self.goal.evaluate(task, context)
...
class ImplementationCheck(Goal):
    ...
    def evaluate_in_context(self, context=None):
        ...
        """
        context = context or {}
        # Create sub-context and figure out scope:
        sub_context = {}
        sub_context.update(context)
        if sub_context.get("scope") is not None:
            scope = sub_context["scope"]
        ...
        # Our match filter:
        match_filter = lambda node, env: my_match(task["code"], node, env)

        # Define match-collecting function
        def collect_matches(scope, memo=None):
            """
        ...
        for (node, envs) in matches:
            for env in envs:
                ...
                for rule in self.subrules:
                    ...
                    # evaluate sub-rule
                    sub_result = rule.evaluate(task, this_sub_context)
        ...
        for n, envs in full_matches:
            identity_or_identities = self.match_identity(task["code"], n, envs)
            ...
            if self.softmax:
                if isinstance(self.softmax, (str, list, tuple)):
                    if "note" in self.softmax:
                        notes.append(
                            "Found more {} than expected.".format(self.pl_name)
                        )
        ...
        for node, envs in full_matches:
            matching_lines.append(mast.node_line(node))
        partial_lines = []
        for node, envs in partial_matches:
            partial_lines.append(mast.node_line(node))
        ...
        explanation = "Expected {quantity} {name}, found {found}{sub}.".format(
            quantity=quantity,
            ...
            sub=(
                ...
                extra_unrefined,
                "matches" if extra_unrefined != 1 else "match",
                "the sub-rule" if len(self.subrules) == 1
                else "all {} sub-rules".format(len(self.subrules))
        ...
        if len(matching_lines) > 0:
            notes.append(
                "Found on line(s): {}".format(
                    ', '.join(
                        html_tools.html_link_to_line(task["taskid"], ln)
...
class NoParseErrors(Goal):
    ...
    def evaluate_in_context(self, context=None):
        ...
        """
        context = context or {}
        if task["parse_errors"] is None or len(task["parse_errors"]) == 0:
            self.result = { "status": "accomplished" }
            self.set_explanation(
                context,
