...
def setup(templates_directory, resources_directory):
    """
...
def render_report(report, output_file):
    """
    Renders an evaluation report object as HTML into the specified output
    file. The report should come from `potluck.rubrics.Rubric.evaluate`.
    """
    # Load our report template
    template = load_template(EVALUATION_REPORT_TEMPLATE)

    # Augment the given rubric report object
    augment_report(report)

    # Render the report to the output file
    with open(output_file, 'w', encoding="utf-8") as fout:
        fout.write(template.render(report))
...
def annotate_line(taskid, filename, lineno, line_html):
    ...
    return (
        ...
        f' onclick="focusCodeLine(\'{taskid}_code_{filename}\', \'{lineid}\');'
        f' return false">'
