...
```py
from potluck.evaluation import specifications as spec
from potluck.evaluation import compare
from potluck.evaluation import harness
...
spec.TestCase("polygon", (40, 6), group_name="advanced").do_setup(
    lambda module, fname, posargs, kwargs: (turtle.lt(45), turtle.fd(20))
)
...
traces.also()\
    .goal("core")\
    .check_invariant(["pos", "heading"], only=["polygon"])
adv_traces.also()\
    .goal("extra")\
    .check_invariant(["pos", "heading"], only=["polygon"])
...
# `potluck.mast` style (e.g., "firstArg, _, thirdArg").
...
class HasPayload:
    ...
    def capture_output(self, capture_errors=False):
        ...
        instead of actually breaking the test.
        """
...
class HasGoal:
    ...
    def check_invariant(
        ...
        # TODO: MUCH better than this (use an omni-compare!)
        fequals = compare.build_float_equality_checker()
    ...
    def refine(self, refiner_class, **refiner_args):
        """
        ...
        provide as keyword arguments any extra input that that refiner
        requires for initialization, beyond parent object. This function
        returns the new `RefinedTest` instance for chaining.
        ...
        """
        return refiner_class(parent=self, **refiner_args)
...
class Check:
    ...
    simplified/specialized ImplementationChecks. Each Check should be
    ...
    def __init__(
        ...
        """
        A check needs a list of patterns (could be length-1), a tuple of
        limits (min/max, either or even both may be None), and it may
        ...
        other `ImplementationCheck` details are specified via calling
        methods on the object after it is created. The name may be a
        string containing HTML code, or a 2-item tuple to explicitly
