...
def evaluate_submission(
    ...
    # Now that we've got a rubric report, write it to the report file.
    logging.log(f"Rendering report to '{report_filename}'...")
    report.render_report(evaluation, report_filename)
...
def launch_rubric_generation(
    ...
    # Set up logging
    if log_file is None:
        log_file = file_utils.unused_filename(
            os.path.join(
                config.BASE_DIR,
                "logs",
                f"{taskid}-rubric-0.log"
            )
        )
    ...
    with open(log_file, 'w') as lout:
        ...
        # Load task meta-data
        import json
        task_info_file = os.path.join(config.BASE_DIR, config.TASKS_FILENAME)
...
def launch_evaluation(
    ...
    with open(log_file, 'w') as lout:
        ...
        if target_file is not None: # explicit
            ...
            user_folder = None
            task_folder = None
            logging.log(f"Submission is (explicit): {submission_target}")
            ...
            logging.log(f"Submission is (implicit): {submission_target}")

        # Fatal error if the submission file/directory doesn't exist
        if not os.path.exists(submission_target):
            logging.log(
                "Fatal error: Submission file (or folder) does not exist"
            )
