...
import json
import logging
from rss_reader_palina_yudzina.display_method import NewsItemDisplay
from dataclasses import asdict
from rss_reader_palina_yudzina.validation_functions import check_limit_arg
...
def print_news_stdout(news_collection):
    """
    Function for printing news to stdout in text format.
    """
    if news_collection:
        for news_item in news_collection:
            news_item.print_news_item()


def print_news_json(news_collection):
    """
    Function for printing news to stdout in json format.
    """
    news_collection_for_json = []

    for news_item in news_collection:
        news_item_for_json = asdict(news_item)
        news_collection_for_json.append(news_item_for_json)
...
def print_news(news_collection, com_line_args, logger):
    """
    Function for printing news to stdout,
    that takes account of json argument.
    """
