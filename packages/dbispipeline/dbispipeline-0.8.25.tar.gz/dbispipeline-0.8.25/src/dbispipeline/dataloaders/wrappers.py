...
def limiting(cls, df_grouping_field=None):
    """
    ...
    def new_init(self,
        ...
        Args:
            ...
            random_seed: Seed to provide to random.seed().
        """
        old_init(self, *args, **kwargs)
    ...
    def new_load(self):
        ...
        targets = _get_targets(dataset['train'], self.max_targets, self.strategy)
        result = [_limit_to_targets(dataset['train'], targets, self.max_documents_per_target)]
        if 'validate' in dataset:
            result.append(_limit_to_targets(dataset['validate'], targets, self.max_documents_per_target))
        if 'test' in dataset:
            result.append(_limit_to_targets(dataset['test'], targets, self.max_documents_per_target))
        ...
        #
        #     if type(pair[0]) == pd.DataFrame:
        #         entry = pair[0].groupby(df_grouping_field)
        #
...
def _get_targets(dataset_part, limit, strategy):
    _, y = dataset_part
    if limit:
        if strategy != 'random':
            raise ValueError('No longer supported. Many explainability issues.')
        if limit >= len(set(y)):
            warnings.warn(f'sample_limit ({limit}) >= targets ({len(set(y))})')
        return random.sample(set(y), limit)
    else:
        return list(set(y))


def _limit_to_targets(dataset_part, remaining_targets, max_documents_per_target):
    if type(dataset_part[0]) == pd.DataFrame:
        df = dataset_part[0]
        key_i = 0
        key = f'y_{key_i}'
        while key in df.columns:
            key_i += 1
            key = f'y_{key_i}'
        df[key] = dataset_part[1]
        sub_df = df[df[key].isin(remaining_targets)]
        if max_documents_per_target:
            sub_df = sub_df.groupby(key).sample(max_documents_per_target)
        return sub_df.drop(columns=key), sub_df[key].values
    else:
        result = zip(*[(x, y) for (x, y) in zip(*dataset_part)
