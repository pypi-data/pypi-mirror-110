...
from azureml.responsibleai.common._errors.error_definitions import (
    AnalysisInitNotExecutedError, AnalysisInitFailedError)
from azureml.responsibleai.common.model_loader import ModelLoader

from ._aml_init_dto import AMLInitDTO
from ._model_analysis_settings import ModelAnalysisSettings
from .model_analysis_run import ModelAnalysisRun
from ._constants import DEFAULT_MAXIMUM_ROWS_FOR_TEST_DATASET
...
class ModelAnalysisConfig:
    ...
    def __init__(self,
                 ...
                 column_transformer: Optional[Any] = None,
        ...
        :param column_transformer: The data transformers to apply to the data before passing it to the model.
    ...
    def azureml_is_initialized(self):
        ...
        if not self._azureml_is_initialized:
            if self._model_analysis_run:
                status = self._model_analysis_run.get_status()
                if status in [RunStatus.FAILED, RunStatus.CANCELED, RunStatus.CANCEL_REQUESTED]:
                    raise UserErrorException._with_error(
                        AzureMLError.create(
                            ...
                            portal_url=str(
                                self._model_analysis_run.get_portal_url())
                        )
                    )
                elif status not in RUNNING_STATES:
                    self._azureml_is_initialized = True
                else:
                    return False
            else:
                raise UserErrorException._with_error(
                    AzureMLError.create(AnalysisInitNotExecutedError))
        return self._azureml_is_initialized
    ...
    def _initialize_azureml(self, experiment_name: str, **kwargs) -> ModelAnalysisRun:
        ...
        self._model_analysis_run = ModelAnalysisRun(experiment, script_run.id)
