...
class Forecasting(Config):
    ...
    def compute_naive_forecast(self, df_forecast, naive_type='roll'):
        """
        Return a dataframe attaching the back suggested interval and forecast per date and item record
        ...
        try:
            for item in df_forecast['Item'].unique():
                ...
                if naive_type == 'last':
                    ...
                    rolling = np.flipud(np.round(target.rolling(window=self.forecast_horizon, min_periods=1).mean().values))
                    df_forecast.loc[mask, 'ForecastNaive'] = rolling
                else:
