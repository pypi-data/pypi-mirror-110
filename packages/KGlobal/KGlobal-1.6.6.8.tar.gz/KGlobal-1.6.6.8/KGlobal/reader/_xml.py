    ...
    def __parse_xml_root(self, xml_objs, handler=None, buffer=None):
        ...
        for xml_obj in xml_objs:
            ...
            if handler and buffer <= len(xml_dicts):
                handler(self.file_path, self.__parse_df(xml_dicts), row_num - len(xml_dicts) + 1, row_num,
                        **self.kwargs)
                xml_dicts.clear()
        ...
        if handler:
            row_num -= 1
            handler(self.file_path, self.__parse_df(xml_dicts), row_num - len(xml_dicts) + 1, row_num, **self.kwargs)
        else:
