...
class InstrumentGeometry:
    ...
    ----------
    files : str or list
        filename or list of data respecting the file format. Up to three files\
        can be provided respecting the followind order :

        1. the file (or list) containing the main bore geometry
        2. the file (or list) with the side holes geometry and location
        3. the file (or list) indicating the fingering chart
    ...
    def __repr__(self):
        return '<openwind.InstrumentGeometry({} main bore parts, {} holes, ' \
               '{})>'.format(len(self.main_bore_shapes),
                             len(self.holes), repr(self.fingering_chart))
...
#%% Parsing hole data
    ...
    def plot_InstrumentGeometry(self, figure=None, note=None, double_plot=True, **kwargs):
        ...
        for shape in self.main_bore_shapes:
            ...
            ax[0].plot(np.append(position, np.flip(position))*mmeter,
                     np.append(radius, np.flip(-radius))*mmeter, **kwargs)
            if double_plot:
