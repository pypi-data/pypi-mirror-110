...
def stellar_framework (freq, psd, r, m, teff, nwalkers=64, dnu=None, back=None,
                       numax=None, Hmax=None, Wenv=None, epsilon=None, width=None, 
                       alpha=None, d02=None, d01=None, d13=None, n_harvey=2, low_cut=10.,
                       guess_back=None, spectro=False, filename_back='background', back_mcmc=True,
                       filemcmc_back='mcmc_sampler_background.h5', nsteps_mcmc_back=1000,
                       pattern_mcmc=True, pattern_mle=True, n_order=3, n_order_peakbagging=3,
                       h0_screening=False, 
                       filename_pattern='pattern', filemcmc_pattern='mcmc_sampler_pattern.h5',
                       ...
                       nsteps_mcmc_peakbagging=1000, mcmcDir='.', instr='geometric',
                       filename_peakbagging='summary_peakbagging.pdf', nopeakbagging=False,
                       coeff_discard_back=5, coeff_discard_pattern=5, coeff_discard_pkb=5,
                       thin_back=1, thin_pattern=1, quickfit=False, num=500,
                       ...
                       guess_angle=90, fit_splittings=False, guess_split=0,
                       frozen_harvey_exponent=False, fit_amp=False, extended=False, author=None,
  ...
  :param h0_screening: if set to ``True``, a fast frequency screening of the p-mode region 
    will be performed in order to determine the mode that it possible to fit. ``n_order_peakbagging``
    value will be ignored in this case. Optional, default ``False``.
  ...
  :param fit_amp: if set to ``True``, amplitude of the modes will be fitted instead of heights (input heights will be transformed inside
    the code and retransformed as outputs).
  :type fit_amp: bool

  :param extended: if set to ``True``, the returned pkb array will be extended to contain asymmetric uncertainties over each parameter.
  :type extended: bool

  :param author: identifier of the peakbagger, if given it will be specified in the header of the output pkb file. 
    Optional, default ``None``.
  :type author: str

  :param pkb_fmt: float formatting of the pkb file that will be created by the function. Optional, default ``None``.
  ...
  :param existing_chains_back: controls the behaviour of the function concerning a background parameters existing hdf5 file. 
    If, ``read``, existing files will be read
    ...
    if ``ignore``, the chain is not read. Optional, default ``read``. The option ``ignore`` will apply only to individual mode chains,
    it will be changed to ``read`` for background and pattern steps. 
  :type existing_chains: str

  :param existing_chains_pattern: Same as ``existing_chains_back`` but for a global pattern parameters existing hdf5 file. 
  :type existing_chains_pattern: str

  :param existing_chains_pkb: Same as ``existing_chains_back`` but for individual mode parameters existing hdf5 files. 
  :type existing_chains_pkb: str

  :param strategy: fitting strategy for individual modes. Can be ``order`` or ``global``. Optional, default ``order``. Note that the ``global`` behaviour
    is experimental. 
  :type strategy: str
  ...
  if back is None :
    fitted_back, param_back = perform_mle_background (freq, psd, n_harvey=n_harvey, r=r, m=m, teff=teff, guess=guess_back, 
                              ...
                              apodisation=apodisation, num=num, reboxing_behaviour=reboxing_behaviour, reboxing_strategy=reboxing_strategy)
    np.savetxt (filename_back+'_parameters.dat', param_back, fmt='%-s', header=make_header_background_param (n_harvey=n_harvey, author=author, mcmc=False))
    ...
    if back_mcmc :
      fitted_back, param_back, sigma_back  = explore_distribution_background (freq, psd,
                                       ...
                                       apodisation=apodisation, num=num, reboxing_behaviour=reboxing_behaviour,
      ...
      np.savetxt (filename_back+'_parameters.dat', np.c_[param_back, sigma_back], fmt='%-s', 
                  header=make_header_background_param (n_harvey=n_harvey, author=author, apodisation=apodisation,
                                                       low_cut=low_cut, nwalkers=nwalkers, nsteps=nsteps_mcmc_back, coeff_discard=coeff_discard_back,
    ...
    np.savetxt (filename_back+'.dat', fitted_back, fmt='%-s', 
                header=make_header_background_vector (author=author,apodisation=apodisation,
                                                      ...
                                                      coeff_discard=coeff_discard_back,
                                                      reboxing_behaviour=reboxing_behaviour, quickfit=quickfit))
  ...
  if a2z_in is None :
    if pattern_mle :
      df_a2z, pattern = perform_mle_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, 
                                             wdw=None, epsilon=epsilon, width=width, alpha=alpha, d01=d01,
                                             d02=d02, d13=d13,
                                             n_order=n_order, split=0, angle=90, fit_l1=fit_l1, 
                                             fit_l3=fit_l3, mass=m, show=False, filename=filename_pattern+'.pdf')
    ...
    if pattern_mcmc :
      df_a2z, pattern, sigma_pattern = explore_distribution_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, wdw=None,
                                                                     n_order=n_order, split=0, angle=90, fit_l1=fit_l1, 
                                                                     fit_l3=fit_l3, mass=m, guess=pattern, show=False,
                                                                     show_corner=True, nsteps=nsteps_mcmc_pattern, 
                                                                     filename=filename_pattern+'.pdf', parallelise=parallelise, progress=progress,
      ...
      np.savetxt (filename_pattern+'.dat', pattern, fmt='%-s', 
                  header=make_header_pattern (author=author, fit_l1=fit_l1, fit_l3=fit_l3,
                                              ...
                                              coeff_discard=coeff_discard_pattern,
                                              n_order=n_order))
    if h0_screening :
    ...
    df_a2z = pattern_to_a2z (*pattern, split=guess_split, angle=guess_angle, orders=orders_for_peakbagging, angle_order=fit_angle, 
                             splitting_order=fit_splittings, common_width=common_width)
    if h0_screening :
  ...
  if not nopeakbagging :
    ...
    df_a2z, pkb = peakbagging (a2z_to_read, freq, psd, back=fitted_back, wdw=None, spectro=spectro,
                   ...
                   strategy=strategy, nwalkers=nwalkers, normalise=False, instr=instr, filename_summary=filename_peakbagging, show_summary=False,
                   coeff_discard=coeff_discard_pkb, save_only_after_sampling=save_only_after_sampling, restrict=True, fit_amp=fit_amp,
                   extended=extended, projected_splittings=projected_splittings, bins=bins, existing_chains=existing_chains_pkb, 
                   fit_splittings=fit_splittings, fit_angle=fit_angle)
  ...
  save_pkb (a2z_file[:len(a2z_file)-3]+'pkb', pkb, fmt=pkb_fmt, 
            ...
            coeff_discard=coeff_discard_pkb, fit_amp=fit_amp) 
