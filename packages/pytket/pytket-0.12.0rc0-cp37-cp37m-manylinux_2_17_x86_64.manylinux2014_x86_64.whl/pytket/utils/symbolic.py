...
def symb_rz(params: ParamsType) -> ImmutableMatrix:
    return ImmutableMatrix(
        [
            [sympy.exp(-0.5j * sympy.pi * params[0]), 0],
            [0, sympy.exp(0.5j * sympy.pi * params[0])],
        ]
    )


def symb_rx(params: ParamsType) -> ImmutableMatrix:
    costerm = sympy.cos(0.5 * sympy.pi * params[0])
    sinterm = -1j * sympy.sin(0.5 * sympy.pi * params[0])
    return ImmutableMatrix(
        [
            [costerm, sinterm],
            [sinterm, costerm],
...
def symb_ry(params: ParamsType) -> ImmutableMatrix:
    costerm = sympy.cos(0.5 * sympy.pi * params[0])
    sinterm = sympy.sin(0.5 * sympy.pi * params[0])
    return ImmutableMatrix(
        [
            [costerm, -sinterm],
            [sinterm, costerm],
...
def symb_u3(params: ParamsType) -> ImmutableMatrix:
    ...
    costerm = sympy.cos(0.5 * sympy.pi * theta)
    sinterm = sympy.sin(0.5 * sympy.pi * theta)
    return ImmutableMatrix(
        [
            [costerm, -sinterm * sympy.exp(1j * sympy.pi * lam)],
            [
                sinterm * sympy.exp(1j * sympy.pi * phi),
                costerm * sympy.exp(1j * sympy.pi * (phi + lam)),
            ],
        ]
    )
...
def symb_iswap(params: ParamsType) -> ImmutableMatrix:
    alpha = params[0]
    costerm = sympy.cos(0.5 * sympy.pi * alpha)
    sinterm = sympy.sin(0.5 * sympy.pi * alpha)
    return ImmutableMatrix(
        [
            [1, 0, 0, 0],
            [0, costerm, 1j * sinterm, 0],
            [0, 1j * sinterm, costerm, 0],
            [0, 0, 0, 1],
        ]
    )
...
def symb_phasediswap(params: ParamsType) -> ImmutableMatrix:
    p, alpha = params
    costerm = sympy.cos(0.5 * sympy.pi * alpha)
    sinterm = 1j * sympy.sin(0.5 * sympy.pi * alpha)
    phase = sympy.exp(2j * sympy.pi * p)
    return ImmutableMatrix(
        [
            [1, 0, 0, 0],
...
def symb_xxphase(params: ParamsType) -> ImmutableMatrix:
    alpha = params[0]
    c = sympy.cos(0.5 * sympy.pi * alpha)
    s = -1j * sympy.sin(0.5 * sympy.pi * alpha)
    return ImmutableMatrix(
        [
            [c, 0, 0, s],
            [0, c, s, 0],
...
def symb_zzphase(params: ParamsType) -> ImmutableMatrix:
    alpha = params[0]
    t = sympy.exp(0.5j * sympy.pi * alpha)
...
def symb_fsim(params: ParamsType) -> ImmutableMatrix:
    ...
    s = -1j * sympy.sin(sympy.pi * alpha)
    t = sympy.exp(-1j * sympy.pi * beta)
...
def circuit_to_symbolic_gates(circ: Circuit) -> Mul:
    ...
    return outmat * sympy.exp((circ.phase * sympy.pi * 1j))
