...
def rotate_vector(v, theta, about=np.array((0, 0))):
...
def Rectangle(lefttop, rightbottom):
    """
...
def rectangle_intersect(a, b, norm_intersect_area=None):
    """
...
def rectangle_a_inside_b(a, b):
    if  isinstance(a, tuple) and isinstance(b, tuple) and len(a)==4 and len (b)==4:
        return rectangle_intersect(Rectangle(Point(a[0], a[1]), Point(a[2], a[3])), Rectangle(Point(b[0], b[1]), Point(b[2], b[3])), norm_intersect_area='b')==1

    return rectangle_intersect(a, b, norm_intersect_area='b')==1
...
def project_polarcoord_lines(lines, img_w, img_h):
    ...
    for i, (rho, theta) in enumerate(lines):
        ...
        p1 = Point(*valid_pts[0])
        p2 = Point(*valid_pts[1])
...
if __name__=="__main__":
    a=Rectangle(Point(0,0),Point(20, 40))
    b=Rectangle(Point(1, 1), Point(10, 20))
    print(rectangle_a_inside_b(a, b))
