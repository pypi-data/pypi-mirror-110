    ...
    def get_linked_items(self, element_codename: str):
        try:
            ...
            for codename in linked_codenames.value:
                ...
                linked_items.append(ContentItem(linked_item["system"],
                                    linked_item["elements"],
...
class ContentItemsFeed:
    ...
    def get_next(self):
        if self.next is not None:
            manager = self.__delivery_client.request_manager
            response = manager.get_request(self.__delivery_client,
                                           self.url, self.next)
            if response.ok:
                builder = self.__delivery_client.content_builder
                next_results = builder.build_content_item_listing(self.__delivery_client,
                                                                  response)
