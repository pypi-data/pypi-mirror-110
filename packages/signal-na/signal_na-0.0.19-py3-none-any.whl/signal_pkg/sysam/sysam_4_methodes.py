...
class Sysam4Methodes(Sysam3Test):
    ...
    def calculer_arguments_config_entrees(self):
        if "entree" in self.chaine_mode:
            voies = [s.lire_voie_base().calculer_numero() for s in self.liste_entrees]
            calibres = [s.lire_voie_base().calibre for s in self.liste_entrees]
            diff = [s.lire_voie_base().calculer_numero() for s in self.liste_entrees_diffs]
    ...
    def calculer_arguments_config_echantillon(self):
        if "entree" in self.chaine_mode:
            if self.base_de_temps_entrees:
                techant = self.base_de_temps_entrees.calculer_Te() / base.voie_base.T_sysam
                nbpoints = self.base_de_temps_entrees.calculer_N()
    ...
    def calculer_arguments_config_trigger(self):
        if "trigger" in self.chaine_mode:
            ...
            voie = st.lire_voie_base().calculer_numero()
            seuil = st.lire_trigger_base().seuil
            montant = 1 if st.lire_trigger_base().montant == True else 0
            pretrigger = st.lire_trigger_base().pretrigger
            pretrigger_souple = 1 if st.lire_trigger_base().pretrigger_souple == True else 0
            hysteresys = 1 if st.lire_trigger_base().hysteresys == True else 0
    ...
    def calculer_arguments_config_sortie(self, n):
        if "sortie"+str(n) in self.chaine_mode and "synchrone" not in self.chaine_mode:
            ...
            techant = s.lire_base_de_temps().calculer_Te() / base.voie_base.T_sysam
            tensions = s.lire_vecteur_signal()
            repetition = -1 if s.lire_voie_base().repetition else 0
    ...
    def calculer_arguments_acquerir_avec_sorties(self):
        if "synchrone" in self.chaine_mode:
            tensions1 = self.liste_sortie1[0].lire_vecteur_signal() if "sortie1" in self.chaine_mode else np.zeros(1)
            tensions2 = self.liste_sortie2[0].lire_vecteur_signal() if "sortie2" in self.chaine_mode else np.zeros(1)
