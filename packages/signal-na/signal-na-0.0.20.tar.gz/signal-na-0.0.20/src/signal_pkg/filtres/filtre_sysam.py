...
class FiltreSysam(FiltreBase):
    ...
    def faire_une_mesure_frequentielle(self, f):
        ...
        sortie_sysam = Signal(type_signal = "cosinus", F = f, Vpp = self.Vpp_sortie_sysam, liste_tmin_tmax = [0, T], Te = Te)
        ...
        entree_filtre = Signal(nom_voie = self.voie_entree_filtre, calibre = self.calibre_entree_filtre, liste_tmin_tmax = liste_tmin_tmax, Te = Te, nom = "$e$")
        sortie_filtre = Signal(nom_voie = self.voie_sortie_filtre, calibre = self.calibre_sortie_filtre, liste_tmin_tmax = liste_tmin_tmax, Te = Te, nom = "$s$")
    ...
    def acquerir_reponse_indicielle(self):
        try:
            ...
            sortie_sysam = Signal(type_signal = "carre", F = 1/T, Vpp = Vpp, offset = 0.5*Vpp, tr = 0.4*T, liste_tmin_tmax = [0, 0.7*T], Te = T/10)
