"""
Functions for creating a partial dependence plot of a feyn model.
"""
import numpy as np
...
def _create_samples(data: np.ndarray, n: int) -> np.ndarray:
    # Find fixed values for remaining features in model.

    # For catgorical features apply the 3 most frequent categories
    if data.dtype == "object":
        cats, counts = np.unique(data, return_counts=True)
        return cats[np.argsort(-counts)][:n]

    else:
        # For numerical features find representative quantiles
        quantiles = np.linspace(0,1, num=n+2)[1:-1] # For n=3 yields [1/3,1/2,1/3]. For n=2 [1/3, 2/3], for n=1 [1/2]
        rounded_quantiles = [np.round(np.quantile(data, x), 2) for x in quantiles]
...
def plot_partial(
    model: feyn.Model, data: Iterable, by: str, fixed: Optional[dict] = None
) -> None:
    ...
    > best = models[0]
    > feyn.plot_partial(best, data, by="age")
    ...
    fixed = (
        {key: _to_numpy_array(val) for key, val in fixed.items()}
    ...
    # Point to features in the chosen model that for our plots needs to be fixed
    cols = [col for col in model.features if col not in [by]]
    ...
    samples = {
        col: fixed[col] if col in fixed else _create_samples(data[col], n=n) for col in cols
    }
    ...
    for col in cols:
        ...
        if samples[col].dtype == "object":
            ...
            samples["id"] = np.char.add(
                samples["id"], np.round(samples[col], 2).astype("U")
            )

    # Perform predictions on the new values
    samples["preds"] = model.predict(samples)
    ...
    if data[by].dtype == "object":
        ...
        sort_df = (
            total_df.groupby(by)
            .agg(preds_sort=("preds", "mean"))
            .reset_index()
            .sort_values("preds_sort")
        )

        total_df = pd.merge(
            total_df, sort_df, on=by, suffixes=(False, False)
        ).sort_values("preds_sort")

        plot_df = pd.merge(
            pd.DataFrame(data), sort_df, on=by, suffixes=(False, False)
        ).sort_values("preds_sort")

        support = plot_df[target]
        ax_scatter.scatter(
            support, plot_df[by], alpha=0.3, color="grey", label="_nolegend_"
        )
        ...
        for i, group in enumerate(total_df.id.sort_values().unique()):
            ...
            ax_scatter.scatter(
                ...
                marker=markerstyles[
                    (i // number_of_colors)
                    - (i // (number_of_colors * len(markerstyles)) * len(markerstyles))
                ],
                label=group,
            )
        ax_scatter.set_xlabel(f"Predicted {target}")
        ax_scatter.legend(
            title=legend_text, loc="center left", bbox_to_anchor=(1.35, 0.5)
        )
        ax_scatter.set_ylabel(by)

        ax_histx.hist(support, bins=30, color="grey", rwidth=0.9, alpha=0.6)

        x, ind, height = np.unique(plot_df[by], return_counts=True, return_index=True)
        ax_histy.barh(
            x[np.argsort(ind)], height[np.argsort(ind)], color="grey", alpha=0.6
        )
        ...
        for i, group in enumerate(total_df.id.unique()):
            ...
            ax_scatter.plot(
                ...
                linestyle=linestyles[
                    (i // number_of_colors)
                    - (i // (number_of_colors * len(linestyles)) * len(linestyles))
                ],
                label=group,
                linewidth=1.5,
            )
        ...
        ax_histx.hist(data[by], bins=30, color="grey", rwidth=0.9, alpha=0.6)
        ax_histy.hist(
            support,
            bins=30,
            color="grey",
