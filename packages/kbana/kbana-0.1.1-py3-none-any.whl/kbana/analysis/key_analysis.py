...
def visualize_key_stroke(records, keyboard_style=None, keyboard_offset=(0, 0), exclude_key_list=[],
    ...
    """
    arguments
    records (dict-like):    is dictionary generated by RecordingSession
                            (flatten current version of records is not supported)
...
def visualize_finger_load(records, axis_handle=plt, numeric='freq', exclude_shift=True):
    """
    visualize finger load of key strokes records by plotting

    arguments
    records (dict-like): key stroke records generated by kbana.capture.RecordSession or kbana.analysis.simulate_record
    axis_handle (matplotlib.pyplot.Axes):  2d Axes which the visualize are drawn (default: matplotlib.pyplot)
    numeric (str): specifies numeric values are displayed on the plot. (default 'freq')
                options
                'prop': proportion
...
class Analysis:
    ...
    def visualize_key_stroke(self, records, axis_handle=plt, *args, **kwargs):
        return visualize_key_stroke(records, self.keyboard_style, axis_handle=axis_handle, *args, **kwargs)
