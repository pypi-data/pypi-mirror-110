...
def is_valid_row_or_col(row_or_col):
    return isinstance(row_or_col, list) and [isinstance(cell, int) for cell in row_or_col]
...
class PuzzleSchema(Schema):
    ...
    def validate_array_lengths(self, data, **kwargs):
        if (
            'rows' in data and isinstance(data['rows'], list) and
            'cols' in data and isinstance(data['cols'], list)
        ):
            rows = data['rows']
            cols = data['cols']
            height = len(rows)
