...
class Task:
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__} topic='{self.topic}' id='{self.msgid}'>"
...
class Queue:
    ...
    def publish_batch(self, topic: str, messages: List[Dict[str, str]], front: bool = False) -> List[str]:
        return rfq.publish.publish_batch(topic=topic, messages=messages, front=front, redis=self.r)
    ...
    def peek(self, topic: str, msgid: str) -> Optional[Dict[str, str]]:
