...
from mlmd.metadata_dao import get_annotation_stats, get_image_metadata, get_model_info_by_dataset, get_tenant_id, insert_image_log_bulk, insert_image_metadata_bulk, get_image_executions_by_version
...
class Dataset():
    ...
    def trigger_retrain(self, models=None):
        url = os.environ.get("RETRAIN_URL")
        username = os.environ.get("RETRAIN_USERNAME")
        password = os.environ.get("RETRAIN_PASSWORD")
        if url is None or username is None or password is None:
            print("Missing required environment variables: RETRAIN_URL, RETRAIN_USERNAME, RETRAIN_PASSWORD")
            return
        authorization = "{}:{}".format(base64.b64encode(username), base64.b64encode(password))
    ...
    def load_filelist(splitter=None):
        """
        Load the images specified in self.filelist to memory variables. A logic of how to split can be defined by input splitter
        """
