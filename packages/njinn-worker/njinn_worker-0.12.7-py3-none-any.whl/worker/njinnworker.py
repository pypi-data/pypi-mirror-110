...
from worker.config import WorkerConfig, get_preferred_encoding
...
class NjinnWorkerStep(bootsteps.StartStopStep):
    ...
    def start(self, worker):
        ...
        self.log.info(
            "Guessing codepage %s, using %s. Please make sure that worker codepage matches shell codepage.",
            get_preferred_encoding(),
            self.njinnworker.config.codepage,
        )
...
class NjinnWorker:
    def __init__(self, registration_token=None, njinn_url=None, no_default_queues=False):
        ...
        if not config_valid and registration_token is not None:
            ...
            self.register(registration_token, njinn_url, no_default_queues)
        ...
        if not self.config.is_valid():
            ...
            # server needs to know platform to correctly assign queues,
            # so update details first, then get current config.
    ...
    def register(self, registration_token, njinn_url, no_default_queues):
        ...
        if no_default_queues:
            data["join_default_queues"] = False
        try:
