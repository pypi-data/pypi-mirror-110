...
from worker.version_utils import BASE_REQUIREMENTS
...
class PackInstallation:
    ...
    def upgrade_pip(self, log=log):
        """Upgrade pip to a recent version (required for installing abi3 wheels).

        We had issues upgrading to a newer version of venv, and the pip version
        that comes with the one that works for us is ancient.
        
        Only call this variant inside a lock
        """
        log.info("%s: Upgrading pip", self.display_name)
        subprocess.run(
            [
                self.get_python(),
                "-m",
    ...
    def __install_worker_dependencies(self, log=log):
        """ Install the dependencies of worker's action.py inside the pack venv

        Only call this variant inside a lock
        """
        log.info("%s: Installing default dependencies.", self.display_name)
        dependencies = " ".join([f'"{r}"' for r in BASE_REQUIREMENTS])
        self.run_pip(
            f"install {dependencies}", log=log,
        )
