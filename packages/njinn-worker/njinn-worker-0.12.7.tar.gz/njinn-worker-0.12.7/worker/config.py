...
class WorkerConfig:
    ...
    @property
    def log_file_timeout(self):
        # always let env var win
        return int(
            os.getenv(
                "NJINN_WORKER_LOG_FILE_TIMEOUT",
                self.config.getint("DEFAULT", "log_file_timeout", fallback=5),
            )
        )

    @log_file_timeout.setter
    def log_file_timeout(self, concurrency):
        self.config["DEFAULT"]["log_file_timeout"] = concurrency
    ...
    def codepage(self):
        return os.getenv(
            "NJINN_WORKER_CODEPAGE",
            self.config.get("DEFAULT", "codepage", fallback=get_preferred_encoding()),
        )

    @codepage.setter
    def codepage(self, codepage):
        self.config["DEFAULT"]["codepage"] = codepage

    @property
    def hard_kill_timeout(self):
        return int(
            os.getenv(
                "NJINN_WORKER_HARD_KILL_TIMEOUT",
                self.config.get("DEFAULT", "hard_kill_timeout", fallback=30),
            )
        )

    @hard_kill_timeout.setter
    def hard_kill_timeout(self, hard_kill_timeout):
        self.config["DEFAULT"]["hard_kill_timeout"] = hard_kill_timeout
    ...
    @classmethod
    def initialize_from_default_location(cls) -> WorkerConfig:
        """for testability only"""
        return cls().load_from_file().update_from_api()
...
class ActionExecutionConfig:
    def __init__(
        ...
        self.njinn_api = None
        self.worker_config: WorkerConfig = None
