...
logging.basicConfig(
    format='[%(asctime)s] ' + _script_name + ' [%(levelname)s] %(funcName)s %(lineno)d: %(message)s'
)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="terraform output json formatted file"
    )
    ...
    try:
        ...
        print('Please ensure session-manager-plugin is installed correctly')
    ...
    valid_container_names = [i['name'] for i in tasks['tasks'][0]['containers']]
    if not args.container:
        # print(f'{Fore.RED}Please supply one of the following{Style.RESET_ALL}')
        ...
        # print('\n'.join(valid_container_names))
        exit(1)
    # pprint([i['name'] for i in tasks['tasks'][0]['containers']])
    ...
    try:
        response = session_client.execute_command(
            ...
            interactive=True,
            task=taskArns['taskArns'][-1], # Always use the last taskArn
        )
    except:
    ...
    profile_name = session_source.profile_name if session_source.profile_name is not None else ''
