...
async def _get_folder(request: web.Request) -> web.Response:
    ...
    _logger.debug(f'Requested folder {request.match_info["id"]}')
...
@action(name='heaserver-folder-folder-open', path='/folders/{folder_id}/items/{id}')
...
@routes.get('/folders/{id}/properties')
@action(name='heaserver-folder-folder-properties-form', path='/folders/{folder_id}/items/{id}/properties')
async def get_folder_properties_form(request: web.Request) -> web.Response:
    """
    Gets the folder with the specified id, including a template for editing the folder's properties.
    :param request: the HTTP request.
    :return: the requested folder or Not Found.
    """
    return await _get_folder(request)
...
@action(name='heaserver-folder-item-move', path='/folders/{folder_id}/items/{id}/mover', include_root=True)
@action(name='heaserver-folder-item-delete', path='/folders/{folder_id}/items/{id}', include_root=True)
@action(name='heaserver-folder-item-duplicate', path='/folders/{folder_id}/items/{id}/duplicator', include_root=True)
@action(name='heaserver-folder-item-properties', path='/folders/{folder_id}/items/{id}/properties', include_root=True)
...
@routes.get('/folders/{folder_id}/items/{id}/properties')
async def get_item_properties_form(request: web.Request) -> web.Response:
    _logger = logging.getLogger(__name__)
    item = await request.app[appproperty.HEA_DB].get(request, MONGODB_ITEMS_COLLECTION,
                                                     var_parts=['folder_id', 'id'], sub=request.headers.get(SUB))
    _logger.debug('got from mongo: %s', item)
    if item is None:
        ...
        wstl_builder = request[requestproperty.HEA_WSTL_BUILDER]
        wstl_builder.data = [item]
        if await _add_actual_object_properties_form(request, wstl_builder, item, headers={SUB: request.headers.get(SUB)}):
            run_time_doc = wstl_builder()
        else:
            run_time_doc = None
    _logger.debug('got from service: %s', run_time_doc)
...
async def put_item_in_folder(request: web.Request) -> web.Response:
    ...
    if request.match_info['folder_id'] == ROOT_FOLDER.id:
        ...
        folder = Folder()
        folder_dict = await request.app[HEA_DB].get(request, MONGODB_FOLDER_COLLECTION, var_parts='folder_id', sub=user)
        _logger.debug(f'Requested folder {request.match_info["folder_id"]}, found folder {folder_dict}')
        if folder_dict is None:
            return response.status_not_found()
        folder.from_dict(folder_dict)
...
async def _add_actual_object(request: web.Request, wstl_builder: RuntimeWeSTLDocumentBuilder, item: Dict[str, Any], headers: Optional[Dict[str, str]] = None):
    ...
    if item:
        ...
        if actual_object_run_time_doc is None:
            ...
            item['actual_object'] = actual_object_run_time_doc['wstl']['data'][0]
            return item
    else:
        return None
