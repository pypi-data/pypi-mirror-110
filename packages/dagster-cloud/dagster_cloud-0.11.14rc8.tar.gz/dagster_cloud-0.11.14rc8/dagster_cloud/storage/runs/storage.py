...
def _run_record_from_graphql(graphene_run_record: Dict) -> RunRecord:
    ...
    return RunRecord(
        record_id=check.int_elem(graphene_run_record, "recordId"),
...
class GraphQLRunStorage(RunStorage, ConfigurableClass):
    ...
    def get_runs(
        ...
        return [
            check.inst(deserialize_json_to_dagster_namedtuple(run), PipelineRun)
            for run in res["data"]["runs"]["getRuns"]
        ]
    ...
    def get_run_group(self, run_id: str) -> Optional[Tuple[str, Iterable[PipelineRun]]]:
        ...
        run_group = res["data"]["runs"]["getRunGroup"]
        ...
        return (
            ...
            [
                check.inst(deserialize_json_to_dagster_namedtuple(run), PipelineRun)
                for run in run_group["serializedRuns"]
            ],
        )
    ...
    def get_run_by_id(self, run_id: str) -> Optional[PipelineRun]:
        ...
        return check.inst(deserialize_json_to_dagster_namedtuple(run), PipelineRun)
    ...
    def get_daemon_heartbeats(self) -> Dict[str, DaemonHeartbeat]:
        ...
        return {
            key: check.inst(deserialize_json_to_dagster_namedtuple(heartbeat), DaemonHeartbeat)
    ...
    def get_backfills(self, status: BulkActionStatus = None, cursor: str = None, limit: int = None):
        ...
        return [
            check.inst(deserialize_json_to_dagster_namedtuple(backfill), PartitionBackfill)
    ...
    def get_backfill(self, backfill_id: str) -> PartitionBackfill:
        ...
        return check.inst(deserialize_json_to_dagster_namedtuple(backfill), PartitionBackfill)
