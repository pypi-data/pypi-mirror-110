...
from dagster.core.storage.event_log.base import EventLogStorage, extract_asset_events_cursor
...
def _event_record_from_graphql(graphene_event_record: Dict) -> EventRecord:
    check.dict_param(graphene_event_record, "graphene_event_record")

    return EventRecord(
        error_info=(
            deserialize_json_to_dagster_namedtuple(
                check.str_elem(graphene_event_record, "errorInfo")
            )
            if graphene_event_record.get("errorInfo") is not None
            else None
        ),
        message=check.str_elem(graphene_event_record, "message"),
        level=check.int_elem(graphene_event_record, "level"),
        user_message=check.str_elem(graphene_event_record, "userMessage"),
        run_id=check.str_elem(graphene_event_record, "runId"),
        timestamp=check.float_elem(graphene_event_record, "timestamp"),
        step_key=check.opt_str_elem(graphene_event_record, "stepKey"),
        pipeline_name=check.opt_str_elem(graphene_event_record, "pipelineName"),
        dagster_event=(
            deserialize_json_to_dagster_namedtuple(
                check.str_elem(graphene_event_record, "dagsterEvent")
            )
            if graphene_event_record.get("dagsterEvent") is not None
            else None
        ),
    )
...
class GraphQLEventLogStorage(EventLogStorage, ConfigurableClass):
    ...
    def get_logs_for_run(
        ...
        return [
            _event_record_from_graphql(event) for event in res["data"]["eventLogs"]["getLogsForRun"]
        ]
    ...
    def get_asset_events(
        ...
        if include_cursor:
            return [
                (
                    asset_record["id"],
                    _event_record_from_graphql(asset_record["eventRecord"]),
                )
