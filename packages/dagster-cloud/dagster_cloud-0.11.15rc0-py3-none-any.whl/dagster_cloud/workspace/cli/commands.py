...
DEFAULT_DAGIT_USERNAME = lambda: os.getenv(
    "DAGSTER_CLOUD_DAGIT_USERNAME",
    "dagster",
)


def _password():
    return os.getenv("DAGSTER_CLOUD_PASSWORD")


def _show_prompt():
    password_exists = bool(_password())
    show_prompt = not password_exists
    return show_prompt


_dagit_client_options = [
    click.option("--url", type=click.STRING, default=DEFAULT_DAGIT_URL, help="Dagit url"),
    click.option(
        "--username",
        type=click.STRING,
        default=DEFAULT_DAGIT_USERNAME,
        help="Dagit basic auth username",
    ),
    click.option(
        "--password",
        type=click.STRING,
        default=_password,
        # Only show the prompt if it's not set in the envvar
        prompt=_show_prompt(),
        hide_input=True,
        help="Dagit basic auth password",
    ),
]
...
def _dagit_client(url, username, password):
    url = f"{url}/graphql"

    auth = HTTPBasicAuth(username, password) if password else HTTPBasicAuth("", "")
    ...
    return GqlShimClient(client=Client(transport=transport, fetch_schema_from_transport=True))
...
@add_options(_dagit_client_options)
@add_options(_deployment_metadata_options)
@click.argument("location", type=click.STRING)
def add_command(url, username, password, location, **kwargs):
    execute_add_command(_dagit_client(url, username, password), location, kwargs)
...
@add_options(_deployment_metadata_options)
@click.argument("location", type=click.STRING)
def update_command(url, username, password, location, **kwargs):
    execute_update_command(_dagit_client(url, username, password), location, kwargs)
...
@add_options(_dagit_client_options)
@click.argument("location", type=click.STRING)
def delete_command(url, username, password, location):
    execute_delete_command(_dagit_client(url, username, password), location)
...
@add_options(_dagit_client_options)
def list_command(url, username, password):
    execute_list_command(_dagit_client(url, username, password))
...
def sync_command(url, username, password, workspace):
    execute_sync_command(_dagit_client(url, username, password), workspace)
