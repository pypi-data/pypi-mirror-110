...
class DagsterCloudClientInstance(DagsterCloudInstance):
    ...
    def create_user_code_launcher(self):
        if not self._user_code_launcher_data:
            ...
            raise DagsterInvariantViolationError(
                "User code launcher is not configured for DagsterCloudClientInstance. "
    ...
    def get():  # pylint: disable=arguments-differ
        ...
        if not isinstance(instance, DagsterCloudClientInstance):
            raise DagsterInvariantViolationError(
                """
DagsterInstance.get() did not return a DagsterCloudClientInstance. Make sure that your"
`dagster.yaml` file is correctly configured to include the following:
custom_instance_class:
  module: dagster_cloud.instance
  class: DagsterCloudClientInstance
"""
            )
        return instance
