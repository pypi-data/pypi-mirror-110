...
class PytorchTrainer:
    ...
    def __init__(self,
        ...
        """
        self.device = 'cuda' if (torch.cuda.is_available() and is_gpu) else 'cpu'
        if is_gpu and self.device == "cpu":
            _logger.warning(Warnings.CPU_DEVICE_WARNING)
        self.model = model_class(num_label_cols)
        self.model.to(self.device)
        self.loss_fn = torch.nn.BCEWithLogitsLoss
        self.optimizer = torch.optim.Adam(params=self.model.parameters(), lr=MultiLabelParameters.LEARNING_RATE)
    ...
    def train(self, training_set):
        ...
        """
        train_sampler = self.sampler(training_set)
    ...
    def validate(self, valid_set):
        ...
        """
        valid_sampler = self.sampler(valid_set)
