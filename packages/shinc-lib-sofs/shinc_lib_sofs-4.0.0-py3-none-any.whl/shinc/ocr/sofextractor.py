...
class SOFTextExtractor:
    ...
    def extractLine(self, text, date_count, dateorder, timemask, tz=timezone.utc):
        ...
        if date_from and time_from:
            sof.startdatetime = self.generateDatetime(date_from, time_from,tz)
        if softype.bounded and time_to:
            if not date_to:
                date_to = date_from
            sof.enddatetime = self.generateDatetime(date_to, time_to,tz)
        return sof

    def textContainsSof(self, text):
        res = False
        for sof in self.soflist.softypes:
            if sof.description.lower() in text:
                return sof
        return res
...
class SOFEvent:
    ...
    def __eq__(self, other):
        if (isinstance(other, SOFEvent)):
            return self.type == other.type and self.startdatetime == other.startdatetime and self.description == other.description
        return False
