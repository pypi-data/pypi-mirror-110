...
class Compiler:
    def __init__(self, src: str = "", isDebug=False):
        ...
        datetime_now = StdlibFunctionSpec(
            self.compile_datetime_now,
            lambda _: FunctionPrototype(
                t.Unit(),
                t.Datetime()
            ),
        )
        datetime_fromtimestamp = StdlibFunctionSpec(
            self.compile_datetime_fromtimestamp,
            lambda _: FunctionPrototype(
                t.Int(),
                t.Datetime()
            )
        ...
        datetime_object.create_method("fromtimestamp", datetime_fromtimestamp, True)

        dict_get = StdlibFunctionSpec(
            self._compile_dict_safe_get,
        ...
        self.std_lib_types = {
            t.Dict: dict_object,
            t.Datetime: datetime_object,
        }
        self.std_lib = {
            "SENDER": StdlibVariable(t.Address(), self.get_sender),
            "SOURCE": StdlibVariable(t.Address(), self.get_source),
            "AMOUNT": StdlibVariable(t.Mutez(), self.get_amount),
            "BALANCE": StdlibVariable(t.Mutez(), self.compile_balance),
            "unit": StdlibVariable(t.Unit(), self.compile_unit),
            "datetime": datetime_object,
            "dic_get": dict_object,
            "blake2b": StdlibFunctionSpec(
                self._compile_blake2b,
                lambda: FunctionPrototype(
                    t.Bytes(),
                    t.Bytes()
                )
            ),
            "pack": StdlibFunctionSpec(
                self._compile_pack,
                lambda: FunctionPrototype(
                    t.Unknown(),
                    t.Bytes()
            ...
            "timedelta": StdlibFunctionSpec(
                self._compile_timedelta,
                lambda: MultiArgFunctionPrototype(
                    [t.Int(), t.Int(), t.Int(), t.Int(), t.Int()],
                    t.Int()
                )
            ),
            "transaction": StdlibFunctionSpec(
                self._compile_transaction,
                lambda contract_type: MultiArgFunctionPrototype(
                    [t.Contract([contract_type]), t.Mutez(), contract_type],
                    t.Operation()
                )
            ),
            "contract": StdlibFunctionSpec(
                self._compile_cast_address_to_contract,
                lambda contract_type: FunctionPrototype(
                    [t.Address()],
                    t.Contract()
                )
            ),
            "mutez": StdlibFunctionSpec(
                self._compile_mutez,
                lambda: FunctionPrototype(t.Int(), t.Mutez())
            ),
            "nat": StdlibFunctionSpec(
                self._compile_nat,
                lambda: FunctionPrototype(t.Int(), t.Nat())
            ),
        }
    ...
    def _compile_blake2b(self, node: ast.Call, e: Env) -> List[Instr]:
        instructions = self._compile(node.args[0], e)
        return instructions + [Instr("BLAKE2B", [], {})]

    def compile_datetime_fromtimestamp(self, timestamp: ast.Constant, e: Env) -> List[Instr]:
        if type(timestamp) != ast.Constant:
            if type(timestamp.value) != int:
                raise E.CompilerException("Can only construct a datetime object for a literal integer", timestamp.lineno)
        return [Instr("PUSH", [t.Datetime(), timestamp.value], {})]
    ...
    def get_expression_type(self, node, e: Env):
        if type(node) == ast.Name:
            ...
            elif node.id in self.std_lib:
                if type(self.std_lib[node.id]) == StdlibVariable:
                    return self.std_lib[node.id].type

                return self.std_lib[node.id]
            else:
            ...
            if type(record) == StdLibClassSpec:
                record = record.constructor.get_prototype().return_type

            attr_value_type = type(record)
            if attr_value_type in self.std_lib_types:
                if node.attr in self.std_lib_types[attr_value_type].methods:
                    return self.std_lib_types[attr_value_type].methods[node.attr].get_prototype(record).return_type
            ...
            elif type(node.func) == ast.Name and node.func.id in self.std_lib:
                if type(self.std_lib[node.func.id]) == StdlibFunctionSpec:
                    return self.std_lib[node.func.id].get_prototype().return_type
                elif type(self.std_lib[node.func.id]) == StdLibClassSpec:
                    return self.std_lib[node.func.id].constructor.get_prototype().return_type
                else:
    ...
    def compile_dict(self, dict_ast: ast.Dict, e: Env, dict_type: t.Dict=None) -> List[Instr]:
        if dict_type:
            ...
            key_type = self.get_expression_type(dict_ast.keys[0], e)
            value_type = self.get_expression_type(dict_ast.values[0], e)
        else:
    ...
    def compile_assign_record(self, node, e: Env, instructions = None):
        ...
        key_type = self.get_expression_type(node.targets[0], e)
        value_type = self.get_expression_type(node.value, e)
    ...
    def compile_assign(self, assign: ast.Assign, e: Env) -> List[Instr]:
        ...
        # type variable
        e.types[assign.targets[0].id] = self.get_expression_type(assign.value, e)
    ...
    @debug
    def compile_list(self, l: ast.List, e: Env, list_type: t.List = None) -> List[Instr]:
        if len(l.elts):
            elt_type = self.get_expression_type(l.elts[0], e)
        else:
            if list_type:
    ...
    def compile_ccall(self, c: ast.Call, e: Env):
        ...
        for i, arg in enumerate(c.args):
            actual_type = self.get_expression_type(arg, e)
    ...
    def compile_fcall(self, f: ast.Call, e: Env):
        if type(f.func) == ast.Attribute:
            func_class = self.get_expression_type(f.func.value, e)
            func_class_type = type(func_class)
            if func_class_type == StdLibClassSpec:
                func_class_type = type(func_class.constructor.get_prototype().return_type)
        ...
        # typecheck argtype
        actual_type = self.get_expression_type(f.args[0], e)
    ...
    def compile_storage(self, storage_ast, e: Env):
        e.types["__STORAGE__"] = self.get_expression_type(storage_ast, e)
        self.contract.storage_type = self.get_expression_type(storage_ast, e)
    ...
    def _compile_attribute(self, node, e: Env) -> List[Instr]:
        ...
        record_type = self.get_expression_type(node.value, e)
        instructions += self._compile(node.value, e)
    ...
    def compile_for(self, node: ast.For, e: Env) -> List[Instr]:
        ...
        iterable_type = self.get_expression_type(node.iter, e)
        if type(iterable_type) == t.List:
            ...
            raise E.NotAnIterableException(iterable_type)
    ...
    def _compile(self, node_ast, e: Optional[Env] = None, instructions = None, current_type: Optional[t.Type] = None) -> List[Instr]:
        ...
        if type(node_ast) == ast.Module:
            ...
            instructions += self.compile_list(node_ast, e, current_type)
