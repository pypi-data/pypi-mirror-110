...
class _DeckAddedHook:
    ...
    def append(self, cb: Callable[["anki.decks.DeckDict"], None]) -> None:
        """(deck: anki.decks.DeckDict)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["anki.decks.DeckDict"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _FieldFilterFilter:
    ...
    def append(
        self,
        cb: Callable[["str", "str", "str", "anki.template.TemplateRenderContext"], str],
    ) -> None:
...
class _NoteTypeAddedHook:
    ...
    def append(self, cb: Callable[["anki.models.NotetypeDict"], None]) -> None:
        """(notetype: anki.models.NotetypeDict)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["anki.models.NotetypeDict"], None]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
...
class _SchedulerNewLimitForSingleDeckFilter:
    ...
    def append(self, cb: Callable[["int", "anki.decks.DeckDict"], int]) -> None:
        """(count: int, deck: anki.decks.DeckDict)"""
        self._hooks.append(cb)

    def remove(self, cb: Callable[["int", "anki.decks.DeckDict"], int]) -> None:
        if cb in self._hooks:
            self._hooks.remove(cb)
