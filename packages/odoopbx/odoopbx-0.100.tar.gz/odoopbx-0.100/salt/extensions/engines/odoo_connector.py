...
def execute(method, args, kwargs):
    ...
    try:
        ...
        if callback:
            ...
            log.debug('Execute callback %s.%s.', callback[0], callback[1])
            try:
    ...
    # Method error.
        ...
        if status_notify_uid:
            ...
            try:
                ...
                log.exception('Method %s status notify error:', method)
        # And raise it again if odoo_raise_exception is True
...
class OdooConnector:
    ...
    async def handle_message_async(self, channel, message):
        ...
        def blocking_call():
            f = partial(
                self.handle_message, __salt__.loader(), channel, message)
            future = self.executor.submit(f)
            return (yield future)
        return await blocking_call()

    def handle_message(self, loader, channel, raw_message):

        def _handle_message(channel, raw_message):
            # Check message type
            try:
            ...
            # Uknown command?
                ...
                log.error('Unknown command: %s', message)
        # A trick with the loader
        with loader_context(loader):
            return _handle_message(channel, raw_message)
    ...
    async def update_token(self):
        ...
        while True:
            try:                
                ...
                log.error('Update token error: %s', e)
                # Prevent flood
                await sleep(1)
            finally:
                await sleep(self.refresh_token_seconds)
