...
def get_workspace(auth, subscription_id, resource_group_name, workspace_name, location=None, cloud="azurecloud", workspace_id=None):
    """
    ...
    try:
        ...
        # Try calling RP directly when arm throws ClientRequestError
        # Work around for synapse scenario till arm supports pl.
        # The ysnapse scenario will always be a privatelink workspace
        _get_workspace_dataplane(auth, subscription_id, resource_group_name, workspace_name, location, 
            cloud, workspace_id)
    except ErrorResponseWrapperException as response_exception:
        module_logger.error(
...
def _get_workspace_dataplane(
    ...
    if location is None or cloud is None or workspace_id is None:
    ...
    return WorkspacesOperations.get(workspaces, resource_group_name, workspace_name, is_dataplane=True)
...
def workspace_sync_keys(auth, resource_group_name, workspace_name, subscription_id, no_wait):
    """
