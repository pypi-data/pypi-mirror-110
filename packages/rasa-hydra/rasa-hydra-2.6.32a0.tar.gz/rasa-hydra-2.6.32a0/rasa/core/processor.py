...
class MessageProcessor:
        ...
        if message_type == "SPOKEN" and len(partials) > 0 and (
            ...
            for partial in partials:
                ...
                if parse_data["intent"]["name"] and parse_data["intent"]["confidence"] > confidence_threshold and (
                        parse_data["intent"]["name"] != "action_handle_error"):
                    logger.info(
                        "[{}] - Received user message '{}' with intent '{}' "
    ...
    def convert_pci_message(self, message):
        pci_msg = UserUttered.redact_msg(message)
    ...
    def convert_pci_entities(self, message):
        ...
        for i in pci_msg:
            if i['entity'] == 'number':
                num_list.append(i)
            elif len(num_list) != 13 and len(num_list) !=15 and len(num_list) != 16 and len(num_list) != 19  and i['entity'] != 'number':
                num_list = []
