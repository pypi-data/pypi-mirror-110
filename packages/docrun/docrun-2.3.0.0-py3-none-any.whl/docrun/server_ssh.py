...
import os
import io
#from scripts.docrun.server_kernel import GLOBALVAR
import traceback
import subprocess
import threading
import time
...
#        transport.send_ignore()106  def md5_of_content(cont):
#        pass
#    except Exception as e:
#        return False
#        pass
...
def get_private_key( key ):
    ...
    try:
        ...
        pass
    print(lt("Key Error: key cannot be parsed") )
    return None
...
def get_exec_connection(config):
    ...
    try:
        ...
        if config.get('auth_key',None):
            ...
            private_key = get_private_key( config['auth_key'] )
...
async def select_sftp_sync_folder(websocket):
    try:
        ...
        if pidl:
            ...
            folder_selected = b""
            pass
        #print("selected path is ", folder_selected)
...
if ! [ -f .bashrc ]; then
    touch .bashrc
        ...
        if err:
            ...
            return
        #print("preperation done without error")

        # check pbs system info
        (pbs_exist, queue_info) = check_if_pbs_exist(client)
        if check_queue:
            await websocket.send( json.dumps({
                "name"   : "submit-task",
                "ckey"   : ckey,
        ...
        for task_batch in batch_codes:
            ...
            if task_queue and pbs_exist:
                #print("run task with pbs system")
                #print("try submit task to pbs", task_options)
                running_mode   = 'pbs'
                stdin, stdout, stderr = client.exec_command( """ 
. /etc/profile
. ~/.bashrc
...
echo -n "" >./.tasks/{0}.log
qsub ./.tasks/{0}.pbs > ./.tasks/{0}.pid 2>&1
                """.format(
...
qstat -Q
    ...
    err = (stderr.read() or b"").decode()
    if err: return (False,'')
    out = out.split("#-qstat-out-#\n")[1]
    #print("check if _pbs_exist out=", out)
    #print("check if _pbs_exist err=", err)
    return (True,out)
...
. /etc/profile
. ~/.bashrc
cd ~/pond-calculation-works/{2}/.tasks/
if [ -f {0}_{1}.pid ]; then
    qdel `cat {0}_{1}.pid`
    rm {0}_{1}.pid
fi
            """.format(task_uid, batch, ckey))
            stdin.channel.shutdown_write()
            stdout.channel.recv_exit_status()
            out = (stdout.read() or b"").decode()
            err = (stderr.read() or b"").decode()
...
if [ -f {0}_{1}.pid ]; then
    rm {0}_{1}.pid
        ...
        #print("check no pbs task:", task_uid)
        stdin, stdout, stderr = client.exec_command("""
cd ~/pond-calculation-works/{2}/.tasks/
if [ -f {0}_{1}.pid ]; then
    kill -9 `cat {0}_{1}.pid`
    rm {0}_{1}.pid
fi
        """.format(task_uid, batch, ckey))
        stdin.channel.shutdown_write()
...
async def check_task(config, ckey, message, websocket):
    ...
    #print('check evaluating batches:', batches, current_batch)
    batch_result   = []
    output_result  = ""
    error_result  = ""
    ...
    echo -n "${{out:{2}}}"
        ...
        out = (stdout.read() or b"").decode()
        err = (stderr.read() or b"").decode()
        #print("check", batch, current_batch)
        #print("out = ", out)
        #print("err = ", err)
...
async def close_ssh_connection(websocket, conn):
    if websocket:
        try:
            ...
            print("send mes in close ssh connection failed:", e)
            pass

    if conn: 
        print("ssh connection ",  conn['ssh_id'], " closed")
...
async def ssh_communication(page_id, ssh_id ):
    try:
        ...
        while True:
            ...
            if not len(data):
                print("recieved empty data, connection closed,", ssh_id)
            ...
            try:
                ...
                print("decode data failed on:",data )
                data = ""
                pass
            #print("get ssh data:", data)
            mes = conn['message']
            ...
            try: 
                ...
                print("websocket send error, set timer to close in 10s:", ssh_id)
        ...
        print("comunication loop quited with error:",e)
        traceback.print_exc()
...
async def get_ssh_connection(websocket, config, ckey='', page_id = ''):
    ...
    try:
        ...
        if config.get('auth_key',None):
            ...
            print("connect with no auth provided")
            return None
        #print("client = ", client)
        ...
        conn['channel'].send("cd ~/pond-calculation-works/{0}".format(ckey))
        return conn
...
async def close_sftp_connection(websocket, conn):
    if websocket:
        ...
        await websocket.send(json.dumps(mes))
    if conn: 
        print("try close sftp connection ", conn['ssh_id'])
...
def sftp_listdir(infos, ckey, sftp, message):
    ...
    try:
        ...
        print("sftp_listdir failed for:",e)
        pass
    INFO['is_sftp_processing']  = False
    pass
...
def sftp_delete_item(infos, sftp, message):
    ...
    try:
        ...
        print("sftp_upload_file failed for:",e)
        pass
    INFO['is_sftp_processing']  = False
    pass
...
def sftp_download_file(infos, sftp, message):
    ...
    try:
        ...
        print("sftp_download_file failed for:",e)
        pass
    INFO['is_sftp_processing']  = False
    pass
...
async def sftp_prepare_files(infos, sftp, message, websocket):
    ...
    try:
        ...
        if pattern:
            ...
            for file in files:
                local_path = os.path.join( infos['local_path'], file )
                remote_path  = infos['remote_path'] + "/" + file
                download_sync_file(sftp, remote_path, local_path)
                pass
        ...
        if not (current_data_file and data_file == current_data_file):
            ...
            data = list(map(lambda x:float(x), filecontent, ))
            pass
        await websocket.send( json.dumps({
        ...
        print('sftp_prepare_files failed for:',e)
        traceback.print_exc()
        pass
    INFO['is_sftp_processing']  = False
    ...
    try:
        ...
        if message_name == 'open-sync-folder':
            try:
                ...
                print("open local sync folder failed for:", e)
                await websocket.send( json.dumps({
                    'name'    : 'sftp-message',
                    'info'    : "",
        ...
        if message_name == 'sftp-message':
            ...
            if not command:
                print("sftp-message without command")
                return
            ...
            if not conn or not conn.get("sftp"):
                ...
                print("failed to get sftp connection record...\n")
                return
            sftp         = conn.get("sftp")
            result       = None
            try:
                ...
                if command == "listdir":
                    ...
                    print("undealing sftp message:", message)
                    pass
            except Exception as e:
                print("failed to execute sftp command: {0} : {1}".format(command,e))
                await websocket.send(json.dumps({
                    'name'     : message.get("message_name",message_name),
                    'ckey'     : ckey,
        ...
        if message_name == 'shell-message':
            ...
            if not conn:
                ...
                print("failed to get ssh connection record ...\n")
                return
            ...
            if data:
                ...
                try:
                    ...
                    print("send data failed for:",e)
                    await close_ssh_connection(websocket, conn)
                    pass
                pass
            return
        print("undealing message:", message)
    except Exception as e:
        traceback.print_exc()
        print("data processing exception raised for:",e)
...
def run_server(): # in non-main thread
    print("starting server for remote calculate on port",port_term)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop( loop )
