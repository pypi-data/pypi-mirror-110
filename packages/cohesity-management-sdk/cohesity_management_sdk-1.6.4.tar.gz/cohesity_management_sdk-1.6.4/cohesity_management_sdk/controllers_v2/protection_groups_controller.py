...
class ProtectionGroupsController(BaseController):
    ...
    def cancel_protection_group_run(self,
                                    id,
                                    run_id,
                                    body):
        ...
        """
        try:
            self.logger.info('cancel_protection_group_run called.')
    
            # Validate required parameters
            self.logger.info('Validating required parameters for cancel_protection_group_run.')
            self.validate_parameters(id=id,
                                     run_id=run_id,
                                     body=body)
    
            # Prepare query URL
            self.logger.info('Preparing query URL for cancel_protection_group_run.')
            _url_path = '/data-protect/protection-groups/{id}/runs/{runId}/cancel'
            _url_path = APIHelper.append_url_with_template_parameters(_url_path, { 
                'id': id,
                'runId': run_id
            })
            ...
            # Prepare headers
            self.logger.info('Preparing headers for cancel_protection_group_run.')
            _headers = {
                'content-type': 'application/json; charset=utf-8'
            }
    
            # Prepare and execute request
            self.logger.info('Preparing and executing request for cancel_protection_group_run.')
            _request = self.http_client.post(_query_url, headers=_headers, parameters=APIHelper.json_serialize(body))
            CustomHeaderAuth.apply(_request, self.config)
            _context = self.execute_request(_request, name = 'cancel_protection_group_run')

            # Endpoint and global error handling using HTTP status codes.
            self.logger.info('Validating response for cancel_protection_group_run.')
            if _context.response.status_code == 0:
                raise ErrorException('Error', _context)
            self.validate_response(_context)
